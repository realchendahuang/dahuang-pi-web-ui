...
describe("recommendedCommand", () => {
  it("recommends update & restart when an update is available", () => {
    const result = recommendedCommand(status({
      release: { packageName: "@realchendahuang/dahuang-pi-web-ui", updateAvailable: true },
...
describe("installationLabel", () => {
  ...
  it("includes source and scope for pi-package installs", () => {
    expect(installationLabel({ kind: "pi-package", source: "npm:@realchendahuang/dahuang-pi-web-ui", scope: "user" }))
      .toBe("npm:@realchendahuang/dahuang-pi-web-ui · user");
  });
