...
export function rememberCachedNewSession(
	session: SessionInfo,
	machineId = defaultMachineId,
	storage = browserStorage(),
): void {
	if (session.messageCount !== 0 || session.archived === true) return;
	const sessions = loadCachedNewSessions(storage).filter(
		(candidate) =>
			candidate.id !== session.id || candidate.machineId !== machineId,
	);
	saveCachedNewSessions(
		[markCachedNewSessionInfo(session, machineId), ...sessions],
		storage,
	);
}

export function markCachedNewSessionInfo(
	session: SessionInfo,
	machineId = defaultMachineId,
): CachedNewSessionInfo {
	return { ...session, browserCachedNew: true, machineId };
}

export function forgetCachedNewSession(
	sessionId: string,
	machineId = defaultMachineId,
	storage = browserStorage(),
): void {
	const sessions = loadCachedNewSessions(storage).filter(
		(session) => session.id !== sessionId || session.machineId !== machineId,
	);
	saveCachedNewSessions(sessions, storage);
}

export function mergeCachedNewSessions(
	cwd: string,
	...
	const retainedCachedSessions = cachedSessions.filter(
		(session) => session.machineId !== machineId || !sessionIds.has(session.id),
	);
	if (retainedCachedSessions.length !== cachedSessions.length)
		saveCachedNewSessions(retainedCachedSessions, storage);
	const cached = retainedCachedSessions.filter(
		(session) => session.machineId === machineId && session.cwd === cwd,
	);
	return [...cached, ...sessions];
}

export function isCachedNewSessionInfo(
	session: SessionInfo | undefined,
): session is CachedNewSessionInfo {
...
export function stripCachedNewSessionMarker(session: SessionInfo): SessionInfo {
	return {
		...
		...("machineId" in session && typeof session.machineId === "string"
...
export function loadCachedNewSessions(
	storage = browserStorage(),
): CachedNewSessionInfo[] {
	try {
...
function saveCachedNewSessions(
	sessions: CachedNewSessionInfo[],
	storage = browserStorage(),
): void {
	try {
...
function parseCachedSession(value: unknown): CachedNewSessionInfo[] {
	...
	const runtimeIdValue = optionalStringField(value, "runtimeId");
	const runtimeId = isAgentRuntimeId(runtimeIdValue)
		? runtimeIdValue
...
function hasCachedNewMarker(
	session: SessionInfo,
): session is SessionInfo & { browserCachedNew: unknown } {
