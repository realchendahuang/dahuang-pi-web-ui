...
const notificationTimestampFormatter = new Intl.DateTimeFormat(undefined, {
	timeStyle: "short",
});
...
export class ChatView extends LitElement {
	...
	@state() private collapsedNotificationTargetKeys: ReadonlySet<string> =
		new Set();
	...
	protected override updated(changed: Map<string, unknown>): void {
		...
		if (
			...
			this.focusPendingNotificationTarget();
	...
	private renderNotificationTray() {
		...
		const hasPendingOverlay =
			inbox.pendingDismissedIds.size > 0 || inbox.dismissAllPending;
		const retainsFocusTarget =
			this.retainedEmptyNotificationTrayTargetKey === chatKey;
		const totalCount = notificationInboxTotalCount(inbox);
		if (totalCount === 0 && !hasPendingOverlay && !retainsFocusTarget)
			return null;
		const collapsed = notificationTrayIsCollapsed(
      ...
      <section class=${`notification-tray${collapsed ? " collapsed" : ""}`} role="region" aria-labelledby="session-notifications-heading" @focusout=${(
				...
				this.releaseEmptyNotificationTray(event);
			}}>
        <header class="notification-header" data-notification-focus="header" tabindex="-1">
          ...
          <div class="notification-header-actions">
            ...
            <button
              ...
              @click=${() => {
								this.toggleNotificationTray(inbox, collapsed);
							}}
        ...
        <div class="notification-list" id="session-notification-list" ?hidden=${collapsed}>
          ...
          ${inbox.notifications.map((notification) => {
						...
						const truncationLabel =
							notificationMessageTruncationLabel(notification);
						return html`
	...
	private toggleNotificationTray(
		inbox: SelectedSessionNotificationView,
		collapsed: boolean,
	): void {
	...
	private dismissNotification(notificationId: string): void {
		...
		const focusTarget = notificationFocusTargetAfterDismiss(
			inbox.notifications,
			notificationId,
		);
		const chatKey = notificationTargetKey(inbox);
		this.pendingNotificationFocus = { chatKey, focusTarget };
		if (focusTarget.kind === "header")
			this.retainedEmptyNotificationTrayTargetKey = chatKey;
	...
	private dismissAllNotifications(): void {
		const inbox = this.notificationInbox;
		if (inbox === undefined || this.onDismissAllNotifications === undefined)
			return;
		const chatKey = notificationTargetKey(inbox);
		this.pendingNotificationFocus = {
			chatKey,
			focusTarget: { kind: "header" },
		};
		this.retainedEmptyNotificationTrayTargetKey = chatKey;
	...
	private releaseEmptyNotificationTray(event: FocusEvent): void {
		...
		if (
			inbox !== undefined &&
			this.retainedEmptyNotificationTrayTargetKey ===
				notificationTargetKey(inbox) &&
			notificationInboxTotalCount(inbox) === 0
		)
			this.retainedEmptyNotificationTrayTargetKey = undefined;
	}
	...
	private focusPendingNotificationTarget(): void {
		...
		if (
			pending === undefined ||
			inbox === undefined ||
			notificationTargetKey(inbox) !== pending.chatKey
		)
			return;
		const target = pending.focusTarget;
		if (target.kind === "header") {
			this.renderRoot
				.querySelector<HTMLElement>("[data-notification-focus='header']")
				?.focus();
			return;
		}
		const row = Array.from(
			this.renderRoot.querySelectorAll<HTMLElement>("[data-notification-id]"),
		).find(
			(candidate) =>
				candidate.dataset["notificationId"] === target.notificationId,
		);
		if (row !== undefined) {
			row.focus();
			return;
		}
		if (notificationInboxTotalCount(inbox) === 0)
			this.retainedEmptyNotificationTrayTargetKey = pending.chatKey;
		this.renderRoot
			.querySelector<HTMLElement>("[data-notification-focus='header']")
			?.focus();
	}
    ...
    `;
		...
		if (part.type === "toolExecution")
			return html`<tool-card class="part" .execution=${part}></tool-card>`;
