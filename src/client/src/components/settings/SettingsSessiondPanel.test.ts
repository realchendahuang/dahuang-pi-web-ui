...
describe("session daemon panel notices", () => {
	it("names the selected machine in the scope description and restart notice", () => {
		...
		expect(notices[0]?.title).toBe(
			t("settings.sessiond.restartTitle", { target: targetLabel }),
		);
	...
	it("orders save/load notices before the restart notice", () => {
		...
		expect(notices[2]?.title).toBe(
			t("settings.sessiond.restartTitle", { target: "local (local gateway)" }),
		);
	});
	...
	it("reports only the blocking error and no activation guidance when config is unavailable", () => {
		...
		expect(notices).toEqual([
			{
				type: "error",
				content: "Selected-machine settings are not available on Lab Mac.",
			},
		]);
	});
});
...
describe("session daemon panel save behavior", () => {
	it("submits command and directory together as one profile save", async () => {
		...
		setPanelConfig(
			panel,
			configResponse({ agent: { command: "pi", dir: "/srv/pi" } }),
		);
		setPanelProperty(panel, "agentDraft", {
			command: " alternate-agent ",
			dir: " /srv/alternate ",
		});
		panel.onSave = onSave;
		...
		expect(onSave.mock.calls).toEqual([
			[{ agent: { command: "alternate-agent", dir: "/srv/alternate" } }],
		]);
	});
	...
	it("preserves a dirty profile draft when an unrelated daemon setting is saved", () => {
		...
		const initial = configResponse({
			agent: { command: "pi", dir: "/srv/pi" },
			spawnSessions: false,
		});
		setPanelConfig(panel, initial);
		callPanelMethod(panel, "updateAgentDraft", {
			command: "alternate-agent",
			dir: "/srv/alternate",
		});

		const toggled = configResponse({
			agent: { command: "pi", dir: "/srv/pi" },
			spawnSessions: true,
		});
		...
		callPanelMethod(
			panel,
			"willUpdate",
			new Map([["configResponse", initial]]),
		);

		expect(Reflect.get(panel, "agentDraft")).toEqual({
			command: "alternate-agent",
			dir: "/srv/alternate",
		});
		...
		callPanelMethod(
			panel,
			"willUpdate",
			new Map([["configResponse", toggled]]),
		);
...
function noticeContext(
	overrides: Partial<SessiondPanelNoticeContext>,
): SessiondPanelNoticeContext {
	return {
		error: "",
		savedMessage: "",
		activeProfile: undefined,
...
function setPanelConfig(
	...
	callPanelMethod(
		panel,
		"willUpdate",
		new Map([["configResponse", undefined]]),
	);
}
...
function setPanelProperty(
	...
	if (!Reflect.set(panel, property, value))
		throw new Error(`Failed to set SettingsSessiondPanel property ${property}`);
}
...
async function callPanelPromise(
	...
	if (!(result instanceof Promise))
		throw new Error(
			`SettingsSessiondPanel.${methodName} did not return a promise`,
		);
	await result;
}
...
function callPanelMethod(
	...
	if (typeof method !== "function")
		throw new Error(`SettingsSessiondPanel.${methodName} is not callable`);
