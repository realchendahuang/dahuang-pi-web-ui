...
describe("settings-plugins-panel layout", () => {
	it("orders load and save notices before the trusted-code warning and plugin content", () => {
		...
		panel.configResponse = configResponse({
			plugins: { "remote-enabled": { enabled: true } },
		});
	...
	it("does not show a false empty state when the plugin response is missing", () => {
		...
		expect(rendered).toContain(
			t("settings.plugins.listUnavailable", {
				target: "Lab Mac (remote machine)",
			}),
		);
		expect(rendered).not.toContain(
			t("settings.plugins.empty", { target: "Lab Mac (remote machine)" }),
		);
	...
	it("keeps loaded plugins visible but disabled when selected-machine config is unavailable", () => {
		...
		const rendered = flattenTemplateContent(panel.render());
		const unavailable = t("settings.sessiond.unavailable");
		...
		expect(countOccurrences(rendered, unavailable)).toBe(1);
		expect(
			templateValues(
				renderPluginTemplate(panel, pluginInfo("remote-disabled", false)),
			).filter(isBoolean),
		).toEqual([false, true]);
	});
});
...
function renderPluginTemplate(
	panel: SettingsPluginsPanel,
	plugin: PiWebPluginInfo,
): TemplateResult {
	const renderPlugin: unknown = Reflect.get(panel, "renderPlugin");
	if (!isPanelRenderPlugin(renderPlugin))
		throw new Error("SettingsPluginsPanel.renderPlugin is not callable");
	return renderPlugin.call(panel, plugin);
}

function isPanelRenderPlugin(
	value: unknown,
): value is (
	this: SettingsPluginsPanel,
	plugin: PiWebPluginInfo,
) => TemplateResult {
