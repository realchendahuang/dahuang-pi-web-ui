...
describe("settings-shortcuts-panel layout", () => {
	it("renders header, ordered notices, and shortcut settings through the shared frame", () => {
		...
		expect(frameNotices(template).map((notice) => notice.type)).toEqual([
			"error",
			"success",
		]);
		expectTextOrder(rendered, [
			...
			t("settings.shortcuts.promptEnterTitle"),
			t("common.configFile"),
	...
	it("keeps the prompt-enter card before the loading shortcuts state", () => {
		...
		expect(rendered).not.toContain(t("common.configFile"));
	});
});
...
describe("settings-shortcuts-panel shortcut row actions", () => {
	it("saves edited shortcuts, disables them with None, and resets overrides", () => {
		...
		const savePanel = panelWithShortcuts(
			{ shortcuts: { "core:other": "mod+o" } },
			onSave,
		);

		findTemplateEventHandler<Event>(
			savePanel.render(),
			"@input=",
		)(
			new EventWithTarget(
				"input",
				new FakeHTMLInputElement(" control + shift + p "),
			),
		);

		expectTextOrder(flattenTemplateContent(savePanel.render()), [
			"Open palette",
			"Ctrl+Shift+P",
			`${t("settings.shortcuts.custom")} · ${t("settings.shortcuts.unsaved")}`,
		]);
		...
		const nonePanel = panelWithShortcuts(
			{
				shortcuts: {
					"core:open-palette": "mod+shift+p",
					"core:other": "mod+o",
				},
			},
			onSave,
		...
		const resetPanel = panelWithShortcuts(
...
function frameNotices(template: TemplateResult): readonly SettingsNotice[] {
	...
	if (notices === undefined)
		throw new Error("Expected settings-panel-frame notices to be rendered");
	return notices;
}
...
function isSettingsNoticeArray(
	value: unknown,
): value is readonly SettingsNotice[] {
...
function findTemplateEventHandler<E extends Event>(
	template: TemplateResult,
	marker: string,
): TemplateEventHandler<E> {
	const handler = findOptionalTemplateEventHandler<E>(template, marker);
	if (handler === undefined)
		throw new Error(`Expected template event handler near ${marker}`);
	return handler;
}

function findOptionalTemplateEventHandler<E extends Event>(
	template: TemplateResult,
	marker: string,
	...
	function findInTemplate(
		...
		for (let index = 0; index < values.length; index += 1) {
			...
			if (
				isTemplateEventHandler<E>(value) &&
				templateEventHandlerMatches(strings, index, marker)
			)
				return value;
...
function templateEventHandlerMatches(
	strings: readonly string[],
	valueIndex: number,
	marker: string,
): boolean {
	return (
		(strings[valueIndex] ?? "").includes(marker) ||
		(strings[valueIndex + 1] ?? "").includes(marker)
	);
}

function isTemplateEventHandler<E extends Event>(
	value: unknown,
