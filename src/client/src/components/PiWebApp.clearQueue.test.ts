...
describe("PiWebApp queued-message clear wiring", () => {
	it("passes a stable supported-runtime callback through to SessionController", () => {
		...
		const state = stateWithRuntime(
			runtimeWithCapabilities([PI_WEB_CAPABILITIES.sessionsClearQueue]),
		);
		setAppState(app, state);
		const controller = appSessionController(app);
		const clearServerQueue = vi
			.spyOn(controller, "clearServerQueue")
			.mockResolvedValue(undefined);

		const firstRender = renderChatView(app, state);
		const secondRender = renderChatView(app, state);
		const firstCallback = templateCallbackAfterMarker(
			firstRender,
			".onClearServerQueue=",
		);
		const secondCallback = templateCallbackAfterMarker(
			secondRender,
			".onClearServerQueue=",
		);

		expect(templateValueAfterMarker(firstRender, ".canClearServerQueue=")).toBe(
			true,
		);
...
function runtimeWithCapabilities(
	capabilities: NonNullable<MachineRuntime["capabilities"]>,
): MachineRuntime {
	return {
		machineId: "local",
		ok: true,
...
function templateCallbackAfterMarker(
	...
	marker: string,
): ClearServerQueueCallback {
	const value = templateValueAfterMarker(template, marker);
	if (!isClearServerQueueCallback(value))
		throw new Error(`Expected callback after ${marker}`);
	return value;
}

function isClearServerQueueCallback(
	value: unknown,
): value is ClearServerQueueCallback {
	return typeof value === "function";
}
