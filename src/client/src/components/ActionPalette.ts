	...
	override render() {
      ...
      <div class="backdrop" @mousedown=${() => this.onCancel?.()}>
        <section @mousedown=${(event: MouseEvent) => {
          ...
          <header>
            <span class="search-icon" aria-hidden="true">${appIcon("search", { size: 16 })}</span>
            <input
            ...
            >
            <button title=${t("palette.close")} aria-label=${t("palette.close")} @click=${() => this.onCancel?.()}>${appIcon("close", { size: 16 })}</button>
          </header>
          <div class="options">
            ${
							actions.length === 0
								...
								: actions.map(
										(action, index) => html`
              <button class=${`${index === this.selectedIndex ? "selected" : ""} ${action.enabled === false ? "disabled" : ""}`} ?disabled=${action.enabled === false} title=${action.disabledReason ?? action.title} ${scrollWhenSelected(index === this.selectedIndex, action.id)} @click=${() => {
								this.run(action);
							}}>
                <span class="main">
...
export function filterActionPaletteActions(
	...
	return actions
		.filter(
			(action) =>
				action.enabled !== false || action.disabledReason !== undefined,
		)
		.filter((action) => {
