...
describe("suggestedMachineNameFromUrl", () => {
  it("suggests the host without protocol or port", () => {
    expect(suggestedMachineNameFromUrl("http://127.0.0.1:31415")).toBe("127.0.0.1");
    expect(suggestedMachineNameFromUrl("https://devbox.example.test:31415/pi-web")).toBe("devbox.example.test");
  });
...
describe("machineBaseUrlValidationMessage", () => {
  it("accepts http and https base URLs", () => {
    expect(machineBaseUrlValidationMessage("http://127.0.0.1:31415")).toBeUndefined();
  ...
  it("explains invalid machine URLs", () => {
    ...
    expect(machineBaseUrlValidationMessage("devbox.local:31415")).toBe("Use an http:// or https:// URL.");
