...
describe("PiWebApp session unread wiring", () => {
	it("shows a server completion for a background chat and acknowledges the exact observed order when viewed", async () => {
		...
		expect(mobileNavigationTab(app)).toMatchObject({
			badge: 1,
			badgeLabel: "1 unread",
			badgeTone: "unread",
		});
		...
		expect(fetchMock.mock.calls[0]?.[0]).toBe(
			"https://pi.example.test/api/machines/local/sessions/background/unread/acknowledge",
		);
	...
	it("keeps the selected chat unread while hidden or unfocused, then acknowledges on a visible focus check", async () => {
		...
		vi.stubGlobal("document", {
			...
			get visibilityState() {
				return documentState.visible ? "visible" : "hidden";
			},
	...
	it("does not acknowledge a selected chat hidden behind a full-screen dialog", async () => {
		...
		exposeSelectedChat(app);
		if (!Reflect.set(app, "settingsSection", "general"))
		...
		if (!Reflect.set(app, "settingsSection", undefined))
			throw new Error("Could not close settings");
		invokeUpdated(app);
		await vi.waitFor(() => {
			expect(navigationUnreadSessionIds(app).size).toBe(0);
		});
	...
	it("fetches initial state only after the selected runtime advertises unread support", async () => {
		...
		const durable = {
			catalogId: "catalog-a",
			catalogRevision: 1,
			sessions: [unreadSummary(background, 1)],
		};
		...
		setState(app, {
			machineRuntimes: {
				local: {
					...
					ok: true,
					checkedAt: "now",
					capabilities: [PI_WEB_CAPABILITIES.sessionsUnread],
				},
			},
		});
		await vi.waitFor(() => {
		...
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchMock.mock.calls[0]?.[0]).toBe(
			"https://pi.example.test/api/machines/local/sessions/unread",
		);
	});
	...
	it("renegotiates cached support before refreshing after a runtime rollback", async () => {
		const fetchMock = vi.fn(() =>
			Promise.reject(new Error("Unexpected unread request")),
		);
		vi.stubGlobal("fetch", fetchMock);
		const app = createApp();
		enableUnread(app);
		...
		if (typeof machines !== "object" || machines === null)
			throw new Error("PiWebApp machine controller is unavailable");
		if (
			!Reflect.set(machines, "refreshMachineRuntime", () => {
				const runtime = {
					machineId: "local",
					ok: true as const,
					checkedAt: "after",
	...
	it("does not treat the legacy browser-local key as unread authority", () => {
		const app = createApp({
			"pi-web-session-unread-v1": JSON.stringify([["local", "background"]]),
		});
		const foreground = session("foreground");
		const background = session("background");
		setAppState(app, {
...
function setAppState(app: PiWebApp, state: AppState): void {
	if (!Reflect.set(app, "state", state))
		throw new Error("Could not set PiWebApp state");
}

function setState(app: PiWebApp, patch: Partial<AppState>): void {
	const method: unknown = Reflect.get(app, "setState");
	if (!isSetAppState(method))
		throw new Error("PiWebApp.setState is not callable");
...
function handleRealtimeEvent(app: PiWebApp, event: BrowserRealtimeEvent): void {
	...
	if (!isHandleRealtimeEvent(method))
		throw new Error("PiWebApp.handleRealtimeEvent is not callable");
	method.call(app, "local", event);
}

function enableUnread(app: PiWebApp): void {
	if (!Reflect.set(app, "unreadConnected", true))
		throw new Error("Could not connect PiWebApp unread state");
	const controller: unknown = Reflect.get(app, "sessionUnread");
	if (typeof controller !== "object" || controller === null)
		throw new Error("PiWebApp unread controller is unavailable");
	const setCapability: unknown = Reflect.get(controller, "setCapability");
	if (typeof setCapability !== "function")
		throw new Error("PiWebApp unread capability setter is unavailable");
...
function exposeSelectedChat(app: PiWebApp): void {
	...
	if (typeof state !== "object" || state === null)
		throw new Error("PiWebApp state is unavailable");
	const selectedSession: unknown = Reflect.get(state, "selectedSession");
	if (typeof selectedSession !== "object" || selectedSession === null)
		throw new Error("Expected a selected chat");
	...
	if (typeof sessionId !== "string" || typeof cwd !== "string")
		throw new Error("Selected chat identity is invalid");
	const selectedMachine: unknown = Reflect.get(state, "selectedMachine");
	const machineId =
		typeof selectedMachine === "object" &&
		selectedMachine !== null &&
		typeof Reflect.get(selectedMachine, "id") === "string"
	...
	if (
		!Reflect.set(
			app,
			"readyChatIdentity",
			JSON.stringify([machineId, sessionId, cwd]),
		)
	) {
...
function setMobileNavigationLayout(app: PiWebApp, mobile: boolean): void {
	...
	if (
		typeof appShell !== "object" ||
		appShell === null ||
		!Reflect.set(appShell, "isMobileNavigationLayout", mobile)
	) {
...
function invokeUpdated(app: PiWebApp): void {
	...
	if (!isUpdatedHook(method))
		throw new Error("PiWebApp.updated is not callable");
	method.call(app);
}

function invokeDisconnected(app: PiWebApp): void {
	const method: unknown = Reflect.get(app, "disconnectedCallback");
	if (!isDisconnectedHook(method))
		throw new Error("PiWebApp.disconnectedCallback is not callable");
	method.call(app);
}

async function renegotiateUnreadMachine(
	app: PiWebApp,
	machineId: string,
): Promise<void> {
	const method: unknown = Reflect.get(app, "renegotiateUnreadMachine");
	if (!isRenegotiateUnreadMachine(method))
		throw new Error("PiWebApp unread renegotiation is not callable");
...
function refreshUnread(app: PiWebApp, machineId: string): Promise<void> {
	...
	if (!isRefreshUnread(refresh))
		throw new Error("PiWebApp unread refresh is not callable");
...
function mobileNavigationTab(app: PiWebApp): AppMobileMainTab {
	...
	if (!isMobileMainTabs(method))
		throw new Error("PiWebApp.mobileMainTabs is not callable");
	const tab = method
		.call(app)
		.find((candidate) => candidate.id === "navigation");
...
function navigationUnreadSessionIds(app: PiWebApp): ReadonlySet<string> {
	...
	if (!isRenderNavigationPanel(method))
		throw new Error("PiWebApp.renderNavigationPanel is not callable");
...
function unreadEvent(
	...
	target: SessionInfo = unread === null
		? session("selected")
...
function isRenderNavigationPanel(
	value: unknown,
): value is RenderNavigationPanel {
...
function isRenegotiateUnreadMachine(
	value: unknown,
): value is RenegotiateUnreadMachine {
	return typeof value === "function";
}
