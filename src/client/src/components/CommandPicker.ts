...
export class CommandPicker extends LitElement {
	...
	@property() override title = t("commandPicker.select");
	...
	override render() {
      ...
      <div class="backdrop" @mousedown=${() => this.onCancel?.()}>
        <section @mousedown=${(event: MouseEvent) => {
          ...
          ${
						this.searchable
							? html`<input placeholder=${t("commandPicker.search")} .value=${this.query} @input=${(
									event: Event,
								) => {
          ...
          <div class="options" @keydown=${(event: KeyboardEvent) => {
						this.handleKeyDown(event);
					}} tabindex="0">
            ${options.map(
							(option, index) => html`
            ...
            ${options.length === 0 ? html`<div class="empty">${t("commandPicker.noMatch")}</div>` : null}
          </div>
        </section>
      </div>
    `;
	...
	override firstUpdated() {
		...
		this.renderRoot
			.querySelector<HTMLElement>(this.searchable ? "input" : ".options")
			?.focus();
	}
