...
import {
	...
	sessionTreeEntryReturnsToEditor,
	sessionTreeVisualDepth,
} from "./SessionTreeNavigator";
...
type SummaryModeMethod = (
	this: SessionTreeNavigator,
...
describe("session-tree-navigator interactions", () => {
	...
	it("uses pointer selection for explicit navigation and retains it after cancellation", async () => {
		...
		expect(componentProperty(navigator, "statusMessage")).toContain(
			"selected history entry is unchanged",
		);
	...
	it("submits trimmed custom focus, exposes busy cancellation, and returns to the same node", async () => {
		...
		expect(onNavigate).toHaveBeenCalledWith("active", {
			mode: "custom",
			instructions: "focus on failed tests",
		});
	...
	it("clears transient cancelling status if navigation rejects after abort", async () => {
		...
		expect(componentProperty(navigator, "statusMessage")).toBe("");
		expect(componentProperty(navigator, "error")).toBe(
			"Could not navigate session history: remote operation failed",
		);
	});
	...
	it("keeps navigation failures actionable and local to the confirmation step", async () => {
		...
		expect(componentProperty(navigator, "busy")).toBe(false);
		expect(componentProperty(navigator, "error")).toBe(
			"Could not navigate session history: The session changed since /tree was opened. Reopen /tree and try again.",
		);
	});
	...
	it("focuses the active leaf selected when the dialog opens", () => {
		...
		const root = {
			...
			querySelectorAll: () => [
				{
					dataset: { treeNodeId: "root" },
					focus: vi.fn(),
					scrollIntoView: vi.fn(),
				},
				{
					dataset: { treeNodeId: "active" },
					focus: activeFocus,
					scrollIntoView: activeScroll,
				},
			],
		};
		if (!Reflect.set(navigator, "renderRoot", root))
			throw new Error("Could not install navigator render root");
	...
	it("keeps an empty tree inert and moves initial focus to the close boundary", async () => {
		...
		const root = {
			querySelector: (selector: string) =>
				selector === ".close-button" ? { focus: closeFocus } : null,
			querySelectorAll: () => [],
		};
		if (!Reflect.set(navigator, "renderRoot", root))
			throw new Error("Could not install navigator render root");
...
function componentProperty(
	navigator: SessionTreeNavigator,
	property: string,
): unknown {
...
function setComponentProperty(
	...
	if (!Reflect.set(navigator, property, value))
		throw new Error(`Could not set navigator property ${property}`);
}
...
function callVoidMethod(
	...
	if (!isVoidMethod(method))
		throw new Error(`SessionTreeNavigator.${methodName} is not callable`);
...
function callSummaryModeMethod(
	navigator: SessionTreeNavigator,
	mode: SessionTreeSummaryChoice["mode"],
): void {
	const method: unknown = Reflect.get(navigator, "selectSummaryMode");
	if (!isSummaryModeMethod(method))
		throw new Error("SessionTreeNavigator.selectSummaryMode is not callable");
