...
import {
	loadAttachmentDelivery,
	saveAttachmentDelivery,
...
export class PromptEditor extends LitElement {
	...
	@state() private attachmentDelivery: PromptAttachmentDelivery =
		loadAttachmentDelivery();
	...
	override render() {
		...
		return html`
      <footer class=${shellMode ? "shell-mode" : ""} @paste=${(
        ...
        <div class="actions">
          ...
          <button class="icon-button send-button" ?disabled=${busy} title=${queuesInput ? t("composer.queueTitle") : t("composer.send")} aria-label=${queuesInput ? t("composer.queue") : t("composer.send")} @click=${() => {
						this.send("followUp");
					}}>${queuesInput ? renderQueueIcon() : renderSendIcon()}</button>
          ${
						this.canSteer && !this.isCompacting
							? html`<button class="icon-button steer-button" ?disabled=${busy} title=${t("composer.steerTitle")} aria-label=${t("composer.steer")} @click=${() => {
									this.send("steer");
								}}>${renderSteerIcon()}</button>`
							: null
					}
          <button class="icon-button stop-button" ?disabled=${this.disabled || !this.canStop} title=${this.canStop ? t("composer.stopTitle") : t("composer.stopIdle")} aria-label=${t("composer.stop")} @click=${() => this.onStop?.()}>${renderStopIcon()}</button>
        </div>
      </footer>
    `;
	}
	...
	private renderAttachments() {
		if (this.attachments.length === 0 && this.attachmentError === undefined)
			return null;
		const canUseInlineDelivery = promptAttachmentsCanUseInlineDelivery(
			this.attachments,
		);
		const delivery = this.effectiveAttachmentDelivery();
		return html`
      <div class="attachments" aria-label=${t("composer.attachments")}>
        ${this.attachments.map(
          ...
          <div class=${`attachment-chip ${isInlinePromptAttachment(attachment) ? "attachment-chip-image" : "attachment-chip-file"}`} title=${attachment.name}>
            ...
            <button type="button" class="attachment-remove" title=${t("composer.removeAttachment", { name: attachment.name })} aria-label=${t("composer.removeAttachment", { name: attachment.name })} @click=${() => {
        ...
        ${
					this.attachments.length > 0
						? html`
          <label class="attachment-delivery" title=${t("composer.delivery")}>
            <select .value=${delivery} @change=${(event: Event) => {
							this.changeDelivery(event);
						}}>
              <option value="inline" ?disabled=${!canUseInlineDelivery}>${canUseInlineDelivery ? t("composer.deliveryInline") : t("composer.deliveryInlineImagesOnly")}</option>
              <option value="folder">${t("composer.deliveryFolder")}</option>
            </select>
          </label>
        `
						: null
	...
	private changeDelivery(event: Event) {
		...
		if (
			requested === "inline" &&
			!promptAttachmentsCanUseInlineDelivery(this.attachments)
		) {
	...
	private currentAttachments(): PromptAttachment[] {
		return this.attachments.map((attachment) =>
			pendingToPromptAttachment(attachment),
		);
	}

	private effectiveAttachmentDelivery(): PromptAttachmentDelivery {
		return effectivePromptAttachmentDelivery(
			...
			state: EditorState.create({
				...
				extensions: [
					...
					EditorView.contentAttributes.of((view) =>
						inputAssistanceContentAttributes(
	...
	private updateDraft(value: string) {
		...
		if (!inputModesEqual(nextInputMode, this.currentInputMode))
			this.currentInputMode = nextInputMode;
	...
	private async refreshCompletions() {
		...
		if (
			...
			this.completions = commands
				.filter((command) =>
					command.name.toLowerCase().includes(trigger.query.toLowerCase()),
				)
				.slice(0, 12)
				.map((command) => ({
					kind: "command",
			...
			const files = await api
				.files(this.cwd, trigger.query, {
					scope: trigger.fileScope,
					machineId: this.machineId,
					projectId: this.projectId,
					workspaceId: this.workspaceId,
					workspaceScoped: this.workspaceScopedFileSuggestions,
				})
				.catch(emptyFileSuggestions);
			if (version !== this.requestVersion) return;
			this.completions = files.slice(0, 12).map((file) => {
	...
	private currentTrigger(): PromptCompletionTrigger | undefined {
		return detectPromptCompletionTrigger(
			this.draft,
			this.editor?.state.selection.main.head ?? this.draft.length,
		);
	}
	...
	private moveCompletion(delta: number): boolean {
		...
		this.selectedIndex =
			(this.selectedIndex + delta + this.completions.length) %
	...
	private pick(item: CompletionItem) {
		...
		const suffix =
			item.kind === "file" &&
			(item.insertText.endsWith("/") || item.cursorOffset !== undefined)
				? ""
				: " ";
		const cursor =
			item.replaceFrom +
		...
		const replaceTo =
			...
			this.draft.slice(item.replaceTo).startsWith('"')
				? item.replaceTo + 1
				: item.replaceTo;
		editor.dispatch({
			changes: {
	...
	private send(streamingBehavior?: "steer" | "followUp") {
		...
		const behavior =
			this.canSteer || this.isCompacting ? streamingBehavior : undefined;
		const attachments =
			pending.length > 0 ? this.currentAttachments() : undefined;
		const delivery = this.effectiveAttachmentDelivery();
		this.resetComposer();
		...
		void this.onSend?.(
			...
			behavior,
			attachments,
			attachments === undefined ? undefined : delivery,
		);
	}
...
function sessionStatusRenderEqual(
...
function fileExtensionLabel(name: string): string {
	...
	if (dotIndex >= 0 && dotIndex < trimmed.length - 1)
		return trimmed.slice(dotIndex + 1, dotIndex + 5).toUpperCase();
	return "FILE";
}
...
function inputAssistanceContentAttributes(
	...
	return inputModeForDraft(draftBeforeCursor).kind === "normal"
		? proseInputAssistanceAttributes
		: codeLikeInputAssistanceAttributes;
}
