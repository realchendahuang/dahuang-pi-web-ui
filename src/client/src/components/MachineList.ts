	...
	implements KeyboardNavigableSection
{
	...
	protected override updated(changed: PropertyValues<this>): void {
		if (
			...
			this.openMenuMachineId !== undefined &&
			!this.machines.some((machine) => machine.id === this.openMenuMachineId)
		)
			this.openMenuMachineId = undefined;
	...
	private renderMachine(machine: Machine) {
		const status =
			this.statuses[machine.id]?.status ?? machine.status ?? "unknown";
		const statusLabel =
			status === "online"
				? "online"
				: status === "offline"
		...
		const hasRemoveAction =
			canRemoveMachine(machine) && this.onRemove !== undefined;
		return html`
      <div
        ...
        @click=${(event: MouseEvent) => {
					activateSelectableRow(event, () => this.onSelect?.(machine));
				}}
        @keydown=${(event: KeyboardEvent) => {
					this.handleMachineKeydown(event, machine);
				}}
      >
        <div class="action-main">
          <span class="action-name machine-primary"><span class="machine-primary-label">${machine.name}</span></span><small>${machine.kind === "local" ? t("machine.local") : (machine.baseUrl ?? t("machine.remote"))} · ${statusLabel}</small>
	...
	private renderActivity(machine: Machine) {
		...
		return renderActionActivityIndicator(
			kind,
			kind === "terminal" ? t("machine.terminalActive") : t("machine.activeLabel"),
		);
	}
      ...
      <div class="action-menu">
        <button
          ...
          @click=${(event: MouseEvent) => {
						...
						this.toggleMenu(machine.id, event.currentTarget);
					}}
        >⋯</button>
        ${
					open
						? html`
          <div class="action-menu-panel machine-menu-panel" id=${menuId} style=${this.menuStyle} @click=${(
						event: MouseEvent,
					) => {
						event.stopPropagation();
					}}>
            <button class="danger" title=${`Remove ${machine.name}`} @click=${() => {
							this.removeMachine(machine);
						}}>${t("machine.removeBtn")}</button>
          </div>
        `
						: null
				}
	...
	private renderHeading() {
		...
		const selectedSummary = this.selected?.name ?? t("nav.machines");
