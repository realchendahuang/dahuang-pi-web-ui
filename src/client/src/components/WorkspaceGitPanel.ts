...
import {
	buildGitFileList,
	type GitFileListModel,
	type GitFileListSubmoduleFile,
	type GitFileListSubmoduleGroup,
} from "../gitFileList";
import {
	readGitFileView,
	writeGitFileView,
...
const EMPTY_VIEW_STATE: GitViewState = {
	nodes: [],
...
export class WorkspaceGitPanel extends LitElement {
	...
	protected override willUpdate(changedProperties: PropertyValues<this>): void {
		...
		if (
			...
			gitPanelContextKey(previous) !== gitPanelContextKey(this.context)
		) {
	...
	override render(): TemplateResult {
      ...
      <section class="toolbar">
        ...
        <div class="toolbar-actions">
          ...
          <button type="button" @click=${context.onRefreshGit}>${t("git.refresh")}</button>
        </div>
      </section>
      <section class="split">
        <div class="list">
	...
	private renderViewToggle(): TemplateResult {
		return html`
      <div class="view-toggle" role="group" aria-label=${t("git.viewMode")}>
        ${this.renderViewToggleButton("list", t("git.viewList"))}
        ${this.renderViewToggleButton("tree", t("git.viewTree"))}
      </div>
    `;
	}

	private renderViewToggleButton(
		view: GitFileView,
		label: string,
	): TemplateResult {
		const active = this.view === view;
		return html`
      <button type="button" class=${active ? "selected" : ""} aria-pressed=${active ? "true" : "false"} @click=${() => {
				this.setView(view);
			}}>${label}</button>
    `;
	}
	...
	private renderExpandCollapseAll(
		...
		const allExpanded = expandablePaths.every((path) =>
			this.expandedDirectories.has(path),
		);
		return html`
      <button type="button" @click=${() => {
				this.toggleExpandAll(expandablePaths, allExpanded);
			}}>${allExpanded ? t("git.collapseAll") : t("git.expandAll")}</button>
    `;
	}
	...
	private renderSubmoduleGroup(
		context: WorkspacePanelContext,
		group: GitFileListSubmoduleGroup,
	): TemplateResult {
		const expanded = this.expandedDirectories.has(group.path);
		return html`
      <button type="button" class="row" style="--depth:0" aria-expanded=${expanded ? "true" : "false"} @click=${() => {
				this.toggleDirectory(group.path);
			}}>
	...
	private renderSubmoduleFileRow(
		context: WorkspacePanelContext,
		entry: GitFileListSubmoduleFile,
	): TemplateResult {
		return this.renderSelectableRow(
			context,
			entry.path,
			entry.relativePath,
			entry.file,
	...
	private renderTreeNode(
		...
		if (node.kind === "directory") {
			...
			return html`
        <button type="button" class="row" style=${`--depth:${String(depth)}`} aria-expanded=${expanded ? "true" : "false"} @click=${() => {
	...
	private renderSelectableRow(
		...
		return html`
      <button type="button" class=${selected ? "row selected" : "row"} style=${`--depth:${String(depth)}`} @click=${() => {
				context.onSelectDiff(path);
			}}>
	...
	private buildViewState(status: GitStatusResponse | undefined): GitViewState {
		...
		if (this.view === "tree") {
			...
			return {
				nodes,
				listModel: EMPTY_LIST_MODEL,
				expandablePaths: collectGitFileTreeDirectoryPaths(nodes),
			};
		}
		...
		return {
			nodes: [],
			listModel,
			expandablePaths: listModel.submodules.map((group) => group.path),
		};
	}
	...
	private toggleExpandAll(
		...
		this.expandedDirectories = allExpanded
			? new Set()
