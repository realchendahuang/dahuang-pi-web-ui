...
type RenderSessionTreeNavigator = (
	this: PiWebApp,
	state: AppState,
) => TemplateResult | null;
...
describe("PiWebApp session tree wiring", () => {
	it("routes navigation, cancellation, abort, and prompt focus through SessionController", async () => {
		...
		const navigateTree = vi
			.spyOn(controller, "navigateTree")
			.mockResolvedValueOnce({ cancelled: false, editorText: "edit" })
			.mockResolvedValueOnce({ cancelled: true, aborted: true });
		const abortTreeNavigation = vi
			.spyOn(controller, "abortTreeNavigation")
			.mockResolvedValue(undefined);
		const closeTreeDialog = vi
			.spyOn(controller, "closeTreeDialog")
			.mockReturnValue(undefined);
		const focusChatComposer = vi.fn(() => Promise.resolve());
		if (!Reflect.set(app, "focusChatComposer", focusChatComposer))
			throw new Error("Could not replace prompt focus boundary");
		...
		await expect(onNavigate("side", { mode: "none" })).resolves.toEqual({
			cancelled: false,
			editorText: "edit",
		});
		...
		await expect(onNavigate("root", { mode: "default" })).resolves.toEqual({
			cancelled: true,
			aborted: true,
		});
		expect(focusChatComposer).toHaveBeenCalledOnce();
	...
	it("does not steal focus after the user selects another session during navigation", async () => {
		...
		if (!Reflect.set(app, "focusChatComposer", focusChatComposer))
			throw new Error("Could not replace prompt focus boundary");
		const onNavigate = navigatorNavigateHandler(
			renderSessionTreeNavigator(app, state),
		);
		...
		const otherSession =
			state.selectedSession === undefined
				? undefined
				: { ...state.selectedSession, id: "session-2" };
		if (!Reflect.set(app, "state", { ...state, selectedSession: otherSession }))
			throw new Error("Could not change selected session");
...
function setAppTree(
	...
	const state = {
		...
		treeDialog: treeSnapshot,
	};
	if (!Reflect.set(app, "state", state))
		throw new Error("Could not set PiWebApp tree state");
	return state;
}

function renderSessionTreeNavigator(
	app: PiWebApp,
	state: AppState,
): TemplateResult {
	const method: unknown = Reflect.get(app, "renderSessionTreeNavigator");
	if (!isRenderSessionTreeNavigator(method))
		throw new Error("PiWebApp.renderSessionTreeNavigator was unavailable");
	const rendered = method.call(app, state);
	if (rendered === null)
		throw new Error("Expected a rendered session tree navigator");
	return rendered;
}
...
function appSessionController(app: PiWebApp): SessionController {
	...
	if (!(controller instanceof SessionController))
		throw new Error("PiWebApp SessionController was unavailable");
	return controller;
}
...
function navigatorNavigateHandler(template: TemplateResult): NavigateHandler {
	...
	if (!isNavigateHandler(value))
		throw new Error("Session tree navigate callback was unavailable");
	return value;
}

function navigatorAbortHandler(template: TemplateResult): AbortHandler {
	const value = templateValueAfterMarker(template, ".onAbort=");
	if (!isAbortHandler(value))
		throw new Error("Session tree abort callback was unavailable");
	return value;
}
...
function navigatorCancelHandler(template: TemplateResult): CancelHandler {
	...
	if (!isCancelHandler(value))
		throw new Error("Session tree cancel callback was unavailable");
	return value;
}

function isRenderSessionTreeNavigator(
	value: unknown,
): value is RenderSessionTreeNavigator {
