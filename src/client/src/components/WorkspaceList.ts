	...
	implements KeyboardNavigableSection
{
	...
	protected override updated(changed: PropertyValues<this>): void {
		if (
			...
			!this.workspaces.some(
				(workspace) => workspace.id === this.openMenuWorkspaceId,
			)
		)
			this.openMenuWorkspaceId = undefined;
	...
	async focusSelectedOrFirst(): Promise<boolean> {
		...
		return focusSelectedOrFirstSelectableRow(this.renderRoot, {
			fallbackSelector: ".section-toggle",
		});
	}
            ...
            ${this.workspaces.map((workspace) => {
                ...
                <div
                  ...
                  @keydown=${(event: KeyboardEvent) => {
										this.handleWorkspaceKeydown(event, workspace);
									}}
                >
	...
	private renderHeading() {
		...
		const selectedSummary =
			this.selected === undefined
				? t("nav.workspaces")
				: `${this.selected.label}${this.selected.isMain ? " · main" : ""} · ${this.selected.path}`;
	...
	private renderActivity(workspace: Workspace): TemplateResult | undefined {
		const kind = workspaceActivityIndicator(
			workspaceActivityFor(workspace, this.activities),
		);
		return renderActionActivityIndicator(
			kind,
			kind === "terminal"
				? t("workspace.terminalActive")
				: t("workspace.activeLabel"),
		);
	}

	private renderWorkspaceMain(
		label: string,
		items: WorkspaceLabelItem[],
		workspace: Workspace,
	): TemplateResult {
		return html`
      <span class="workspace-primary">
        <span class="workspace-primary-label">${label}</span>
        ${this.isDeleting(workspace) ? html`<span class="workspace-status">${t("workspace.deleting")}</span>` : null}
      </span>
      ${
				items.length === 0
					? null
	...
	private renderWorkspaceMenu(
      ...
      <div class="action-menu">
        <button
          ...
          title=${t("workspace.actionsDetailsTitle")}
          aria-label=${t("workspace.actionsDetailsFor", { name: label })}
	...
	private renderWorkspaceActions(
      ...
      <div class="workspace-menu-actions">
        <button class="danger" title=${deleting ? t("workspace.deleting") : t("workspace.deleteBtn")} ?disabled=${deleting} @click=${() => {
	...
	private renderWorkspaceDetails(
		label: string,
		items: WorkspaceLabelItem[],
		workspace: Workspace,
	): TemplateResult {
      ...
      <dl class="workspace-menu-details">
        <div class="workspace-detail-row">
          <dt>${workspace.branch === undefined ? t("workspace.workspaceLabel") : t("workspace.branchLabel")}</dt>
          <dd>${label}</dd>
        </div>
        <div class="workspace-detail-row">
          <dt>${t("workspace.pathLabel")}</dt>
        ...
        ${
					items.length === 0
						...
						: html`
          <div class="workspace-detail-row">
            <dt>${t("workspace.detailsLabel")}</dt>
	...
	private handleWorkspaceKeydown(
		event: KeyboardEvent,
		workspace: Workspace,
	): void {
