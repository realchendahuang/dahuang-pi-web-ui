...
describe("ChatView image event wiring", () => {
	...
	it("re-pins late image loads only while already pinned to the bottom", () => {
		...
		if (
			...
			throw new Error("Could not observe ChatView.scrollToBottom");
		const rendered = renderPart(view, {
			type: "image",
			mimeType: "image/png",
			data: "QUJD",
		});
		...
		if (!Reflect.set(view, "pinnedToBottom", true))
			throw new Error("Could not set ChatView.pinnedToBottom");
		onLoad(new Event("load"));
		if (!Reflect.set(view, "pinnedToBottom", false))
			throw new Error("Could not set ChatView.pinnedToBottom");
		onLoad(new Event("load"));
	...
	it("opens and closes the image zoom target on click and close", () => {
		...
		if (typeof close !== "function")
			throw new Error("ChatView.closeImageZoom is not callable");
		close.call(view);
		expect(zoomedImage(view)).toBeUndefined();
	});
});
...
function renderPart(
	...
	if (!isRenderPart(method))
		throw new Error("ChatView.renderPart is not callable");
