...
describe("workspace-git-panel view toggle", () => {
	it("switches between list and tree rendering and persists the choice", () => {
		...
		panel.context = workspacePanelContext({
			gitStatus: gitStatus({ files: [gitFile("src/main.ts")] }),
			onSelectDiff,
		});
		...
		expect(
			findOptionalTemplateClickHandlerForText(rendered, "src/main.ts"),
		).toBeUndefined();
	...
	it("restores the persisted view on construction", () => {
		...
		expect(
			findOptionalTemplateClickHandlerForText(panel.render(), "main.ts"),
		).toBeDefined();
	});
});
...
describe("workspace-git-panel toolbar order", () => {
	it("renders expand/collapse-all before the view toggle, which stays anchored next to Refresh", () => {
		...
		listPanel.context = workspacePanelContext({
			gitStatus: gitStatus({ files: [gitFile("one.ts")] }),
		});
		const withoutExpandAll = toolbarSlots(listPanel.render());
		expect(withoutExpandAll.expandCollapseSlot).toBeNull();

		const treePanel = newGitPanel({
			[GIT_FILE_VIEW_STORAGE_KEY]: "tree",
		}).panel;
		...
		if (!isTemplateResult(expandCollapse))
			throw new Error("expand/collapse-all control missing in tree view");
...
describe("workspace-git-panel expand/collapse all", () => {
	it("expands and collapses every directory, flipping its label", () => {
		...
		expect(
			findOptionalTemplateClickHandlerForText(panel.render(), "b.ts"),
		).toBeUndefined();
		...
		let rendered = panel.render();
		expect(
			findOptionalTemplateClickHandlerForText(rendered, "Expand all"),
		).toBeUndefined();
		...
		expect(
			findOptionalTemplateClickHandlerForText(rendered, "b.ts"),
		).toBeUndefined();
	});
	...
	it("expands and collapses submodule groups in list view", () => {
		...
		rendered = panel.render();
		expect(
			findOptionalTemplateClickHandlerForText(rendered, "abc1234 → def5678"),
		).toBeUndefined();
		expect(
			findOptionalTemplateClickHandlerForText(rendered, "lib.ts"),
		).toBeUndefined();
	});
});
...
describe("workspace-git-panel rows", () => {
	it("toggles a directory's children when its row is clicked", () => {
		...
		templateClickHandlerForText(panel.render(), "src")(new Event("click"));
		expect(
			findOptionalTemplateClickHandlerForText(panel.render(), "a.ts"),
		).toBeDefined();
	...
	it("renders submodule groups with a basename header and badge, and pointer rows when expanded", () => {
		...
		expect(
			findOptionalTemplateClickHandlerForText(collapsed, "abc1234 → def5678"),
		).toBeUndefined();
...
describe("workspace-git-panel context reset", () => {
	it("resets expansion state when the workspace context key changes", () => {
		...
		templateClickHandlerForText(rendered, "Expand all");
		expect(
			findOptionalTemplateClickHandlerForText(rendered, "a.ts"),
		).toBeUndefined();
	});
...
function newGitPanel(storageValues: Record<string, string> = {}): {
	panel: WorkspaceGitPanel;
	storage: FakeStorage;
} {
 ...
 */
function toolbarSlots(rendered: TemplateResult): {
	expandCollapseSlot: unknown;
	toggleIndex: number;
} {
	const strings = templateStrings(rendered);
	const values = templateValues(rendered);
	const actionsIndex = strings.findIndex((chunk) =>
		chunk.includes("toolbar-actions"),
	);
	if (actionsIndex < 0)
		throw new Error("toolbar-actions markup missing from the rendered panel");
	const toggleIndex = actionsIndex + 1;
	const toggle = values[toggleIndex];
	if (
		!isTemplateResult(toggle) ||
		!templateStrings(toggle).some((chunk) => chunk.includes("view-toggle"))
	) {
		throw new Error(
			"view toggle is not the control immediately after the expand/collapse-all slot",
		);
	}
	const refreshOpenChunk = strings[toggleIndex + 1] ?? "";
	const refreshCloseChunk = strings[toggleIndex + 3] ?? "";
	if (
		!refreshOpenChunk.includes("<button") ||
		!refreshCloseChunk.includes("</button>")
	) {
		throw new Error(
			"Refresh button does not immediately follow the view toggle",
		);
	}
...
function rowTextFor(rendered: TemplateResult, anchor: string): string {
	...
	if (text === undefined)
		throw new Error(`Expected a rendered row containing ${anchor}`);
	return text;
}

function findDeepestTemplateText(
	value: unknown,
	anchor: string,
): string | undefined {
...
function submoduleStatus(): GitStatusResponse {
	return gitStatus({
		...
		files: [
			gitFile("vendor/harl", {
				submoduleFromCommit: "abc1234",
				submoduleToCommit: "def5678",
			}),
...
function workspacePanelContext(
	patch: Partial<WorkspacePanelContext> = {},
	...
	return {
		...
		files: patch.files ?? {
			readFile: vi.fn<WorkspacePanelContext["files"]["readFile"]>(() =>
				Promise.reject(new Error("not implemented")),
			),
			writeFile: vi.fn<WorkspacePanelContext["files"]["writeFile"]>(() =>
				Promise.reject(new Error("not implemented")),
			),
			deleteFile: vi.fn<WorkspacePanelContext["files"]["deleteFile"]>(() =>
				Promise.reject(new Error("not implemented")),
			),
		...
		prompt: patch.prompt ?? {
			insertText: vi.fn<WorkspacePanelContext["prompt"]["insertText"]>(),
			getText: vi.fn<WorkspacePanelContext["prompt"]["getText"]>(() => ""),
			getSelection: vi.fn<WorkspacePanelContext["prompt"]["getSelection"]>(
				() => null,
			),
		},
		terminal: patch.terminal ?? {
			open: vi.fn<WorkspacePanelContext["terminal"]["open"]>(),
			runCommand: vi.fn<WorkspacePanelContext["terminal"]["runCommand"]>(() =>
				Promise.reject(new Error("not implemented")),
			),
		},
		host: patch.host ?? {
		...
		workspaceUploadDefaultFolder:
			patch.workspaceUploadDefaultFolder ?? ".pi-web/uploads",
		onRefreshFiles:
			patch.onRefreshFiles ?? vi.fn<WorkspacePanelContext["onRefreshFiles"]>(),
		onExpandDir:
			patch.onExpandDir ?? vi.fn<WorkspacePanelContext["onExpandDir"]>(),
		onSelectFile:
			patch.onSelectFile ?? vi.fn<WorkspacePanelContext["onSelectFile"]>(),
		onStartWorkspaceUpload:
			patch.onStartWorkspaceUpload ??
			vi.fn<WorkspacePanelContext["onStartWorkspaceUpload"]>(() => undefined),
		onCancelWorkspaceUpload:
			patch.onCancelWorkspaceUpload ??
			vi.fn<WorkspacePanelContext["onCancelWorkspaceUpload"]>(),
		onClearWorkspaceUpload:
			patch.onClearWorkspaceUpload ??
			vi.fn<WorkspacePanelContext["onClearWorkspaceUpload"]>(),
		onRefreshGit:
			patch.onRefreshGit ?? vi.fn<WorkspacePanelContext["onRefreshGit"]>(),
		onSelectDiff:
			patch.onSelectDiff ?? vi.fn<WorkspacePanelContext["onSelectDiff"]>(),
		onSelectTerminal:
			patch.onSelectTerminal ??
			vi.fn<WorkspacePanelContext["onSelectTerminal"]>(),
	};
}
