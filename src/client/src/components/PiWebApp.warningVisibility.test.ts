...
describe("PiWebApp session-warning visibility wiring", () => {
	it("keeps the warning control present and toggles the warning area from the status bar", () => {
		...
		expect(
			templateValueAfterMarker(renderChatView(app, state), ".warningsVisible="),
		).toBe(true);
		expect(templateValueAfterMarker(visibleStatusBar, ".warningCount=")).toBe(
			2,
		);
		expect(
			templateValueAfterMarker(visibleStatusBar, ".warningsExpanded="),
		).toBe(true);

		const toggle = templateCallbackAfterMarker(
			visibleStatusBar,
			".onToggleWarnings=",
		);
		toggle();
		...
		expect(templateValueAfterMarker(collapsedStatusBar, ".warningCount=")).toBe(
			2,
		);
		expect(
			templateValueAfterMarker(collapsedStatusBar, ".warningsExpanded="),
		).toBe(false);
		...
		expect(
			templateValueAfterMarker(
				renderChatView(app, returningState),
				".warningsVisible=",
			),
		).toBe(true);
		expect(
			templateValueAfterMarker(
				renderStatusBar(app, returningState),
				".warningCount=",
			),
		).toBe(0);
		...
		expect(templateValueAfterMarker(returnedStatusBar, ".warningCount=")).toBe(
			2,
		);
		expect(
			templateValueAfterMarker(returnedStatusBar, ".warningsExpanded="),
		).toBe(false);
		...
		expect(templateValueAfterMarker(restoredStatusBar, ".warningCount=")).toBe(
			2,
		);
		expect(
			templateValueAfterMarker(restoredStatusBar, ".warningsExpanded="),
		).toBe(true);
	});
});
...
function syncWarningVisibility(app: PiWebApp): void {
	...
	if (!isSyncWarningVisibility(method))
		throw new Error("PiWebApp.syncSessionWarningVisibility is not callable");
	method.call(app);
}

function renderChatView(app: PiWebApp, state: AppState): TemplateResult {
	const method: unknown = Reflect.get(app, "renderChatView");
	if (!isRenderChatView(method))
		throw new Error("PiWebApp.renderChatView is not callable");
...
function renderStatusBar(app: PiWebApp, state: AppState): TemplateResult {
	...
	if (!isRenderStatusBar(method))
		throw new Error("PiWebApp.renderStatusBar is not callable");
	return method.call(app, state);
}

function templateCallbackAfterMarker(
	template: TemplateResult,
	marker: string,
): WarningVisibilityCallback {
	const value = templateValueAfterMarker(template, marker);
	if (!isWarningVisibilityCallback(value))
		throw new Error(`Expected callback after ${marker}`);
	return value;
}
...
function isSyncWarningVisibility(
	value: unknown,
): value is SyncWarningVisibility {
	return typeof value === "function";
}

function isWarningVisibilityCallback(
	value: unknown,
): value is WarningVisibilityCallback {
	return typeof value === "function";
}
