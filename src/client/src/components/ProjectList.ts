	...
	implements KeyboardNavigableSection
{
	...
	protected override updated(changed: PropertyValues<this>): void {
		if (
			changed.has("projects") &&
			this.openMenuProjectId !== undefined &&
			!this.projects.some((project) => project.id === this.openMenuProjectId)
		)
			this.openMenuProjectId = undefined;
            ...
            ${this.projects.map(
              ...
              <div
                ...
                @keydown=${(event: KeyboardEvent) => {
									this.handleProjectKeydown(event, project);
								}}
              >
                ...
                <div class="action-menu">
                  <button class="action-menu-toggle" title=${t("project.actionsMenu")} aria-label=${`Actions for ${project.name}`} @click=${(
										event: MouseEvent,
									) => {
										event.stopPropagation();
										this.toggleMenu(project.id, event.currentTarget);
									}}>⋯</button>
                  ${
										this.openMenuProjectId === project.id
											? html`
                    <div class="action-menu-panel" style=${this.menuStyle}>
                      <button title=${t("project.closeBtn")} @click=${() => {
												this.close(project);
											}}>${t("project.closeBtn")}</button>
                    </div>
                  `
											: null
									}
	...
	private renderActivity(project: Project) {
		const kind = projectActivityIndicator(
			project,
			this.workspacesByProjectId[project.id] ?? [],
			this.activities,
		);
		return renderActionActivityIndicator(
			kind,
			kind === "terminal" ? t("project.terminalActive") : t("project.activeLabel"),
		);
	}
	...
	private close(project: Project) {
		this.openMenuProjectId = undefined;
		if (confirm(t("project.closeConfirm", { name: project.name })))
			this.onClose?.(project);
	}
