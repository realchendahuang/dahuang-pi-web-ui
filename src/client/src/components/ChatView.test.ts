...
describe("ChatView notification tray wiring", () => {
	...
	it("wires individual dismissal and recovers header focus after the final row", () => {
		...
		view.notificationInbox = emptyNotificationInbox(
			requireNotificationInbox(view),
		);
	...
	it("keeps a collapsed tray closed for new arrivals and isolates matching session ids by exact chat", () => {
		...
		const collapsedTargetKeys: unknown = Reflect.get(
			view,
			"collapsedNotificationTargetKeys",
		);
		if (!(collapsedTargetKeys instanceof Set))
			throw new Error("Expected collapsed notification target keys");
		const firstNotification = inbox.notifications[0];
		if (firstNotification === undefined)
			throw new Error("expected a retained notification");
		const newArrival = {
			...inbox,
			notifications: [
				{ ...firstNotification, id: "daemon-a:2", order: 2 },
				...inbox.notifications,
			],
			retainedCount: 2,
		};
		expect(notificationTrayIsCollapsed(collapsedTargetKeys, newArrival)).toBe(
			true,
		);
		expect(
			notificationTrayIsCollapsed(collapsedTargetKeys, {
				...newArrival,
				cwd: "/other",
			}),
		).toBe(false);
...
function renderNotificationTray(view: ChatView): TemplateResult | null {
	...
	if (!isRenderNotificationTray(method))
		throw new Error("ChatView.renderNotificationTray is not callable");
	return method.call(view);
}
...
function focusPendingNotificationTarget(view: ChatView): void {
	...
	if (!isFocusPendingNotificationTarget(method))
		throw new Error("ChatView.focusPendingNotificationTarget is not callable");
	method.call(view);
}
...
function isRenderNotificationTray(
	value: unknown,
): value is RenderNotificationTray {
	return typeof value === "function";
}

function isFocusPendingNotificationTarget(
	value: unknown,
): value is FocusPendingNotificationTarget {
	return typeof value === "function";
}
...
function installNotificationFocusRoot(
	view: ChatView,
): ReturnType<typeof vi.fn> {
	const headerFocus = vi.fn();
	const renderRoot = {
		querySelector: (selector: string) =>
			selector === "[data-notification-focus='header']"
				? { focus: headerFocus }
				: null,
		querySelectorAll: () => [],
	};
	if (!Reflect.set(view, "renderRoot", renderRoot))
		throw new Error("Could not install notification focus root");
	return headerFocus;
}
