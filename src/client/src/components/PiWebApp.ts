...
import {
	...
	type SessionCleanupPreviewResponse,
	type SessionCleanupRequest,
	type SessionInfo,
	type SessionTreeNavigateResult,
...
import {
	emptyMachineNavigationSnapshot,
	machineNavigationSnapshotFromState,
	routeFromMachineNavigationSnapshot,
	SessionStorageMachineNavigationMemory,
	type MachineNavigationSnapshot,
	type WorkspaceRouteSurface,
} from "../controllers/machineNavigationMemory";
...
import {
	...
	sessionCleanupUnavailableMessage,
...
import {
	queryNamespace,
	readNamespacedString,
	setNamespacedQueryKey,
...
import {
	...
	mainViewClass,
} from "../appShell/panelCollapseController";
import {
	PanelResizeController,
	type PanelResizeConstraints,
	type ResizablePanelSide,
} from "../appShell/panelResizeController";
...
import type {
	AppMobileMainTab,
	AppMobileMainTabIcon,
} from "./appShell/AppMobileMainTabs";
import {
	shouldShowMachinesSection,
	type AppNavigationPanel,
...
/** How long a manual workspace-tab selection suppresses activity-driven auto switching. */
const WORKSPACE_TOOL_PIN_MS = 5 * 60 * 1000;
const REMOTE_ROUTE_RESTORE_RETRY_DELAYS_MS = [
	1_000, 3_000, 8_000, 15_000, 30_000,
] as const;
...
export class PiWebApp extends LitElement {
	...
	private readonly sessionUnread = new SessionUnreadController({
		...
		onBackgroundError: (operation, machineId, error) => {
			console.warn(
				`Failed to ${operation} session unread state for ${machineId}`,
				error,
			);
		},
	});
	@state() private unreadSessionIds: ReadonlySet<string> =
		this.sessionUnread.unreadSessionIds(
	...
	private readonly projectActivityOwnership =
		new ProjectActivityOwnershipCoordinator(
			() => this.state,
			(patch) => {
				this.setState(patch);
			},
			{
				...
				onError: ({ machineId, projectId, error }) => {
					console.warn(
						`Failed to discover project activity ownership for ${projectId} on ${machineId}`,
						error,
					);
				},
			},
	...
	private readonly activity = new ActivityController(
		...
		{
			onActivityApplied: (machineId) => {
				void this.projectActivityOwnership.handleActivityApplied(machineId);
			},
		},
	);
	...
	private readonly piWebStatusController = new PiWebStatusController(
		...
		{
			onRefreshError: (machineId, error) => {
	...
	private readonly machineNavigation =
		new SessionStorageMachineNavigationMemory();
	...
	private themePreference: ThemePreference =
		readStoredThemePreference() ?? DEFAULT_THEME_PREFERENCE;
	@state() private activeThemeId: QualifiedContributionId = CLASSIC_THEME_ID;
	@state() private isRefreshingApp = false;
	@state() private sessionCleanupDialog: SessionCleanupDialogState | undefined;
	@state() private settingsSection: SettingsSection | undefined =
		readSettingsSection();
	@state() private shortcutConfig: PiWebShortcutConfig = {};
	@state() private workspaceUploadDefaultFolder =
		effectiveWorkspaceUploadFolder(undefined);
	...
	private readonly onKeyDown = (event: KeyboardEvent) => {
		if (
			...
			return;
		if (
			this.keyboard.handle(event, this.getDefaultActions(), {
				shortcuts: this.shortcutConfig,
			})
		) {
	...
	protected override willUpdate(): void {
		...
		if (previous !== undefined) this.maybeAutoOpenChanges(previous, this.state);
		this.previousAppState = this.state;
	}

	/**
	 * Right-panel activity routing (spec §5): when the agent introduces working-
	 * tree changes while the user has not recently pinned a workspace tab,
	 * switch the panel to Changes. Never switches on tablet/mobile layouts and
	 * never overrides a recent manual selection.
	 */
	private maybeAutoOpenChanges(previous: AppState, next: AppState): void {
		if (!this.isDesktopSideBySideLayout()) return;
		if (Date.now() - this.workspaceToolPinnedAt < WORKSPACE_TOOL_PIN_MS) return;
		if (next.workspaceTool === "core:workspace.changes") return;
		if (previous.selectedWorkspace?.id !== next.selectedWorkspace?.id) return;
		const prevCount = previous.gitStatus?.files.length ?? 0;
		const nextCount = next.gitStatus?.files.length ?? 0;
		if (prevCount === 0 && nextCount > 0) {
			this.openWorkspaceTool("core:workspace.changes");
		}
	}

	private previousAppState: AppState | undefined;
	private workspaceToolPinnedAt = 0;
	...
	private syncSessionWarningVisibility(): void {
		...
		this.sessionWarningVisibility = reconcileSessionWarningVisibility(
			...
			session === undefined
				? undefined
				: machineSessionKey(selectedMachineId(this.state), session.id),
	...
	private async commitReadyChatAfterRender(
		machineId: string,
		session: SessionInfo,
	): Promise<void> {
		const identity = unreadChatIdentity(machineId, session);
		await this.updateComplete;
		if (!this.unreadConnected || selectedChatIdentity(this.state) !== identity)
			return;
	...
	private syncUnreadSessionIds(): void {
		const next = this.sessionUnread.unreadSessionIds(
			selectedMachineId(this.state),
			this.state.sessions,
		);
		if (!sameStringSet(next, this.unreadSessionIds))
	...
	private isSessionSeen(machineId: string, session: SessionInfo): boolean {
		...
		const identity = unreadChatIdentity(machineId, session);
		if (
			selectedChatIdentity(this.state) !== identity ||
	...
	override connectedCallback(): void {
		...
		window.addEventListener(
			"keydown",
			this.onKeyDown,
			GLOBAL_SHORTCUT_LISTENER_OPTIONS,
		);
		this.systemLightThemeMedia?.addEventListener(
			"change",
			this.onSystemLightThemeChange,
		);
	...
	override disconnectedCallback(): void {
		...
		this.systemLightThemeMedia?.removeEventListener(
			"change",
			this.onSystemLightThemeChange,
		);
		this.keyboard.reset();
		...
		this.git.dispose();
		if (this.piWebStatusTimer !== undefined)
			window.clearInterval(this.piWebStatusTimer);
		this.piWebStatusTimer = undefined;
		this.clearScheduledPiWebStatusRefresh();
		if (this.workspaceDeletionPollTimer !== undefined)
	...
	private setState(patch: Partial<AppState>) {
		...
		if (machineUnreadInputsChanged(previous, this.state))
		...
		if (machineActivitySubscriptionInputsChanged(previous, this.state))
			this.syncMachineActivitySubscriptions();
	...
	private async loadProjectsAndRestoreRoute() {
		...
		const initialRouteMachineHealth =
			this.state.machineStatuses[effectiveRoute.machineId ?? "local"];
		if (effectiveRoute !== route)
			this.replaceRouteAndClearWorkspaceQuery(effectiveRoute);
		await this.projects.loadProjects();
		await this.withChatScrollTransition(() =>
			this.restoreRouteFor(effectiveRoute, false),
		);
		if (
			this.shouldDeferRemoteRouteRestore(
				effectiveRoute,
				initialRouteMachineHealth,
			)
		)
	...
	private async refreshWorkspaceActivity(
		...
		try {
			...
			console.warn(
				`Failed to refresh workspace activity for ${machineId}`,
				error,
			);
		}
	}
				...
				: this.state.machines
						.filter((machine) =>
							shouldRefreshMachineActivity(
								machine,
								this.state.machineStatuses[machine.id],
							),
						)
						.map((machine) => machine.id);
		await Promise.all(
			machineIds.map((machineId) => this.refreshWorkspaceActivity(machineId)),
		);
	}
	...
	private async refreshCurrentWorkspaceSurface(): Promise<void> {
		...
		const tool =
			this.state.mainView !== "chat" && this.state.mainView !== "navigation"
				? this.state.mainView
				: this.state.workspaceTool;
		if (tool === "core:workspace.files") await this.files.refreshFiles();
		else if (tool === "core:workspace.git") await this.git.refreshGit();
		else if (tool === "core:workspace.terminal" && workspace !== undefined)
			await this.refreshActiveTerminals(workspace);
	}
	...
	private async restoreRouteFor(
		...
		try {
			...
			if (route.tool?.startsWith("machine.") === true)
				await selectedMachinePluginLoad;
			...
			if (this.routeMatchesCurrentSelection(route)) {
				if (routeSurface.selectedTerminalId !== undefined)
					this.rememberSelectedTerminal(routeSurface.selectedTerminalId);
				await this.refreshRestoredWorkspaceTool(
					route.tool,
					routeSurface.selectedFilePath,
				);
			...
			if (this.routeRestoreDepth === 0)
				this.restoringRouteTerminalId = undefined;
			if (selectedMachineId(this.state) !== machineBeforeRestore)
				this.schedulePiWebStatusRefresh();
		}
	}
	...
	private readWorkspaceRouteSurface(route: AppRoute): WorkspaceRouteSurface {
		if (route.projectId === undefined || route.projectId === "")
			return emptyWorkspaceRouteSurface();
		return {
			...
			selectedTerminalId: readNamespacedString(
				TERMINAL_ROUTE_NAMESPACE,
				"terminal",
			),
		};
	}
	...
	private replaceRouteAndClearWorkspaceQuery(route: AppRoute): void {
		...
		setNamespacedQueryKey(FILES_ROUTE_NAMESPACE, "file", undefined, {
			replace: true,
		});
		setNamespacedQueryKey(GIT_ROUTE_NAMESPACE, "diff", undefined, {
			replace: true,
		});
		setNamespacedQueryKey(TERMINAL_ROUTE_NAMESPACE, "terminal", undefined, {
			replace: true,
		});
	}

	private shouldDeferRemoteRouteRestore(
		route: AppRoute,
		routeMachineHealth = this.state.machineStatuses[route.machineId ?? "local"],
	): boolean {
	...
	private schedulePendingRemoteRouteRestore(
		delayMs = remoteRouteRestoreRetryDelay(this.remoteRouteRestoreAttempt),
	): void {
		if (this.pendingRemoteRouteRestore === undefined) return;
	...
	private async retryPendingRemoteRouteRestore(): Promise<void> {
		...
		try {
			...
			await this.withChatScrollTransition(() =>
				this.restoreRouteFor(route, false),
			);
			if (!this.pendingRemoteRouteRestoreStillCurrent(route)) return;
			this.clearPendingRemoteRouteRestore();
			this.rememberCurrentMachineNavigation();
	...
	private scheduleNextRemoteRouteRestoreAttempt(route: AppRoute): void {
		...
		if (
			this.remoteRouteRestoreAttempt >=
			REMOTE_ROUTE_RESTORE_RETRY_DELAYS_MS.length
		) {
	...
	private setRemoteRouteRestoreMessage(
		route: AppRoute,
		options: { exhausted?: boolean } = {},
	): void {
		const machineId = route.machineId ?? "local";
		const machineName =
			this.state.machines.find((machine) => machine.id === machineId)?.name ??
			this.state.selectedMachine?.name ??
			"Remote machine";
		const health = this.state.machineStatuses[machineId];
		const detail =
			health?.error ?? (this.state.error === "" ? undefined : this.state.error);
		const prefix =
			options.exhausted === true
				? `${machineName} is still unavailable.`
				: `${machineName} is unavailable; reconnecting…`;
		this.setState({
			error: `${prefix}${detail === undefined ? "" : ` ${detail}`}`,
		});
	}
	...
	private async restoreRouteMachine(
		route: AppRoute,
		updateUrl: boolean,
	): Promise<void> {
		...
		const machine = this.state.machines.find(
			(candidate) => candidate.id === routeMachineId,
		);
	...
	private routeMatchesCurrentSelection(route: AppRoute): boolean {
		return (
			(route.machineId ?? "local") ===
				(this.state.selectedMachine?.id ?? "local") &&
	...
	private async refreshRestoredWorkspaceTool(
		...
		if (tool === "core:workspace.files") await this.files.refreshFiles();
		if (tool === "core:workspace.files" && selectedFilePath !== undefined)
			await this.files.restoreFile(selectedFilePath);
		if (tool === "core:workspace.git") await this.git.refreshGit();
	}

	private async withChatScrollTransition(
		action: () => Promise<void>,
	...
	private rememberCurrentMachineNavigation(): void {
		this.machineNavigation.remember(
			machineNavigationSnapshotFromState(this.state),
		);
	}

	private syncWorkspaceRouteSurfaceToUrl(): void {
		this.writeWorkspaceRouteSurfaceToUrl(
			machineNavigationSnapshotFromState(this.state).surface,
		);
	}

	private writeMachineNavigationSnapshotToUrl(
	...
	private writeWorkspaceRouteSurfaceToUrl(
		surface: WorkspaceRouteSurface,
	): void {
		setNamespacedQueryKey(
			FILES_ROUTE_NAMESPACE,
			"file",
			surface.selectedFilePath,
			{ replace: true },
		);
		setNamespacedQueryKey(
			GIT_ROUTE_NAMESPACE,
			"diff",
			surface.selectedDiffPath,
			{ replace: true },
		);
		setNamespacedQueryKey(
			TERMINAL_ROUTE_NAMESPACE,
			"terminal",
			surface.selectedTerminalId,
			{ replace: true },
		);
	}
	...
	private async selectMachineWithMemory(
		...
		if (options.rememberCurrent !== false && !this.routeRestoreInProgress)
			this.rememberCurrentMachineNavigation();
		const seq = ++this.machineNavigationRestoreSeq;
		const snapshot =
			this.machineNavigation.latest(machine.id) ??
			emptyMachineNavigationSnapshot(machine.id);
		...
		if (
			seq !== this.machineNavigationRestoreSeq ||
			this.state.selectedMachine?.id !== machine.id
		)
			return;
	...
	private shouldPreserveUnrestoredMachineNavigation(
		snapshot: MachineNavigationSnapshot,
	): boolean {
		return (
	...
	private openWorkspaceTool(tool: QualifiedContributionId) {
		if (tool === "core:workspace.terminal")
			this.terminalAutoStartWorkspaceId = this.state.selectedWorkspace?.id;
	...
	private openTerminal(options?: { terminalId?: string | undefined }): void {
		if (options?.terminalId !== undefined)
			this.selectTerminal(options.terminalId, { replace: true });
		this.openWorkspaceTool("core:workspace.terminal");
	}

	private terminalCommandRunsForOrigin(
		origin: string,
		machineId = selectedMachineId(this.state),
	): TerminalCommandRunsInternalRuntime {
		...
		const runtime = createTerminalCommandRunsRuntime(origin, {
			api: {
				runTerminalCommand: (runtimeOrigin, input) =>
	...
	private async openRuntimeTerminal(
		...
		if (
			...
			await this.restoreRouteFor(
				...
				false,
				{ selectedTerminalId: options?.terminalId },
	...
	private rememberSelectedTerminal(terminalId: string | undefined): void {
		...
		if (terminalId === undefined)
			this.terminalSelection.forgetWorkspace(
				this.terminalWorkspaceKey(workspace),
			);
		else
			this.terminalSelection.rememberTerminal(
				this.terminalWorkspaceKey(workspace),
				terminalId,
			);
	}
	...
	private handleWorkspaceChange(previous: AppState, next: AppState) {
		...
		const selectedTerminalId = this.routeRestoreInProgress
			...
			: next.selectedWorkspace === undefined
				? undefined
				: this.terminalSelection.latestTerminalId(
						this.terminalWorkspaceKey(next.selectedWorkspace),
					);
	...
	private syncSessionUnreadMachines(): void {
		...
		const machineIds = new Set(
			this.state.machines.map((machine) => machine.id),
		);
		...
		for (const machineId of machineIds) {
			...
			const capability =
				this.unreadRuntimeRefreshes.has(machineId) || !runtime.ok
					? "unknown"
					: supportsPiWebCapability(runtime, PI_WEB_CAPABILITIES.sessionsUnread)
						? "supported"
						: "unsupported";
			if (this.sessionUnread.setCapability(machineId, capability))
	...
	private async renegotiateUnreadMachines(): Promise<void> {
		...
		await Promise.all(
			[...machineIds].map((machineId) =>
				this.renegotiateUnreadMachine(machineId),
			),
		);
	}
	...
	private renegotiateUnreadMachine(machineId: string): Promise<void> {
		...
		const refresh = Promise.resolve().then(async () => {
			refreshed =
				(await this.machines.refreshMachineRuntime(machineId)) !== undefined;
		});
	...
	private connectRealtime(): void {
		...
		this.realtime.connect(
			(event) => {
				this.handleRealtimeEvent(machineId, event);
			},
			() => {
				void this.renegotiateUnreadMachine(machineId);
				const workspace = this.state.selectedWorkspace;
				if (workspace !== undefined)
					void this.refreshActiveTerminals(workspace);
	...
	private syncMachineActivitySubscriptions(): void {
		...
		for (const machineId of desiredMachineIds) {
			...
			socket.connect(
				(event) => {
					this.handleMachineActivityEvent(machineId, event);
				},
				() => {
					void this.renegotiateUnreadMachine(machineId);
					void this.refreshWorkspaceActivity(machineId);
	...
	private handleMachineActivityEvent(
		machineId: string,
		event: BrowserRealtimeEvent,
	): void {
		if (event.type === "sessions.unread")
			this.sessionUnread.applyEvent(machineId, event);
		else if (event.type === "workspace.activity")
			this.activity.applyWorkspaceActivity(event.activity, machineId);
	}
	...
	private handleRealtimeEvent(
		...
		if (event.type === "sessions.unread")
			...
			this.activity.applyWorkspaceActivity(event.activity);
	...
	private applyTerminalEvent(event: TerminalUiEvent): void {
		...
		if (workspace === undefined) return;
		const cwd =
			event.type === "terminal.closed" ? event.cwd : event.terminal.cwd;
		if (cwd !== workspace.path) return;
		if (event.type === "terminal.created" && !event.terminal.exited)
			this.activeTerminalIds.add(event.terminal.id);
		else
			this.activeTerminalIds.delete(
				event.type === "terminal.closed" ? event.terminalId : event.terminal.id,
			);
		if (event.type === "terminal.closed") {
			this.terminalSelection.forgetTerminal(event.terminalId);
			if (this.state.selectedTerminalId === event.terminalId)
				this.selectTerminal(undefined, { replace: true });
		}
	...
	private async refreshActiveTerminals(workspace: Workspace): Promise<void> {
		const machineId = selectedMachineId(this.state);
		try {
			const terminals = await terminalsApi.terminals(
				workspace.projectId,
				workspace.id,
				machineId,
			);
	...
	private handleMachineChange(previous: AppState, next: AppState): void {
		if (
			(previous.selectedMachine?.id ?? "local") ===
			(next.selectedMachine?.id ?? "local")
		)
			return;
		this.projectActivityOwnership.handleSelectedMachineChanged();
		const pendingMachineId =
			this.pendingRemoteRouteRestore?.machineId ?? "local";
		if (pendingMachineId !== (next.selectedMachine?.id ?? "local"))
			this.clearPendingRemoteRouteRestore();
	...
	private renderWorkspacePanel() {
		...
		const panelContext =
			workspace === undefined
				? undefined
				: this.createWorkspacePanelContext(workspace);
		const emptyState =
			workspace === undefined ? this.workspacePanelEmptyState() : undefined;
		return html`
      <workspace-panel
        id="workspace-panel"
        .workspace=${workspace}
        ...
        .onSelectTool=${(tool: QualifiedContributionId) => {
					this.workspaceToolPinnedAt = Date.now();
	...
	private renderNavigationPanelEdgeControl() {
      ...
      <app-panel-edge-control
        ...
        .onResize=${(width: number) => {
					this.panelResize.resizePanel("navigation", width, { persist: false });
				}}
        .onResizeEnd=${() => {
					this.panelResize.persistPanelSizes();
				}}
        .onReset=${() => {
					this.resetResizablePanel("navigation");
				}}
      ></app-panel-edge-control>
    `;
	}
	...
	private renderWorkspacePanelEdgeControl() {
      ...
      <app-panel-edge-control
        ...
        .onResize=${(width: number) => {
					this.panelResize.resizePanel("workspace", width, { persist: false });
				}}
        .onResizeEnd=${() => {
					this.panelResize.persistPanelSizes();
				}}
        .onReset=${() => {
					this.resetResizablePanel("workspace");
				}}
      ></app-panel-edge-control>
    `;
	}
	...
	private resizablePanelConstraints(
	...
	private resizablePanelMaxWidth(
		side: ResizablePanelSide,
		...
		const maxWidth = Math.floor(
			shellWidth -
				otherPanelWidth -
				PANEL_EDGE_COLUMNS_WIDTH_PX -
				MIN_RESIZABLE_CHAT_WIDTH_PX,
		);
		return Math.max(
	...
	private oppositeResizablePanelWidth(side: ResizablePanelSide): number {
		const otherSide: ResizablePanelSide =
			side === "navigation" ? "workspace" : "navigation";
		if (this.isResizablePanelCollapsedOrStacked(otherSide)) return 0;
		return (
			this.measuredPanelWidth(otherSide) ??
			this.panelResize.panelWidth(otherSide)
		);
	}

	private isResizablePanelCollapsedOrStacked(
		side: ResizablePanelSide,
	): boolean {
		if (side === "navigation")
			return this.panelCollapse.navigationPanelCollapsed;
		return (
	...
	private measuredPanelWidth(side: ResizablePanelSide): number | undefined {
		const element =
			side === "navigation"
				? this.navigationPanelFrame
				: this.workspacePanelFrame;
	...
	private canReloadSessions(): boolean {
		...
		return (
			runtime?.ok === true &&
			supportsPiWebCapability(runtime, PI_WEB_CAPABILITIES.sessionsReload)
		);
	}
	...
	private canCleanupSessions(): boolean {
		...
		return (
			runtime?.ok === true &&
			supportsPiWebCapability(runtime, PI_WEB_CAPABILITIES.sessionsCleanup)
		);
	}

	private hasAuthoritativeSessionPersistence(): boolean {
		return runtimeHasAuthoritativeSessionPersistence(
			this.selectedMachineRuntime(),
		);
	}

	private supportsWorkspaceFileSuggestions(
		machineId = selectedMachineId(this.state),
	): boolean {
	...
	private async previewSessionCleanup(
		request: SessionCleanupRequest,
	): Promise<void> {
		if (!this.canCleanupSessions()) {
			this.sessionCleanupDialog = {
				...(this.sessionCleanupDialog ?? {}),
				error: this.sessionCleanupUnavailableMessage(),
				preview: undefined,
		...
		try {
			...
			if (selectedMachineId(this.state) === machineId)
				this.sessionCleanupDialog = {
					...this.sessionCleanupDialog,
					loading: false,
					error: `Failed to preview cleanup: ${errorMessage(error)}`,
				};
		}
	}
	...
	private async runSessionCleanup(
		...
		if (
			dialog?.preview === undefined ||
			sessionCleanupRequestKey(dialog.previewRequest) !==
				sessionCleanupRequestKey(request)
		) {
			this.sessionCleanupDialog = {
				...(dialog ?? {}),
				error: "Preview cleanup before running it.",
			};
			return;
		}
		if (!this.canCleanupSessions()) {
		...
		try {
			...
			if (selectedMachineId(this.state) === machineId)
				this.sessionCleanupDialog = {
					...this.sessionCleanupDialog,
					running: false,
					error: `Failed to run cleanup: ${errorMessage(error)}`,
				};
		}
	}
	...
	private renderNavigationPanel() {
      ...
      <app-navigation-panel
        ...
        .onToggleProjects=${() => {
					this.navigationSections.toggle("projects");
				}}
        .onToggleWorkspaces=${() => {
					this.navigationSections.toggle("workspaces");
				}}
        .onToggleSessions=${() => {
					this.navigationSections.toggle("sessions");
				}}
        ...
        .onCleanupSessions=${() => {
					this.openSessionCleanupDialog();
				}}
        .onFocusNavigationTarget=${(target: NavigationFocusTarget) => {
					void this.focusNavigationTarget(target);
				}}
	...
	private async selectNavigationItem(
		section: NavigationSection,
		nextTarget: NavigationFocusTarget,
		action: () => Promise<void>,
	): Promise<void> {
	...
	private async focusNavigationTarget(
	...
	private async focusNavigationSection(
		...
		if (
			section === "machines" &&
			!shouldShowMachinesSection(this.state.machines)
		) {
	...
	private async navigateSessionTree(
		targetId: string,
		summaryChoice: SessionTreeSummaryChoice,
	...
	private renderSessionTreeNavigator(state: AppState) {
		return state.treeDialog === undefined
			? null
			: html`
      <session-tree-navigator
        ...
        .onCancel=${() => {
					this.closeSessionTreeNavigator();
				}}
      ></session-tree-navigator>
    `;
	}
	...
	private visibleWorkspacePanels(): QualifiedWorkspacePanelContribution[] {
		...
		return this.plugins
			.getWorkspacePanels()
			.filter((panel) => panel.visible?.(context) ?? true);
	}
	...
	private mobilePanelBadge(
		panel: QualifiedWorkspacePanelContribution,
	): unknown {
		const workspace = this.state.selectedWorkspace;
	...
	private mobilePanelIcon(
		panel: QualifiedWorkspacePanelContribution,
	): AppMobileMainTabIcon | undefined {
		switch (panel.id) {
			case "core:workspace.files":
				return "files";
			case "core:workspace.git":
				return "git";
			case "core:workspace.terminal":
				return "terminal";
			default:
				return undefined;
		}
	...
	private workspaceLabelItems(workspace: Workspace): WorkspaceLabelItem[] {
		return this.plugins.getWorkspaceLabelItems(
			this.createWorkspaceLabelContext(workspace),
		);
	}

	private createWorkspaceLabelContext(
		workspace: Workspace,
	): WorkspaceLabelContext {
	...
	private createWorkspaceFiles(
		workspace: Workspace,
		machineId: string,
	): WorkspaceFiles {
		return {
			readFile: (path: string) =>
				workspacesApi.workspaceFile(
					workspace.projectId,
					workspace.id,
					path,
					machineId,
			...
			writeFile: async (path, content, options) => {
				const result = await workspacesApi.writeWorkspaceFile(
					workspace.projectId,
					workspace.id,
					path,
					content,
			...
			deleteFile: async (path) => {
				const result = await workspacesApi.deleteWorkspaceFile(
					workspace.projectId,
					workspace.id,
					path,
					machineId,
			...
			moveFile: async (fromPath, toPath, options) => {
				const result = await workspacesApi.moveWorkspaceFile(
					workspace.projectId,
					workspace.id,
					fromPath,
					toPath,
	...
	private createWorkspacePanelContext(
		...
		const createContext = (origin: string): WorkspacePanelContext => {
			const terminalCommandRuns = this.terminalCommandRunsForOrigin(
				origin,
				machineId,
			);
			return installWorkspacePanelScope(
				{
					...
					terminal: {
						...
						runCommand: (input) =>
							terminalCommandRuns.runCommand({ ...input, workspace }),
					},
					...
					workspaceUploadDefaultFolder: workspaceEffectiveUploadFolder(
						workspace.effectiveConfig,
						this.workspaceUploadDefaultFolder,
					),
					onRefreshFiles: () => {
						void this.files.refreshFiles();
					},
					...
					onStartWorkspaceUpload: (files, options) =>
						this.files.startWorkspaceUpload(files, options),
					onCancelWorkspaceUpload: (batchId) => {
						this.files.cancelWorkspaceUpload(batchId);
					},
					onClearWorkspaceUpload: (batchId) => {
						this.files.clearWorkspaceUpload(batchId);
					},
					onRefreshGit: () => {
	...
	private getActions(): AppAction[] {
		return applyActiveShortcutPreferences(
	...
	private getDefaultActions(): AppAction[] {
		return [
			...this.plugins.getActions(this.createPluginRuntimeContext()),
			...this.sessionActions(),
			...this.navigationFocusActions(),
			...this.panelLayoutActions(),
		];
	}
		...
		return [
			{
				...
				...(canCleanup
					...
					: {
							enabled: false,
							disabledReason: this.sessionCleanupUnavailableMessage(),
						}),
				run: () => {
					this.openSessionCleanupDialog();
				},
	...
	private async loadExternalPlugins(): Promise<void> {
		await this.registerExternalPlugins("PI WEB plugins", () =>
			loadExternalPlugins(),
		);
	}
	...
	private async loadPluginsForMachine(machine: Machine): Promise<void> {
		...
		if (
			machine.kind !== "remote" ||
			this.loadedMachinePluginIds.has(machine.id)
		)
			return;
		const existing = this.machinePluginLoadPromises.get(machine.id);
		if (existing !== undefined) return existing;

		const load = this.registerExternalPlugins(
			`PI WEB plugins from ${machine.name}`,
			() =>
				loadExternalPlugins(
					`api/machines/${encodeURIComponent(machine.id)}/pi-web-plugins/manifest.json`,
					{
						machineId: machine.id,
						shouldLoadPlugin: (entry) =>
							this.plugins.shouldLoadRemotePlugin(
								entry.id,
								entry.machineSpecific,
							),
					},
				),
		)
			.then((loaded) => {
				if (loaded) this.loadedMachinePluginIds.add(machine.id);
			})
			.finally(() => {
				this.machinePluginLoadPromises.delete(machine.id);
			});
		this.machinePluginLoadPromises.set(machine.id, load);
		await load;
	}

	private async registerExternalPlugins(
		label: string,
		load: () => Promise<PiWebPluginRegistration[]>,
	): Promise<boolean> {
		try {
			...
			for (const registration of registrations) {
				try {
					...
					console.warn(
						`Failed to register PI WEB plugin ${registration.id}`,
						error,
					);
				}
			}
		...
		return {
			...
			getSelection: () => {
				...
				return {
					...
					text: editor.state.sliceDoc(sel.from, sel.to),
				};
			},
		};
	}
			...
			installPluginRuntimeScope(
				{
					...
					refreshSelectedMachine: async () => {
						await Promise.all([
							this.machines.refreshMachineHealth(),
							this.machines.refreshMachineRuntime(),
						]);
					},
					removeSelectedMachine: () => this.removeMachine(),
					openSelectedMachine: () => {
						this.openSelectedMachine();
					},
					...
					checkForPiWebUpdates: () =>
						this.piWebStatusController.checkForUpdates(),
					reloadPage: () => {
						this.hardReloadApp();
					},
					deleteWorkspace: (workspace) => this.deleteWorkspace(workspace),
					startSession: () =>
						this.withChatScrollTransition(() => this.startSessionAndOpenChat()),
	...
	private async deleteWorkspace(
		...
		try {
			...
			if (commandWorkspace !== undefined)
				void this.openRuntimeTerminal(machineId, commandWorkspace, {
					terminalId: run.terminalId,
				});
		} catch (error) {
			if (selectedMachineId(this.state) === machineId)
				this.setState({
					error: `Failed to start workspace deletion: ${errorMessage(error)}`,
				});
		}
	}

	private async workspaceForCommandRun(
		run: TerminalCommandRun,
	): Promise<Workspace | undefined> {
		let workspaces =
			this.state.selectedProject?.id === run.projectId
				? this.state.workspaces
				: this.state.workspacesByProjectId[run.projectId];
		if (workspaces === undefined || workspaces.length === 0)
			workspaces = await this.workspaces.refreshProjectWorkspaces(
				run.projectId,
			);
		return workspaces.find((workspace) => workspace.id === run.workspaceId);
	}

	private recordWorkspaceDeletionRun(
		run: TerminalCommandRun,
		machineId: string,
	): void {
		if (selectedMachineId(this.state) !== machineId) return;
		...
		this.setState({
			workspaceDeletionRuns: {
				...this.state.workspaceDeletionRuns,
				[workspaceId]: run,
			},
		});
	...
	private async refreshWorkspaceDeletionRuns(): Promise<void> {
		...
		try {
			const runs = await this.terminalCommandRunsForOrigin(
				"core",
				machineId,
			).listCommandRuns(workspaceDeletionRunFilter(project.id));
			if (selectedMachineId(this.state) !== machineId) return;
			...
			for (const run of Object.values(latestRuns)) {
				if (!isWorkspaceDeletionRunPending(run))
					await this.handleCompletedWorkspaceDeletionRun(run, machineId);
			}
		} catch (error) {
	...
	private updateWorkspaceDeletionPolling(): void {
		const hasPendingDeletion = Object.values(
			this.state.workspaceDeletionRuns,
		).some(isWorkspaceDeletionRunPending);
	...
	private async handleCompletedWorkspaceDeletionRun(
		...
		if (run.status === "succeeded") {
			await this.workspaces.refreshAfterWorkspaceDeleted(
				run.projectId,
				workspaceId,
			);
			if (selectedMachineId(this.state) !== machineId) return;
			this.setState({
				workspaceDeletionRuns: omitWorkspaceDeletionRun(
					this.state.workspaceDeletionRuns,
					workspaceId,
				),
			});
		...
		if (run.status === "failed") {
			this.setState({
				error: "Workspace deletion failed. See terminal output.",
			});
			this.updateWorkspaceDeletionPolling();
		}
	}
	...
	private async removeMachine(
		machine: Machine | undefined = this.state.selectedMachine,
	): Promise<void> {
		if (machine === undefined || machine.kind === "local") return;
		if (
			!window.confirm(
				`Remove ${machine.name}?\n\nThis only removes it from this PI WEB gateway.`,
			)
		)
			return;
		const wasSelected = this.state.selectedMachine?.id === machine.id;
		if (wasSelected) this.rememberCurrentMachineNavigation();
		const fallback = await this.machines.deleteMachine(machine, {
			selectFallback: !wasSelected,
		});
		if (!this.state.machines.some((candidate) => candidate.id === machine.id))
			this.machineNavigation.forget(machine.id);
		if (wasSelected && fallback !== undefined)
			await this.selectMachineWithMemory(fallback, { rememberCurrent: false });
	}
	...
	private openThemeDialog() {
		...
		const autoValue = this.themePreference.auto
			? THEME_AUTO_OFF_VALUE
			: THEME_AUTO_ON_VALUE;
		this.setState({
			themeDialog: {
				title: "Select Theme",
				selectedValue:
					selectedThemeId === undefined
						? autoValue
						: `${THEME_OPTION_PREFIX}${selectedThemeId}`,
				options: [
					{
						value: autoValue,
	...
	private pickTheme(value: string) {
		...
		if (value === THEME_AUTO_ON_VALUE || value === THEME_AUTO_OFF_VALUE) {
			const selectedThemeId =
				this.resolveCurrentThemePreference().selectedTheme?.id;
			if (selectedThemeId === undefined) return;
			this.themePreference = {
				themeId: selectedThemeId,
				auto: value === THEME_AUTO_ON_VALUE,
			};
		...
		const theme = this.plugins
			.getThemes()
			.find((candidate) => candidate.id === themeId);
		if (theme === undefined) return;
		this.themePreference = {
			themeId: theme.id,
			auto: this.themePreference.auto,
		};
		this.applyPreferredTheme(true);
	}
	...
	private toggleTheme(): void {
		...
		const pair =
			this.themePairForTheme(active.id) ?? resolution.selectedThemePair;
		if (pair !== undefined) {
			const nextId = active.colorScheme === "light" ? pair.dark : pair.light;
			const nextTheme = this.plugins
				.getThemes()
		...
		const fallback = this.plugins
			.getThemes()
			.find((candidate) => candidate.colorScheme !== active.colorScheme);
		if (fallback === undefined) return;
		this.themePreference = { themeId: fallback.id, auto: false };
		this.applyPreferredTheme(true);
	}

	private resolveCurrentThemePreference(
	...
	private themePairForTheme(
		themeId: QualifiedContributionId,
	): QualifiedThemePairContribution | undefined {
	...
	private themeOptionLabel(
		...
		const markers = [
			...
			...(theme.id === this.activeThemeId && theme.id !== selectedThemeId
				? ["active"]
				: []),
		];
		return markers.length === 0
			? theme.name
			: `${theme.name} ✓ ${markers.join(" · ")}`;
	}
	...
	private sendPrompt(
		...
		attachments?: import("../api").PromptAttachment[],
		delivery?: import("../../../shared/apiTypes").PromptAttachmentDelivery,
	): void {
		const hasAttachments = attachments !== undefined && attachments.length > 0;
		if (
			!hasAttachments &&
			streamingBehavior === undefined &&
			this.auth.handleSlashCommand(text)
		)
			return;
	...
	private readonly handleSendPrompt = (
		text: string,
		streamingBehavior?: "steer" | "followUp",
		attachments?: import("../api").PromptAttachment[],
		delivery?: import("../../../shared/apiTypes").PromptAttachmentDelivery,
	...
	private readonly handleDismissNotification = (
		...
		return [
			{
				...
				...(unreadCount === 0
					...
					: {
							badge: unreadCount,
							badgeLabel: `${String(unreadCount)} unread`,
							badgeTone: "unread",
						}),
			},
	...
	private renderAppRefresh() {
		return html`<app-refresh-control .onReload=${() => {
			this.hardReloadApp();
		}}></app-refresh-control>`;
	}
      ...
      <div class=${this.panelCollapse.shellClass(state.mainView)} style=${this.panelResize.shellStyle({ navigation: this.resizablePanelConstraints("navigation"), workspace: this.resizablePanelConstraints("workspace") })}>
          ...
          ${
            ...
            ${
							state.authDialog !== undefined
								? html`<auth-dialog .state=${state.authDialog} .onChooseMethod=${(
        ...
        ${
					state.actionPaletteOpen
						? html`<action-palette .actions=${this.getActions()} .onRun=${(
								action: AppAction,
							) => {
        ...
        ${
					state.projectDialogOpen
						? html`<project-dialog .machineId=${selectedMachineId(state)} .onSubmit=${(path: string, create: boolean) => this.projects.addProject(path, create)} .onCancel=${() => {
        ...
        ${
					state.machineDialogOpen
						? html`<machine-dialog .error=${state.error} .onSubmit=${(input: MachineDialogSubmit) => this.submitMachineDialog(input)} .onCancel=${() => {
								this.setState({ machineDialogOpen: false });
							}}></machine-dialog>`
						: null
				}
        ${
					this.sessionCleanupDialog !== undefined
						? html`<session-cleanup-dialog .canCleanup=${this.canCleanupSessions()} .unavailableMessage=${this.sessionCleanupUnavailableMessage()} .preview=${this.sessionCleanupDialog.preview} .previewRequest=${this.sessionCleanupDialog.previewRequest} .result=${this.sessionCleanupDialog.result} .loading=${this.sessionCleanupDialog.loading === true} .running=${this.sessionCleanupDialog.running === true} .error=${this.sessionCleanupDialog.error ?? ""} .onPreview=${(
								request: SessionCleanupRequest,
							) => {
								void this.previewSessionCleanup(request);
							}} .onRun=${(request: SessionCleanupRequest) => {
								void this.runSessionCleanup(request);
							}} .onClose=${() => {
								this.closeSessionCleanupDialog();
        ...
        ${
					this.settingsSection !== undefined
						? html`<settings-dialog .section=${this.settingsSection} .machine=${state.selectedMachine} .machineRuntime=${this.selectedMachineRuntime()} .actions=${this.getDefaultActions()} .onNavigate=${(
								...
								this.closeSettings();
							}} .onConfigSaved=${(config: PiWebConfigValues) => {
								this.applyClientConfig(config);
							}} .onRefreshMachineRuntime=${async (machineId: string) => {
								await this.machines.refreshMachineRuntime(machineId);
							}}></settings-dialog>`
						: null
				}
...
function pluginMachineFromState(
	state: Pick<AppState, "selectedMachine">,
): PluginMachine {
	const machine = state.selectedMachine;
	if (machine !== undefined)
		return { id: machine.id, name: machine.name, kind: machine.kind };
	return { id: "local", name: "local", kind: "local" };
}

function unreadChatIdentity(
	machineId: string,
	session: Pick<SessionInfo, "id" | "cwd">,
): string {
	return JSON.stringify([machineId, session.id, session.cwd]);
}

function selectedChatIdentity(
	state: Pick<AppState, "selectedMachine" | "selectedSession">,
): string | undefined {
	const session = state.selectedSession;
	return session === undefined
		? undefined
		: unreadChatIdentity(selectedMachineId(state), session);
}

function machineUnreadInputsChanged(
	previous: AppState,
	next: AppState,
): boolean {
	return (
...
function machineActivitySubscriptionInputsChanged(
	previous: AppState,
	next: AppState,
): boolean {
	return (
...
function shouldSubscribeToMachineActivity(
	machine: Machine,
...
function shouldRefreshMachineActivity(
	machine: Machine,
	health: MachineHealth | undefined,
): boolean {
...
function remoteRouteRestoreRetryDelay(attempt: number): number {
	const index = Math.min(
		attempt,
		REMOTE_ROUTE_RESTORE_RETRY_DELAYS_MS.length - 1,
	);
...
function omitWorkspaceDeletionRun(
	...
	return Object.fromEntries(
		Object.entries(runs).filter(([candidate]) => candidate !== workspaceId),
	);
}
...
function thinkingDescription(level: string): string | undefined {
	switch (level) {
		...
		default:
			return undefined; // unknown level from a newer pi: no description
	}
}
