...
import {
	TERMINAL_SOFT_KEYS,
	terminalSoftKeySequence,
	type TerminalModesSnapshot,
	type TerminalSoftKeyDefinition,
} from "../terminalKeys";
...
export class TerminalSoftKeys extends LitElement {
	...
	@property({ attribute: false }) onInput: (
		data: string,
		options: TerminalSoftKeyInputOptions,
	) => void = () => undefined;
	...
	private onSoftKeyClick(
		event: MouseEvent,
		key: TerminalSoftKeyDefinition,
	): void {
		if (
			Date.now() - this.lastPointerFinishedAt <
	...
	override render() {
		...
		return html`
      <div class="terminal-soft-keys" role="toolbar" aria-label=${t("terminal.softKeys")}>
        ${TERMINAL_SOFT_KEYS.map(
					(key) => html`
          <button
            type="button"
            class="soft-key"
            ...
            @pointerdown=${(event: PointerEvent) => {
							this.onSoftKeyPointerDown(event, key);
						}}
            @pointermove=${(event: PointerEvent) => {
							this.onSoftKeyPointerMove(event);
						}}
            @pointerup=${(event: PointerEvent) => {
							this.onSoftKeyPointerUp(event, key);
						}}
            @pointercancel=${(event: PointerEvent) => {
							this.onSoftKeyPointerCancel(event);
						}}
            @click=${(event: MouseEvent) => {
							this.onSoftKeyClick(event, key);
						}}
          >${key.label}</button>
        `,
				)}
