...
describe("PromptEditor draft replacement", () => {
	it("replaces durable and CodeMirror text, moves the cursor, and resets completion state", () => {
		...
		Reflect.set(editor, "completions", [
			{ kind: "command", insertText: "/tree", replaceFrom: 0, replaceTo: 4 },
		]);
		...
		expect(
			loadDraft(machineRuntimeSessionKey("remote-a", "pi", "session-1")),
		).toBe("!pwd");
		expect(Reflect.get(editor, "currentInputMode")).toEqual({
			kind: "shell",
		...
		if (!isRecord(transaction) || !isRecord(transaction["selection"]))
			throw new Error("Expected a CodeMirror replacement transaction");
	...
	it("clears an existing durable draft and CodeMirror document", () => {
		...
		const key = machineRuntimeSessionKey("local", "pi", "session-2");
		const dispatch = vi.fn<(transaction: unknown) => void>();
		...
		const transaction = dispatch.mock.calls[0]?.[0];
		if (!isRecord(transaction) || !isRecord(transaction["selection"]))
			throw new Error("Expected a CodeMirror clearing transaction");
