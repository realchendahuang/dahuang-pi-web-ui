...
import { DEFAULT_LOCALE, resetLocaleForTests, setLocale } from "./i18n";
import type {
	SessionNotification,
...
function notification(
	order: number,
	severity: SessionNotification["severity"] = "info",
...
function summary(
	overrides: Partial<SessionNotificationSummary> = {},
): SessionNotificationSummary {
	return {
		sessionId: "session-1",
		cwd: "/repo",
...
function snapshot(
	notifications: SessionNotification[] = [notification(1)],
	overrides: Partial<SessionNotificationInboxSnapshot> = {},
): SessionNotificationInboxSnapshot {
	return {
		...
		summary: summary({
			...
			...optionalHighestSeverity(notifications),
		}),
		notifications,
		dismissThrough: {
			order: notifications[0]?.order ?? 0,
			overflowWatermark: 0,
		},
		...overrides,
	};
...
function addedEvent(
	entry: SessionNotification,
	inboxRevision: number,
	catalogRevision = inboxRevision,
): SessionNotificationInboxEvent {
	const notifications = [entry, notification(1)].filter(
		(item, index, all) =>
...
describe("selected notification projection", () => {
	it("joins a snapshot with newer buffered events without duplicate cards or replay announcements", () => {
		let inbox = installSelectedNotificationSnapshot(
			undefined,
			target,
			snapshot(),
		);
		...
		expect(inbox.notifications.map((entry) => entry.id)).toEqual([
			"daemon-a:2",
			"daemon-a:1",
		]);
		expect(inbox.announcements).toEqual([
			{ id: "daemon-a:2:daemon-a:2", severity: "warning", message: "notice 2" },
		]);
	...
	it("marks gaps and resync deltas stale and clears old-daemon contents", () => {
		const current = installSelectedNotificationSnapshot(
			undefined,
			target,
			snapshot(),
		);
		const gap = applySelectedNotificationEvent(
			current,
			target,
			addedEvent(notification(3), 3),
		);
		...
		expect(
			applySelectedNotificationEvent(current, target, resyncEvent).needsRefresh,
		).toBe(true);
	...
	it("drops old-daemon optimistic cutoffs and announcements when a restart snapshot arrives", () => {
		const current = {
			...
			announcements: [
				{ id: "old-announcement", severity: "error" as const, message: "old" },
			],
		};
		const restarted = installSelectedNotificationSnapshot(
			current,
			target,
			snapshot([notification(1)], { daemonInstanceId: "daemon-b" }),
		);
	...
	it("applies optimistic individual and cutoff dismissals while preserving newer arrivals", () => {
		const base = installSelectedNotificationSnapshot(
			undefined,
			target,
			snapshot(
...
describe("notification presentation helpers", () => {
	it("chooses next, previous, then header focus targets", () => {
		...
		expect(
			notificationFocusTargetAfterDismiss(notifications, "daemon-a:2"),
		).toEqual({ kind: "notification", notificationId: "daemon-a:1" });
		expect(
			notificationFocusTargetAfterDismiss(notifications, "daemon-a:1"),
		).toEqual({ kind: "notification", notificationId: "daemon-a:2" });
		expect(
			notificationFocusTargetAfterDismiss([notification(1)], "daemon-a:1"),
		).toEqual({ kind: "header" });
	});
	...
	it("retains collapse state by exact machine, cwd, and session identity", () => {
		...
		expect(
			notificationTrayIsCollapsed(collapsed, {
				...target,
				machineId: "remote",
			}),
		).toBe(false);
		expect(
			notificationTrayIsCollapsed(collapsed, { ...target, cwd: "/other" }),
		).toBe(false);
		expect(
			notificationTrayIsCollapsed(collapsed, {
				...target,
		...
		expect(notificationTargetKey(target)).not.toBe(
			notificationTargetKey({ ...target, cwd: "/repo|session-1" }),
		);
		expect(
			notificationTrayIsCollapsed(
				setNotificationTrayCollapsed(collapsed, target, false),
				target,
			),
		).toBe(false);
	});
	...
	it("derives compact tray copy and a count that includes older unseen notifications", () => {
		...
		expect(notificationInboxOverflowLabel(1)).toBe(
			"1 older notification not shown.",
		);
		expect(notificationInboxOverflowLabel(23)).toBe(
			"23 older notifications not shown.",
		);
		expect(notificationMessageTruncationLabel({ truncated: true })).toBe(
			"Message truncated at 8 KiB.",
		);
		expect(
			notificationMessageTruncationLabel({ truncated: false }),
		).toBeUndefined();
	});
	...
	it("keeps live announcements concise and dismiss labels meaningful", () => {
		...
		const announcement: SessionNotificationAnnouncement = {
			...
			severity: "error",
			message:
				"An arbitrarily long extension message that should not be read by the assertive live region",
		};
		...
		expect(notificationAnnouncementLabel(announcement)).toBe(
			"Error notification received.",
		);
		expect(notificationDismissLabel(longNotification)).toMatch(
			/^Dismiss notification: Build failed x+…$/u,
		);
		expect(
			notificationDismissLabel({ message: "   ", severity: "warning" }),
		).toBe("Dismiss warning notification");
	});
});

function optionalHighestSeverity(
	notifications: readonly SessionNotification[],
): { highestSeverity?: SessionNotification["severity"] } {
...
function highestSeverity(
	notifications: readonly SessionNotification[],
): SessionNotification["severity"] | undefined {
	if (notifications.some((entry) => entry.severity === "error")) return "error";
	if (notifications.some((entry) => entry.severity === "warning"))
		return "warning";
