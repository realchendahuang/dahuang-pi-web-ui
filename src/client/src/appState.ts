...
export interface AppState {
	...
	/** Agent runtime catalogs (Pi/OMP), kept separate from Pi-Web component runtime status. */
	agentRuntimeCatalogs: Record<string, AgentRuntimesResponse>;
	agentRuntimeCatalogErrors: Record<string, string>;
	selectedAgentRuntimeIds: Partial<Record<string, AgentRuntimeId>>;
	projects: Project[];
	...
	modelDialog:
		| { title: string; options: CommandOption[]; selectedValue?: string }
		| undefined;
	thinkingDialog:
		| { title: string; options: CommandOption[]; selectedValue?: string }
		| undefined;
...
export type WorkspaceScopedStateReset = Pick<
	AppState,
	| "sessions"
