...
import {
	...
	templateEventHandlerAfterValue,
...
describe("capturePromptAttachments", () => {
	...
	it("captures generic files with their browser MIME type", async () => {
		const result = await capturePromptAttachments(
			[
				file("report.pdf", "application/pdf", 1234),
	...
	it("uses application/octet-stream when the browser does not provide a MIME type", async () => {
		const result = await capturePromptAttachments([file("archive", "")], () =>
			Promise.resolve("x"),
		);
...
describe("effectivePromptAttachmentDelivery", () => {
	it("preserves inline delivery when all pending attachments are supported images", () => {
		expect(
			effectivePromptAttachmentDelivery("inline", [
	...
	it("preserves an explicit folder preference for supported images", () => {
		expect(
			effectivePromptAttachmentDelivery("folder", [
				{ kind: "image", mimeType: "image/png" },
			]),
		).toBe("folder");
	});
...
describe("PromptEditor attachment wiring", () => {
	...
	it("captures pasted files, strips data URL prefixes, and keeps successful reads when others fail", async () => {
		...
		const restoreFileReader = installFileReaderStub([
			...
			{
				kind: "error",
				error: new DOMException("File unavailable", "NotReadableError"),
			},
		]);
		...
		try {
			...
			const send = templateEventHandlerAfterMarker(
				editor.render(),
				"send-button",
			);
	...
	it("removes a pending attachment chip before sending the remaining attachments", () => {
		...
		const removeReport = templateEventHandlerAfterValue(
			editor.render(),
			"Remove report.pdf",
			"@click=",
		);
...
function setPromptEditorPrivate(
	editor: PromptEditor,
	property: string,
	value: unknown,
): void {
	if (!Reflect.set(editor, property, value))
		throw new Error(`Failed to set PromptEditor ${property}`);
}
	...
	class StubFileReader {
		...
		readAsDataURL(): void {
			...
			if (outcome === undefined)
				throw new Error("Unexpected FileReader.readAsDataURL call");
