...
export function sessionTimeGroupId(
	isoDate: string,
	now: Date = new Date(),
): SessionTimeGroupId {
 ...
 */
export function groupBySessionTime<T>(
	items: readonly T[],
	modifiedOf: (item: T) => string,
	now: Date = new Date(),
): SessionTimeGroup<T>[] {
 ...
 */
export function groupSessionRowsByTime<T extends { depth: number }>(
	rows: readonly T[],
	modifiedOf: (row: T) => string,
	now: Date = new Date(),
): SessionTimeGroup<T>[] {
	let rootModified = "";
	const effective = rows.map((row) => {
		...
		return {
			row,
			effectiveModified: row.depth === 0 ? modifiedOf(row) : rootModified,
		};
	});
	return groupBySessionTime(
		effective,
		(entry) => entry.effectiveModified,
		now,
	).map((group) => ({
		...group,
