...
import { localizeActionLabels, localizePanelTitle } from "../i18n";
import type {
	PiWebPluginRegistration,
	PluginAction,
	PluginRuntimeContext,
	QualifiedContributionId,
	QualifiedPluginAction,
...
const pluginRuntimeScopes = new WeakMap<
	PluginRuntimeContext,
	(pluginId: string) => PluginRuntimeContext
>();
const workspacePanelScopes = new WeakMap<
	WorkspacePanelContext,
	(pluginId: string) => WorkspacePanelContext
>();
...
export class PluginRegistry {
	...
	private readonly remoteMachineSpecificPluginIds = new Map<
		string,
		Set<string>
	>();
	...
	register(registration: PiWebPluginRegistration): void {
		...
		const machineSpecific = this.parseMachineSpecific(
			id,
			registration.machineSpecific,
		);
		if (this.pluginIds.has(id)) throw new Error(`Duplicate plugin id: ${id}`);
		if (
			this.isRemoteDuplicateHiddenByGateway(
				registration.sourcePluginId,
		...
		if (apiVersion !== 1)
			throw new Error(
				`Unsupported plugin API version for ${id}: ${String(apiVersion)}`,
			);
		...
		for (const panel of contributions.workspacePanels ?? [])
			this.workspacePanels.push(
				this.qualifyWorkspacePanel(
					id,
					panel,
					registration.machineId,
					registration.sourcePluginId,
		...
		for (const contribution of contributions.workspaceLabels ?? [])
			this.workspaceLabels.push(
				this.qualifyWorkspaceLabelContribution(
					id,
					contribution,
					registration.machineId,
					registration.sourcePluginId,
		...
		if (registration.machineId === undefined) {
			for (const theme of contributions.themes ?? [])
				this.themes.push(this.qualifyTheme(id, theme));
			for (const pair of contributions.themePairs ?? [])
				this.themePairs.push(this.qualifyThemePair(id, pair));
			...
			addMappedSetValue(
				this.remoteMachineSpecificPluginIds,
				registration.sourcePluginId,
				registration.machineId,
			);
		}
		...
		return this.actions
			...
			.map((action) => {
				...
				const disabledReason =
					enabled === false
						? action.disabledReason?.(scopedContext)
						: undefined;
				const labels = localizeActionLabels({
					id: action.id,
				...
				if (labels.description !== undefined)
					qualified.description = labels.description;
				if (action.shortcut !== undefined) qualified.shortcut = action.shortcut;
				if (labels.group !== undefined) qualified.group = labels.group;
				if (enabled !== undefined) qualified.enabled = enabled;
				if (disabledReason !== undefined && disabledReason !== "")
					qualified.disabledReason = disabledReason;
				return qualified;
			});
	}

	getWorkspacePanels(): QualifiedWorkspacePanelContribution[] {
		return [...this.workspacePanels]
			.map((panel) => ({
				...panel,
				title: localizePanelTitle(panel.localId, panel.title),
			}))
			.sort(
				(left, right) =>
	...
	private qualifyWorkspacePanel(
		...
		return {
			...
			visible: (context: WorkspacePanelContext) =>
				...
				(visible?.(workspacePanelContextFor(context, pluginId)) ?? true),
			...(badge === undefined
				? {}
				: {
						badge: (context: WorkspacePanelContext) =>
							this.isContributionActive(
								pluginId,
								machineId,
								context.machine.id,
								...
								? badge(workspacePanelContextFor(context, pluginId))
								: undefined,
					}),
			render: (context: WorkspacePanelContext) =>
				panel.render(workspacePanelContextFor(context, pluginId)),
		};
	}
	...
	private qualifyThemePair(
		pluginId: string,
		pair: ThemePairContribution,
	): QualifiedThemePairContribution {
	...
	private qualify(pluginId: string, localId: string): QualifiedContributionId {
		...
		if (this.contributionIds.has(qualified))
			throw new Error(`Duplicate contribution id: ${qualified}`);
		this.contributionIds.add(qualified);
		return qualified;
	}

	private qualifyReference(
		pluginId: string,
		localId: string,
	): QualifiedContributionId {
		this.validateLocalId(localId);
	...
	private isContributionActive(
		...
		if (machineId === undefined)
			return !this.isGatewayPluginHiddenForMachine(pluginId, selectedMachineId);
		return (
			machineId === selectedMachineId &&
			!this.isRemotePluginHiddenByGateway(sourcePluginId, machineId)
		);
	}

	private isRemoteDuplicateHiddenByGateway(
		sourcePluginId: string | undefined,
		machineId: string | undefined,
	...
	private isRemotePluginHiddenByGateway(
		sourcePluginId: string | undefined,
		machineId: string,
	): boolean {
	...
	private isGatewayPluginHiddenForMachine(
		pluginId: string,
		machineId: string,
	): boolean {
		return (
			...
			(this.gatewayMachineSpecificPluginIds.has(pluginId) ||
				this.remoteMachineSpecificPluginIds.get(pluginId)?.has(machineId) ===
					true)
		);
	}
	...
	private validateLocalId(localId: string): void {
		if (!localIdPattern.test(localId))
			throw new Error(`Invalid contribution id: ${localId}`);
	}
	...
	private parseMachineSpecific(pluginId: string, value: unknown): boolean {
		...
		if (typeof value !== "boolean")
			throw new Error(
				`Invalid plugin machineSpecific value for ${pluginId}: ${formatUnknownValue(value)}`,
			);
		return value;
	}
}

function pluginRuntimeContextFor(
...
export function installPluginRuntimeScope(
	context: PluginRuntimeContext,
	scope: (pluginId: string) => PluginRuntimeContext,
): PluginRuntimeContext {
...
export function installWorkspacePanelScope(
	context: WorkspacePanelContext,
	scope: (pluginId: string) => WorkspacePanelContext,
): WorkspacePanelContext {
	workspacePanelScopes.set(context, scope);
	return context;
}

function addMappedSetValue(
