...
export interface SelectedSessionNotificationInbox
	extends SessionNotificationTarget {
...
export interface SelectedSessionNotificationView
	extends SessionNotificationTarget {
	daemonInstanceId: string;
	notifications: SessionNotification[];
...
const emptyDismissThrough: SessionNotificationDismissThrough = {
	order: 0,
	overflowWatermark: 0,
};

export function loadingSelectedNotificationInbox(
...
export function installSelectedNotificationSnapshot(
	...
	if (
		snapshot.summary.sessionId !== target.sessionId ||
		snapshot.summary.cwd !== target.cwd
	)
		throw new Error(
			"Notification inbox snapshot does not match the selected session",
		);
	const sameDaemon =
		current !== undefined &&
	...
	return {
		...
		...(sameDaemon && current.optimisticDismissAllThrough !== undefined
			? { optimisticDismissAllThrough: current.optimisticDismissAllThrough }
			: {}),
...
export function applySelectedNotificationEvent(
	...
	if (
		event.summary.sessionId !== target.sessionId ||
		event.summary.cwd !== target.cwd
	) {
		return {
			value: current ?? loadingSelectedNotificationInbox(target),
			needsRefresh: false,
			changed: false,
		};
	}
	...
	if (current.daemonInstanceId !== event.daemonInstanceId) {
		return {
			...
			changed:
				...
				current.daemonInstanceId !== undefined ||
				current.notifications.length > 0,
		};
	}
	const currentRevision = current.summary?.inboxRevision ?? 0;
	if (event.summary.inboxRevision <= currentRevision)
		return { value: current, needsRefresh: false, changed: false };
	if (
		current.status !== "fresh" ||
		event.summary.inboxRevision !== currentRevision + 1 ||
		event.delta.kind === "resync"
	) {
		const stale = { ...current, status: "stale" as const };
		return {
			value: stale,
			needsRefresh: true,
			changed: current.status !== "stale",
		};
	}
		...
		case "added": {
			...
			if (
				current.notifications.some(
					(notification) => notification.id === delta.notification.id,
				)
			) {
				return {
					value: { ...current, status: "stale" },
					needsRefresh: true,
					changed: true,
				};
			}
			notifications = [delta.notification, ...current.notifications]
				.filter(
					(notification) => notification.id !== delta.evictedNotificationId,
				)
	...
	if (
		!notificationListMatchesSummary(notifications, event.summary) ||
	...
	const announcements =
		announcement === undefined
			...
			: [...current.announcements, announcement].slice(
					-SESSION_NOTIFICATION_LIMIT,
				);
	return {
		value: {
...
export function selectedNotificationView(
	inbox: SelectedSessionNotificationInbox | undefined,
): SelectedSessionNotificationView | undefined {
	if (
		inbox?.status !== "fresh" ||
		inbox.daemonInstanceId === undefined ||
		inbox.summary === undefined
	)
		return undefined;
	const pendingDismissedIds = new Set(inbox.optimisticDismissedIds);
	const through = inbox.optimisticDismissAllThrough;
	const notifications = inbox.notifications.filter(
		(notification) =>
			!pendingDismissedIds.has(notification.id) &&
			(through === undefined || notification.order > through.order),
	);
	let discardedCount = effectiveDiscardedCount(
		inbox.summary.discardedCount,
		inbox.dismissThrough.overflowWatermark,
		through?.overflowWatermark,
	);
	if (notifications.length === 0 && pendingDismissedIds.size > 0)
		discardedCount = 0;
	return {
...
export function notificationAnnouncementLabel(
	announcement: Pick<SessionNotificationAnnouncement, "severity">,
): string {
	return t("chat.notificationReceived", {
		severity: notificationSeverityLabel(announcement.severity),
	});
}

export function notificationInboxTotalCount(
	inbox: Pick<
		SelectedSessionNotificationView,
		"retainedCount" | "discardedCount"
	>,
): number {
	return inbox.retainedCount + inbox.discardedCount;
}

export function notificationTrayHeading(
	inbox: Pick<
		SelectedSessionNotificationView,
		"retainedCount" | "discardedCount"
	>,
): string {
	return t("chat.notifications", { count: notificationInboxTotalCount(inbox) });
}

export function notificationDismissLabel(
	notification: Pick<SessionNotification, "message" | "severity">,
): string {
	const message = notification.message.replace(/\s+/gu, " ").trim();
	if (message === "")
		return t("chat.dismissSeverity", {
			severity: notificationSeverityLabel(notification.severity).toLowerCase(),
		});
	...
	const summary =
		characters.length <= maxCharacters
			...
			: `${characters
					...
					.join("")
					.trimEnd()}…`;
	return t("chat.dismissWithSummary", { summary });
}
...
export function notificationFocusTargetAfterDismiss(
	...
	if (next !== undefined)
		return { kind: "notification", notificationId: next.id };
	const previous = notifications[index - 1];
	return previous === undefined
		? { kind: "header" }
		: { kind: "notification", notificationId: previous.id };
}

export function notificationTargetKey(
	target: SessionNotificationTarget,
): string {
	return JSON.stringify([target.machineId, target.cwd, target.sessionId]);
}

export function notificationTrayIsCollapsed(
	collapsedTargetKeys: ReadonlySet<string>,
	target: SessionNotificationTarget,
): boolean {
	return collapsedTargetKeys.has(notificationTargetKey(target));
}

export function setNotificationTrayCollapsed(
	collapsedTargetKeys: ReadonlySet<string>,
	target: SessionNotificationTarget,
	collapsed: boolean,
): ReadonlySet<string> {
...
export function notificationMessageTruncationLabel(
	notification: Pick<SessionNotification, "truncated">,
): string | undefined {
...
export function notificationTargetsEqual(
	left: SessionNotificationTarget,
	right: SessionNotificationTarget,
): boolean {
	return (
...
function staleSelectedNotificationInbox(
	target: SessionNotificationTarget,
): SelectedSessionNotificationInbox {
	return {
...
function notificationListMatchesSummary(
	notifications: readonly SessionNotification[],
	summary: SessionNotificationSummary,
): boolean {
	return (
		notifications.length === summary.retainedCount &&
		highestNotificationSeverity(notifications) === summary.highestSeverity
	);
}
...
function highestNotificationSeverity(
	...
	for (const notification of notifications)
		highest = higherNotificationSeverity(highest, notification.severity);
	return highest;
}

function effectiveDiscardedCount(
	discardedCount: number,
	overflowWatermark: number,
	throughOverflowWatermark: number | undefined,
): number {
	if (discardedCount === 0 || throughOverflowWatermark === undefined)
		return discardedCount;
	const firstWatermark = overflowWatermark - discardedCount + 1;
	const acknowledged = Math.max(
		0,
		Math.min(discardedCount, throughOverflowWatermark - firstWatermark + 1),
	);
	return discardedCount - acknowledged;
}

function optionalSeverity(severity: SessionNotificationSeverity | undefined): {
	highestSeverity?: SessionNotificationSeverity;
} {
