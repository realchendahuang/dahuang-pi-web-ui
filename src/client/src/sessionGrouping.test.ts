...
describe("sessionTimeGroupId", () => {
	it("groups timestamps into recency buckets", () => {
		expect(sessionTimeGroupId(iso(new Date(2026, 6, 25, 9, 0, 0)), NOW)).toBe(
			"today",
		);
		expect(sessionTimeGroupId(iso(new Date(2026, 6, 25, 0, 0, 0)), NOW)).toBe(
			"today",
		);
		expect(sessionTimeGroupId(iso(new Date(2026, 6, 24, 23, 59, 0)), NOW)).toBe(
			"yesterday",
		);
		expect(sessionTimeGroupId(iso(new Date(2026, 6, 20, 12, 0, 0)), NOW)).toBe(
			"week",
		);
		expect(sessionTimeGroupId(iso(new Date(2026, 6, 18, 12, 0, 0)), NOW)).toBe(
			"week",
		);
		expect(sessionTimeGroupId(iso(new Date(2026, 6, 17, 12, 0, 0)), NOW)).toBe(
			"older",
		);
		expect(sessionTimeGroupId(iso(new Date(2025, 0, 1, 12, 0, 0)), NOW)).toBe(
			"older",
		);
	});
...
describe("formatRelativeTime", () => {
	...
	it("renders bucket labels for recent days", () => {
		expect(formatRelativeTime(iso(new Date(2026, 6, 24, 10, 0, 0)), NOW)).toBe(
			"Yesterday",
		);
		expect(
			formatRelativeTime(iso(new Date(2026, 6, 21, 10, 0, 0)), NOW),
		).toMatch(/Tue/);
	});

	it("renders a short date for older items in the same year", () => {
		expect(
			formatRelativeTime(iso(new Date(2026, 2, 12, 10, 0, 0)), NOW),
		).toMatch(/Mar 12/);
	});

	it("includes the year for earlier years", () => {
		expect(
			formatRelativeTime(iso(new Date(2024, 2, 12, 10, 0, 0)), NOW),
		).toMatch(/2024/);
	});
...
describe("groupSessionRowsByTime", () => {
	it("keeps tree children in their root session's group", () => {
		const rows = [
			{
				depth: 0,
				modified: iso(new Date(2026, 6, 25, 10, 0, 0)),
				id: "root-today",
			},
			{
				depth: 1,
				modified: iso(new Date(2026, 0, 1, 10, 0, 0)),
				id: "child-old",
			},
			{
				depth: 0,
				modified: iso(new Date(2026, 6, 1, 10, 0, 0)),
				id: "root-older",
			},
		];
