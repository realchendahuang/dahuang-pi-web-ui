...
describe("panel resize behavior", () => {
	...
	it("clamps panel widths to broad fallback bounds", () => {
		expect(clampPanelWidth("navigation", 50)).toBe(200);
	...
	it("reads, writes, and clears stored panel widths", () => {
		...
		writeStoredPanelSizes(
			{ navigationPanelWidth: 260, workspacePanelWidth: 640 },
			storage,
		);
	...
	it("clamps stored panel widths and ignores invalid values", () => {
		const storage = new FakeStorage({
			[PANEL_SIZE_STORAGE_KEY]: JSON.stringify({
				version: 1,
				navigationPanelWidth: 9999,
				workspacePanelWidth: "wide",
			}),
		});

		expect(readStoredPanelSizes(storage)).toEqual({
			navigationPanelWidth: 4096,
		});
	});
	...
	it("ignores storage failures", () => {
		...
		expect(readStoredPanelSizes(storage)).toEqual({});
		expect(() => {
			writeStoredPanelSizes({ navigationPanelWidth: 260 }, storage);
		}).not.toThrow();
	});
});
