...
export type PanelSizeStorage = Pick<
	Storage,
	"getItem" | "setItem" | "removeItem"
>;
export type PanelResizeConstraintsBySide = Partial<
	Record<ResizablePanelSide, PanelResizeConstraints>
>;
...
export class PanelResizeController implements ReactiveController {
	...
	panelWidth(side: ResizablePanelSide, measuredWidth?: number): number {
		return clampPanelWidth(
			side,
			measuredWidth ??
				this.storedPanelWidth(side) ??
				this.constraints(side).defaultWidth,
		);
	}
	...
	resetPanels(options: PanelResetOptions = {}): void {
		if (
			this.panelSizes.navigationPanelWidth === undefined &&
			this.panelSizes.workspacePanelWidth === undefined
		)
			return;
		this.panelSizes = {};
		if (options.persist !== false) this.persistPanelSizes();
	...
	shellStyle(constraintsBySide: PanelResizeConstraintsBySide = {}): string {
		...
		if (this.panelSizes.navigationPanelWidth !== undefined) {
			declarations.push(
				`--navigation-panel-size: ${formatPanelWidth(clampPanelWidth("navigation", this.panelSizes.navigationPanelWidth, constraintsBySide.navigation))};`,
			);
		}
		if (this.panelSizes.workspacePanelWidth !== undefined) {
			declarations.push(
				`--workspace-panel-size: ${formatPanelWidth(clampPanelWidth("workspace", this.panelSizes.workspacePanelWidth, constraintsBySide.workspace))};`,
			);
		}
	...
	private storedPanelWidth(side: ResizablePanelSide): number | undefined {
		return side === "navigation"
			? this.panelSizes.navigationPanelWidth
			: this.panelSizes.workspacePanelWidth;
	}
}

export function panelResizeConstraints(
	side: ResizablePanelSide,
...
export function panelWidthFromDrag(
	...
	startClientX: number,
	currentClientX: number,
	constraints = panelResizeConstraints(side),
): number {
	return clampPanelWidth(
		side,
		startWidth + panelResizeDelta(side, startClientX, currentClientX),
		constraints,
	);
}
...
export function panelWidthFromKeyboard(
	...
	currentWidth: number,
	key: string,
	options: PanelKeyboardResizeOptions = {},
): number | undefined {
	...
	const step =
		options.largeStep === true
			? constraints.largeKeyboardStep
			: constraints.keyboardStep;
...
export function clampPanelWidth(
	...
	return Math.round(
		Math.min(Math.max(width, constraints.minWidth), constraints.maxWidth),
	);
}

export function readStoredPanelSizes(
	storage: PanelSizeStorage | undefined = browserPanelSizeStorage(),
...
export function writeStoredPanelSizes(
	panelSizes: PanelSizePreferences,
	storage: PanelSizeStorage | undefined = browserPanelSizeStorage(),
): void {
	if (storage === undefined) return;
	try {
		if (
			panelSizes.navigationPanelWidth === undefined &&
			panelSizes.workspacePanelWidth === undefined
		) {
			storage.removeItem(PANEL_SIZE_STORAGE_KEY);
			return;
		}
		const envelope: StoredPanelSizeEnvelope = { version: 1 };
		if (panelSizes.navigationPanelWidth !== undefined)
			envelope.navigationPanelWidth = clampPanelWidth(
				"navigation",
				panelSizes.navigationPanelWidth,
			);
		if (panelSizes.workspacePanelWidth !== undefined)
			envelope.workspacePanelWidth = clampPanelWidth(
				"workspace",
				panelSizes.workspacePanelWidth,
			);
...
function parseStoredPanelSizes(value: unknown): PanelSizePreferences {
	...
	if (navigationWidth !== undefined)
		panelSizes.navigationPanelWidth = clampPanelWidth(
			"navigation",
			navigationWidth,
		);
	if (workspaceWidth !== undefined)
		panelSizes.workspacePanelWidth = clampPanelWidth(
			"workspace",
			workspaceWidth,
		);
	return panelSizes;
...
function panelSizesWithWidth(
	panelSizes: PanelSizePreferences,
	side: ResizablePanelSide,
	width: number,
): PanelSizePreferences {
	if (side === "navigation")
		return { ...panelSizes, navigationPanelWidth: width };
	return { ...panelSizes, workspacePanelWidth: width };
}

function panelSizesWithoutSide(
	panelSizes: PanelSizePreferences,
	side: ResizablePanelSide,
): PanelSizePreferences {
	if (side === "navigation") {
		return panelSizes.workspacePanelWidth === undefined
			? {}
			: { workspacePanelWidth: panelSizes.workspacePanelWidth };
	}
	return panelSizes.navigationPanelWidth === undefined
		? {}
		: { navigationPanelWidth: panelSizes.navigationPanelWidth };
}
