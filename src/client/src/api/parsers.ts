import {
	...
	type PiWebAgentDirEnvSource,
	type PiWebCapability,
	type PiWebComponentStatus,
	type PiWebConfigEnvOverrides,
	type PiWebConfigResponse,
	type PiWebConfigValues,
	type PiWebInstallationInfo,
	type PiWebPluginConfigMap,
	type PiWebPluginInfo,
	type PiWebPluginsResponse,
	type PiWebPluginScope,
	type PiWebReleaseStatus,
	type PiWebRuntimeComponent,
	type PiWebRuntimeResponse,
	type PiWebServiceComponent,
	type PiWebShortcutConfig,
	type PiWebStatusMessage,
	type PiWebStatusResponse,
	type PiWebStatusSeverity,
	type Project,
	type QueuedSessionMessage,
	type SavedPromptAttachment,
	type SessionBulkArchiveResponse,
	type SessionBulkDeleteArchivedResponse,
	type SessionBulkFailure,
	type SessionCleanupExecuteResponse,
	type SessionCleanupPreviewResponse,
	type SessionCleanupProjectSummary,
	type SessionCleanupThresholds,
	type SessionCleanupTotals,
	type SessionInfo,
	type SessionModel,
	...
	type SessionWarning,
	type SessionWarningSeverity,
	type SlashCommand,
	type TerminalCommandRun,
	type TerminalCommandRunStatus,
	type TerminalInfo,
	type ThinkingLevelsResponse,
...
import { isOmpSettingType } from "../../../shared/apiTypes";
import {
...
const knownAgentRuntimeCapabilities: ReadonlySet<string> = new Set(
	Object.values(AGENT_RUNTIME_CAPABILITIES),
);
...
export function parseMessagePage(value: unknown): MessagePage {
	if (Array.isArray(value))
		return { messages: value, start: 0, total: value.length };
	const record = requireRecord(value);
	return {
		messages: parseUnknownArray(record["messages"]),
		start: requireNumber(record, "start"),
...
export function parseMachineHealth(value: unknown): MachineHealth {
	...
	return {
		...
		...(record["web"] === undefined
			? {}
			: { web: parsePiWebComponentStatus(record["web"]) }),
		...(record["sessiond"] === undefined
			? {}
			: { sessiond: parsePiWebComponentStatus(record["sessiond"]) }),
...
function requireMachineKind(
	record: Record<string, unknown>,
	key: string,
): MachineKind {
	const value = requireString(record, key);
	if (value !== "local" && value !== "remote")
		throw new Error(`Expected machine kind field: ${key}`);
	return value;
}

function optionalMachineStatus(
	record: Record<string, unknown>,
	key: string,
	...
	if (
		...
		throw new Error(`Expected machine status field: ${key}`);
	return value;
}
...
export function parseWorkspace(value: unknown): Workspace {
	...
	return {
		...
		...optionalField(
			"effectiveConfig",
			optionalWorkspaceEffectiveConfig(record["effectiveConfig"]),
		),
	};
}

function optionalWorkspaceEffectiveConfig(
	value: unknown,
): Workspace["effectiveConfig"] | undefined {
	if (value === undefined) return undefined;
	if (!isRecord(value) || Array.isArray(value))
		throw new Error("Invalid workspace effectiveConfig field");
	return {
		...optionalField("uploads", optionalUploads(value["uploads"])),
	};
}

export function parseAgentRuntimesResponse(
	value: unknown,
): AgentRuntimesResponse {
	const record = requireRecord(value);
	const defaultRuntimeId = parseAgentRuntimeId(
		record["defaultRuntimeId"],
		"defaultRuntimeId",
	);
	const runtimes = arrayOf(parseAgentRuntimeDescriptor)(record["runtimes"]);
	if (!runtimes.some((runtime) => runtime.id === defaultRuntimeId))
		throw new Error(
			"Default agent runtime is not present in the runtime catalog",
		);
	return { defaultRuntimeId, runtimes };
}

function parseAgentRuntimeDescriptor(value: unknown): AgentRuntimeDescriptor {
	const record = requireRecord(value);
	const kind = requireString(record, "kind");
	if (kind !== "pi-embedded" && kind !== "omp-rpc")
		throw new Error("Invalid agent runtime driver kind");
	const capabilities = parseAgentRuntimeCapabilities(record["capabilities"]);
	return {
		id: parseAgentRuntimeId(record["id"], "id"),
		kind,
		label: requireString(record, "label"),
		available: requireBoolean(record, "available"),
		command: requireString(record, "command"),
		profileDir: requireString(record, "profileDir"),
		capabilities,
		...optionalField("version", optionalString(record, "version")),
		...optionalField(
			"protocolVersion",
			optionalNumber(record, "protocolVersion"),
		),
		...optionalField(
			"unavailableReason",
			optionalString(record, "unavailableReason"),
		),
	};
}

function parseAgentRuntimeCapabilities(
	value: unknown,
): AgentRuntimeCapability[] {
	if (!Array.isArray(value))
		throw new Error("Expected array field: capabilities");
	return value.map((capability) => {
		if (!isAgentRuntimeCapability(capability))
			throw new Error("Invalid agent runtime capability");
		return capability;
	});
}

function isAgentRuntimeCapability(
	value: unknown,
): value is AgentRuntimeCapability {
	return typeof value === "string" && knownAgentRuntimeCapabilities.has(value);
}

function parseAgentRuntimeId(
	value: unknown,
	key: string,
): AgentRuntimeDescriptor["id"] {
	if (!isAgentRuntimeId(value))
		throw new Error(`Invalid agent runtime id field: ${key}`);
	return value;
}

function parseSessionRuntimeId(value: unknown): SessionInfo["runtimeId"] {
	// Rolling compatibility with pre-runtime-catalog session daemons. The only
	// runtime they can own is the embedded Pi runtime.
	return value === undefined
		? AGENT_RUNTIME_IDS.pi
		: parseAgentRuntimeId(value, "runtimeId");
}
...
export function parseSessionInfo(value: unknown): SessionInfo {
	...
	return {
		...
		runtimeId: parseSessionRuntimeId(record["runtimeId"]),
...
function parseSessionWarningSeverity(value: unknown): SessionWarningSeverity {
	if (value !== "info" && value !== "warning" && value !== "error")
		throw new Error("Invalid session warning severity");
	return value;
}

function parseSessionWarningDismiss(
	value: unknown,
): { id: string } | undefined {
	if (value === undefined) return undefined;
	const record = requireRecord(value);
...
function optionalWarnings(
	value: unknown,
): Pick<SessionStatus, "warnings"> | object {
...
export function parseSessionStatus(value: unknown): SessionStatus {
	...
	return {
		...
		...optionalField(
			"persisted",
			parseOptionalBoolean(record["persisted"], "persisted"),
		),
		...
		queuedMessages:
			record["queuedMessages"] === undefined
				? []
				: arrayOf(parseQueuedSessionMessage)(record["queuedMessages"]),
...
export function parseSessionStreamSnapshot(
	value: unknown,
...
export function parseSessionUnreadCatalogSnapshot(
	value: unknown,
): SessionUnreadCatalogSnapshot {
	const record = requireRecord(value);
	const catalogRevision = requireNonNegativeSafeInteger(
		record,
		"catalogRevision",
	);
	const sessions = boundedArrayOf(
		record["sessions"],
		parseSessionUnreadSummary,
		SESSION_UNREAD_LIMIT,
		"sessions",
	);
	...
	return {
		catalogId: requireBoundedNonEmptyString(
			record,
			"catalogId",
			SESSION_UNREAD_CATALOG_ID_MAX_LENGTH,
		),
...
export function parseSessionUnreadEvent(value: unknown): SessionUnreadEvent {
	const record = requireRecord(value);
	if (record["type"] !== "sessions.unread")
		throw new Error("Invalid session unread event type");
	const sessionId = requireBoundedNonEmptyString(
		record,
		"sessionId",
		SESSION_UNREAD_SESSION_ID_MAX_LENGTH,
	);
	const cwd = requireBoundedNonEmptyString(
		record,
		"cwd",
		SESSION_UNREAD_CWD_MAX_LENGTH,
	);
	...
	const unread =
		record["unread"] === null
			? null
			: parseSessionUnreadSummary(record["unread"]);
	if (
		unread !== null &&
		(unread.sessionId !== sessionId || unread.cwd !== cwd)
	) {
...
function parseSessionUnreadSummary(value: unknown): SessionUnreadSummary {
	...
	return {
		...
		cwd: requireBoundedNonEmptyString(
			record,
			"cwd",
			SESSION_UNREAD_CWD_MAX_LENGTH,
		),
...
function assertUniqueUnreadSummaries(
	summaries: readonly SessionUnreadSummary[],
): void {
	const identities = summaries.map((summary) =>
		JSON.stringify([summary.sessionId, summary.cwd]),
	);
	if (new Set(identities).size !== identities.length)
		throw new Error("Duplicate session unread identity");
	...
	if (new Set(completionOrders).size !== completionOrders.length)
		throw new Error("Duplicate session unread completion order");
}

function assertUnreadNewestFirst(
	summaries: readonly SessionUnreadSummary[],
): void {
	for (let index = 1; index < summaries.length; index += 1) {
		...
		if (
			previous === undefined ||
			current === undefined ||
			previous.completionOrder <= current.completionOrder
		) {
...
function requireBoundedNonEmptyString(
	...
	if (value.length > maxLength)
		throw new Error(`String field exceeds limit: ${key}`);
	return value;
}
...
function requirePositiveSafeInteger(
	...
	if (value === 0)
		throw new Error(`Expected positive safe integer field: ${key}`);
	return value;
}

export function parseSessionNotificationInboxSnapshot(
	value: unknown,
): SessionNotificationInboxSnapshot {
	const record = requireRecord(value);
	const summary = parseSessionNotificationSummary(record["summary"]);
	const notifications = boundedArrayOf(
	...
	if (summary.retainedCount !== notifications.length)
		throw new Error("Notification snapshot retained count mismatch");
	if (summary.highestSeverity !== highestNotificationSeverity(notifications))
		throw new Error("Notification snapshot severity mismatch");
	const dismissThrough = parseSessionNotificationDismissThrough(
		record["dismissThrough"],
	);
	const newestOrder = notifications[0]?.order ?? 0;
	if (dismissThrough.order !== newestOrder)
		throw new Error("Notification snapshot dismiss cutoff mismatch");
	if (dismissThrough.overflowWatermark < summary.discardedCount)
		throw new Error("Notification snapshot overflow cutoff mismatch");
	return {
...
export function parseSessionNotificationInboxEvent(
	value: unknown,
): SessionNotificationInboxEvent {
	const record = requireRecord(value);
	if (record["type"] !== "notifications.inbox")
		throw new Error("Invalid notification inbox event type");
	const summary = parseSessionNotificationSummary(record["summary"]);
	const dismissThrough = parseSessionNotificationDismissThrough(
		record["dismissThrough"],
	);
	if (dismissThrough.overflowWatermark < summary.discardedCount)
		throw new Error("Notification event overflow cutoff mismatch");
	const delta = parseSessionNotificationInboxDelta(record["delta"]);
	if (delta.kind === "cleared" && !notificationSummaryIsEmpty(summary))
		throw new Error("Notification clear event summary mismatch");
	if (delta.kind === "added" && summary.retainedCount === 0)
		throw new Error("Notification add event summary mismatch");
	return {
...
function parseSessionNotificationSummary(
	value: unknown,
): SessionNotificationSummary {
	const record = requireRecord(value);
	const retainedCount = requireNonNegativeSafeInteger(record, "retainedCount");
	if (retainedCount > SESSION_NOTIFICATION_LIMIT)
		throw new Error("Notification retained count exceeds limit");
	const discardedCount = requireNonNegativeSafeInteger(
		record,
		"discardedCount",
	);
	const highestSeverity = optionalSessionNotificationSeverity(
		record["highestSeverity"],
	);
	if ((retainedCount === 0) !== (highestSeverity === undefined))
		throw new Error("Notification summary severity mismatch");
	return {
...
function parseSessionNotification(value: unknown): SessionNotification {
	...
	if (
		...
		throw new Error("Notification message exceeds byte limit");
	const receivedAt = requireString(record, "receivedAt");
	if (!Number.isFinite(Date.parse(receivedAt)))
		throw new Error("Invalid notification receive time");
...
function parseSessionNotificationDismissThrough(
	value: unknown,
): SessionNotificationDismissThrough {
	const record = requireRecord(value);
	return {
		order: requireNonNegativeSafeInteger(record, "order"),
		overflowWatermark: requireNonNegativeSafeInteger(
			record,
			"overflowWatermark",
		),
	};
...
function parseSessionNotificationInboxDelta(
	value: unknown,
): SessionNotificationInboxDelta {
	const record = requireRecord(value);
	switch (record["kind"]) {
		case "added": {
			const evictedNotificationId = optionalString(
				record,
				"evictedNotificationId",
			);
			return {
				kind: "added",
				notification: parseSessionNotification(record["notification"]),
				...(evictedNotificationId === undefined
					? {}
					: { evictedNotificationId }),
			};
		}
		case "dismissed": {
			const notificationIds = boundedArrayOf(
			...
			if (new Set(notificationIds).size !== notificationIds.length)
				throw new Error("Duplicate dismissed notification id");
			return { kind: "dismissed", notificationIds };
		}
		case "cleared":
			return {
				kind: "cleared",
				reason: parseSessionNotificationClearReason(record["reason"]),
			};
		case "resync":
			return { kind: "resync" };
		default:
...
function parseSessionNotificationSeverity(
	value: unknown,
...
function optionalSessionNotificationSeverity(
	value: unknown,
): SessionNotificationSeverity | undefined {
	return value === undefined
		? undefined
		: parseSessionNotificationSeverity(value);
}

function parseSessionNotificationClearReason(
	value: unknown,
...
function boundedArrayOf<T>(
	...
	if (value.length > limit)
		throw new Error(`Array field exceeds limit: ${field}`);
	return value.map(parse);
}
...
function requireNonNegativeSafeInteger(
	...
	if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0)
		throw new Error(`Expected non-negative safe integer field: ${key}`);
	return value;
}

function assertUniqueNotifications(
	...
	if (
		new Set(notifications.map((notification) => notification.order)).size !==
		notifications.length
	)
		throw new Error("Duplicate notification order");
}
...
function assertNewestFirst(
	...
	for (let index = 1; index < notifications.length; index += 1) {
		...
		if (
			...
			throw new Error("Notifications are not newest-first");
	}
}

function notificationSummaryIsEmpty(
	summary: SessionNotificationSummary,
): boolean {
	return summary.retainedCount === 0 && summary.discardedCount === 0;
}

function highestNotificationSeverity(
	notifications: readonly SessionNotification[],
): SessionNotificationSeverity | undefined {
...
export function parseSessionCleanupPreviewResponse(
	value: unknown,
): SessionCleanupPreviewResponse {
	const record = requireRecord(value);
	const skippedBusySessionIds =
		record["skippedBusySessionIds"] === undefined
			? undefined
			: arrayOfString(record["skippedBusySessionIds"], "skippedBusySessionIds");
	return {
...
export function parseSessionCleanupExecuteResponse(
	value: unknown,
): SessionCleanupExecuteResponse {
	const record = requireRecord(value);
	return {
		...parseSessionCleanupPreviewResponse(record),
		archivedSessionIds: arrayOfString(
			record["archivedSessionIds"],
			"archivedSessionIds",
		),
		deletedSessionIds: arrayOfString(
			record["deletedSessionIds"],
			"deletedSessionIds",
		),
	};
}

export function parseSessionBulkArchiveResponse(
	value: unknown,
): SessionBulkArchiveResponse {
	const record = requireRecord(value);
	if (record["archived"] !== true)
		throw new Error("Expected bulk archived response");
	return {
		archived: true,
		archivedSessionIds: arrayOfString(
			record["archivedSessionIds"],
...
export function parseSessionBulkDeleteArchivedResponse(
	value: unknown,
): SessionBulkDeleteArchivedResponse {
	const record = requireRecord(value);
	if (record["deleted"] !== true)
		throw new Error("Expected bulk deleted response");
	return {
		deleted: true,
		deletedSessionIds: arrayOfString(
			record["deletedSessionIds"],
...
function parseSessionCleanupThresholds(
	value: unknown,
): SessionCleanupThresholds {
	const record = requireRecord(value);
	return {
		...optionalField(
			"archiveIdleDays",
			optionalNumber(record, "archiveIdleDays"),
		),
		...optionalField(
			"deleteArchivedDays",
			optionalNumber(record, "deleteArchivedDays"),
		),
	};
}

function parseSessionCleanupProjectSummary(
	value: unknown,
...
function parseSessionModel(value: unknown): SessionModel {
	const record = requireRecord(value);
	return {
		...optionalField("provider", optionalString(record, "provider")),
		...optionalField("id", optionalString(record, "id")),
...
export function parseModelSelectionResponse(
	value: unknown,
...
export function parseThinkingLevelsResponse(
	value: unknown,
...
function parseAuthStatusSource(value: unknown): AuthStatusSource {
	if (
		...
		throw new Error("Invalid auth status source");
	return value;
}
...
function parseAuthProviderStatus(value: unknown): AuthProviderStatus {
	...
	const source =
		record["source"] === undefined
			? undefined
			: parseAuthStatusSource(record["source"]);
	return {
...
function parseAuthProviderOption(value: unknown): AuthProviderOption {
	...
	if (loginFlow !== undefined && loginFlow !== "interactive")
		throw new Error("Invalid auth provider login flow");
	return {
		id: requireString(record, "id"),
		name: requireString(record, "name"),
...
export function parseAuthProvidersResponse(
	value: unknown,
	...
	const flow = {
		...
		progress: arrayOf((item) => {
			if (typeof item !== "string")
				throw new Error("Expected progress item string");
			return item;
		})(record["progress"]),
...
function parseOAuthFlowStatus(value: unknown): OAuthFlowState["status"] {
	if (
		...
		throw new Error("Invalid OAuth flow status");
	return value;
}
...
function optionalOAuthAuth(value: unknown): OAuthFlowState["auth"] | undefined {
	...
	return {
		...
		...optionalField(
			"deviceCode",
			optionalOAuthDeviceCode(record["deviceCode"]),
		),
	};
}

function optionalOAuthDeviceCode(
	value: unknown,
): NonNullable<OAuthFlowState["auth"]>["deviceCode"] | undefined {
	if (value === undefined) return undefined;
	const record = requireRecord(value);
	return {
		...
		...optionalField(
			"intervalSeconds",
			optionalNumber(record, "intervalSeconds"),
		),
		...optionalField(
			"expiresInSeconds",
			optionalNumber(record, "expiresInSeconds"),
		),
	};
}

function optionalOAuthPrompt(
	value: unknown,
): OAuthFlowState["prompt"] | undefined {
	if (value === undefined) return undefined;
	const record = requireRecord(value);
	const kind = requireString(record, "kind");
	if (kind !== "prompt" && kind !== "manual")
		throw new Error("Invalid OAuth prompt kind");
	const promptType =
		record["promptType"] === undefined
			? kind === "manual"
				? "manual_code"
				: "text"
			: parseOAuthPromptType(record["promptType"]);
	return {
...
function parseOAuthPromptType(
	value: unknown,
): "text" | "secret" | "manual_code" {
	if (value !== "text" && value !== "secret" && value !== "manual_code")
		throw new Error("Invalid OAuth prompt type");
	return value;
}

function optionalOAuthSelect(
	value: unknown,
): OAuthFlowState["select"] | undefined {
	if (value === undefined) return undefined;
	const record = requireRecord(value);
	return {
		requestId: requireString(record, "requestId"),
		message: requireString(record, "message"),
		options: arrayOf(parseCommandOption)(record["options"]),
	};
}
		...
		return {
			...
			...optionalField(
				...
				record["links"] === undefined
					? undefined
					: arrayOf(parseOAuthInfoLink)(record["links"]),
			),
		};
	})(value);
}

function parseOAuthInfoLink(
	value: unknown,
): NonNullable<NonNullable<OAuthFlowState["info"]>[number]["links"]>[number] {
	const record = requireRecord(value);
	return {
		url: requireString(record, "url"),
		...optionalField("label", optionalString(record, "label")),
...
function optionalContextUsage(
	value: unknown,
): Pick<SessionStatus, "contextUsage"> | object {
	if (value === undefined) return {};
	const record = requireRecord(value);
	return {
		contextUsage: {
			tokens: numberOrNull(record, "tokens"),
			contextWindow: requireNumber(record, "contextWindow"),
...
export function parseFileSuggestion(value: unknown): FileSuggestion {
	const record = requireRecord(value);
	const kind = requireString(record, "kind");
	if (kind !== "tracked" && kind !== "untracked" && kind !== "other")
...
export function parseFileTreeResponse(value: unknown): FileTreeResponse {
	...
	return {
		path: requireString(record, "path"),
		entries: arrayOf(parseFileTreeEntry)(record["entries"]),
		scannedAt: requireString(record, "scannedAt"),
		truncated: requireBoolean(record, "truncated"),
	};
}
...
function parseFileTreeEntry(value: unknown): FileTreeEntry {
	...
	if (type !== "file" && type !== "directory" && type !== "symlink")
		throw new Error("Invalid file tree entry type");
	return {
		name: requireString(record, "name"),
		path: requireString(record, "path"),
		type,
		...optionalField("size", optionalNumber(record, "size")),
...
export function parseFileContentResponse(value: unknown): FileContentResponse {
	...
	return {
		path: requireString(record, "path"),
		...optionalField("language", optionalString(record, "language")),
		...optionalField("mediaType", optionalFileMediaType(record["mediaType"])),
		...optionalField("mimeType", optionalString(record, "mimeType")),
		encoding,
		size: requireNumber(record, "size"),
		modifiedAt: requireString(record, "modifiedAt"),
		content: requireString(record, "content"),
		truncated: requireBoolean(record, "truncated"),
		binary: requireBoolean(record, "binary"),
	};
}

export function parseWriteWorkspaceFileResponse(
	value: unknown,
...
export function parseDeleteWorkspaceFileResponse(
	value: unknown,
...
export function parseMoveWorkspaceFileResponse(
	value: unknown,
...
function optionalFileMediaType(
	value: unknown,
): FileContentResponse["mediaType"] | undefined {
	if (value === undefined) return undefined;
...
export function parseGitStatusResponse(value: unknown): GitStatusResponse {
	...
	return {
		...
		...optionalField("branch", optionalString(record, "branch")),
		...optionalField("upstream", optionalString(record, "upstream")),
		...optionalField("ahead", optionalNumber(record, "ahead")),
		...optionalField("behind", optionalNumber(record, "behind")),
		files: arrayOf(parseGitStatusFile)(record["files"]),
		submodules:
			record["submodules"] === undefined
				? []
				: arrayOfString(record["submodules"], "submodules"),
	};
}
...
function parseGitStatusFile(value: unknown): GitStatusFile {
	...
	return {
		path: requireString(record, "path"),
		...optionalField("oldPath", optionalString(record, "oldPath")),
		index: parseGitFileState(record["index"]),
		workingTree: parseGitFileState(record["workingTree"]),
		...optionalField(
			"submoduleFromCommit",
			optionalString(record, "submoduleFromCommit"),
		),
		...optionalField(
			"submoduleToCommit",
			optionalString(record, "submoduleToCommit"),
		),
	};
}
...
export function parseTerminalInfo(value: unknown): TerminalInfo {
	...
	return {
		...
		name: requireString(record, "name"),
		createdAt: requireString(record, "createdAt"),
		exited: requireBoolean(record, "exited"),
		...optionalField("exitCode", optionalNumber(record, "exitCode")),
		...optionalField("commandRunId", optionalString(record, "commandRunId")),
	};
}
...
function parseTerminalCommandRunStatus(
	value: unknown,
	...
	return Object.fromEntries(
		Object.entries(record).map(([field, fieldValue]) => {
			if (typeof fieldValue !== "string")
				throw new Error(`Expected string record field: ${key}.${field}`);
...
export function parseWorkspaceActivityResponse(
	value: unknown,
): WorkspaceActivityResponse {
	const record = requireRecord(value);
	return {
		workspaces: arrayOf(parseWorkspaceActivity)(record["workspaces"]),
		generatedAt: requireString(record, "generatedAt"),
	};
}
...
export function parseOmpConfigResponse(value: unknown): OmpConfigResponse {
	const record = requireRecord(value);
	return {
		available: requireBoolean(record, "available"),
		...optionalField("command", optionalString(record, "command")),
		...optionalField("configPath", optionalString(record, "configPath")),
		...optionalField("error", optionalString(record, "error")),
		settings: arrayOf(parseOmpSettingDescriptor)(record["settings"]),
	};
}

function parseOmpSettingDescriptor(value: unknown): OmpSettingDescriptor {
	const record = requireRecord(value);
	const type: unknown = record["type"];
	if (!isOmpSettingType(type))
		throw new Error("Expected OMP setting type field: type");
	return {
		key: requireString(record, "key"),
		type,
		description: requireString(record, "description"),
		...optionalField("value", record["value"]),
		...optionalField("default", record["default"]),
		...optionalField(
			"enumValues",
			optionalStringArray(record["enumValues"], "enumValues"),
		),
		...optionalField("tab", optionalString(record, "tab")),
		...optionalField("group", optionalString(record, "group")),
	};
}
...
function parsePiWebConfigValues(value: unknown): PiWebConfigValues {
	...
	return {
		...
		...optionalField(
			"allowedHosts",
			optionalAllowedHosts(record["allowedHosts"]),
		),
		...
		...optionalField(
			"maxUploadBytes",
			optionalNumber(record, "maxUploadBytes"),
		),
...
function optionalAgent(value: unknown): PiWebConfigValues["agent"] | undefined {
	if (value === undefined) return undefined;
	if (!isRecord(value) || Array.isArray(value))
		throw new Error("Invalid PI WEB agent field");
	return {
...
function optionalAllowedHosts(
	value: unknown,
): PiWebConfigValues["allowedHosts"] | undefined {
	if (value === undefined) return undefined;
...
function optionalPathAccess(
	value: unknown,
): PiWebConfigValues["pathAccess"] | undefined {
	if (value === undefined) return undefined;
	...
	return {
		...optionalField(
			"allowedPaths",
			optionalStringArray(allowedPaths, "pathAccess.allowedPaths"),
		),
	};
}
...
function optionalUploads(
	value: unknown,
): PiWebConfigValues["uploads"] | undefined {
	if (value === undefined) return undefined;
	if (!isRecord(value) || Array.isArray(value))
		throw new Error("Invalid PI WEB uploads field");
	return {
...
function optionalShortcuts(value: unknown): PiWebShortcutConfig | undefined {
	if (value === undefined) return undefined;
	if (!isRecord(value) || Array.isArray(value))
		throw new Error("Invalid PI WEB shortcuts field");
	return Object.fromEntries(
		Object.entries(value).map(([actionId, shortcut]) => {
			if (
				shortcut !== null &&
				(typeof shortcut !== "string" || shortcut === "")
			)
				throw new Error("Invalid PI WEB shortcut field");
...
function optionalPlugins(value: unknown): PiWebPluginConfigMap | undefined {
	if (value === undefined) return undefined;
	if (!isRecord(value) || Array.isArray(value))
		throw new Error("Invalid PI WEB plugins field");
	return Object.fromEntries(
		Object.entries(value).map(([pluginId, config]) => {
			if (!isRecord(config) || Array.isArray(config))
				throw new Error("Invalid PI WEB plugin config field");
			const enabled = config["enabled"];
			if (enabled !== undefined && typeof enabled !== "boolean")
				throw new Error("Invalid PI WEB plugin enabled field");
			const settings = config["settings"];
			if (
				settings !== undefined &&
				(!isRecord(settings) || Array.isArray(settings))
			)
				throw new Error("Invalid PI WEB plugin settings field");
...
function parsePiWebConfigEnvOverrides(value: unknown): PiWebConfigEnvOverrides {
	...
	return {
		...
		// Optional so responses from older servers keep their original shape;
		// absent flags read as "not locked" at call sites (`=== true` checks).
		...optionalField("ompCommand", optionalBoolean(record, "ompCommand")),
		...optionalField("ompAgentDir", optionalBoolean(record, "ompAgentDir")),
		...optionalField("defaultRuntime", optionalBoolean(record, "defaultRuntime")),
	};
}

function optionalAgentDirSource(record: Record<string, unknown>): {
	agentDirSource?: PiWebAgentDirEnvSource;
} {
	const value = record["agentDirSource"];
	if (value === undefined) return {};
	if (value !== "pi-web" && value !== "pi-compatibility")
		throw new Error("Invalid PI WEB agentDirSource field");
...
export function parsePiPackageMutationResponse(
	value: unknown,
): PiPackageMutationResponse {
	const record = requireRecord(value);
	...
	const scope =
		record["scope"] === undefined
			? undefined
			: parsePiPackageScope(record["scope"]);
...
function parsePiPackageScope(value: unknown): PiPackageScope {
	if (value !== "user" && value !== "project")
		throw new Error("Invalid Pi package scope");
	return value;
}

function parsePiPackageMutationAction(value: unknown): PiPackageMutationAction {
	if (value !== "install" && value !== "remove" && value !== "update")
		throw new Error("Invalid Pi package mutation action");
	return value;
}

export function parsePiWebPluginsResponse(
	value: unknown,
...
function parsePiWebPluginInfo(value: unknown): PiWebPluginInfo {
	...
	return {
		...
		machineSpecific:
			parseOptionalBoolean(record["machineSpecific"], "machineSpecific") ??
			false,
...
function parsePiWebPluginScope(value: unknown): PiWebPluginScope {
	if (
		...
		throw new Error("Invalid PI WEB plugin scope");
	return value;
}
...
function parseOptionalBoolean(
	...
	if (value === undefined) return undefined;
	if (typeof value !== "boolean")
		throw new Error(`Expected optional boolean field: ${key}`);
	return value;
}
...
export function parsePiWebRuntimeResponse(
	value: unknown,
...
function parsePiWebComponents(
	value: unknown,
): PiWebStatusResponse["components"] {
	const record = requireRecord(value);
	return {
		web: parsePiWebComponentStatus(record["web"]),
		sessiond: parsePiWebComponentStatus(record["sessiond"]),
...
function parsePiWebRuntimeComponents(
	value: unknown,
): PiWebRuntimeResponse["components"] {
	const record = requireRecord(value);
	return {
		web: parsePiWebRuntimeComponent(record["web"]),
		sessiond: parsePiWebRuntimeComponent(record["sessiond"]),
	};
}
...
function parsePiWebRuntimeComponent(value: unknown): PiWebRuntimeComponent {
	...
	const activeAgentProfile =
		activeAgentProfileValue === undefined
			? undefined
			: parseActiveAgentProfileDescriptor(activeAgentProfileValue);
	if (
		activeAgentProfileValue !== undefined &&
		(component !== "sessiond" || activeAgentProfile === undefined)
	)
		throw new Error("Invalid active agent profile descriptor");
	return {
		component,
		label: requireString(record, "label"),
		...optionalField(
			"runtimeVersion",
			optionalString(record, "runtimeVersion"),
		),
		available: requireBoolean(record, "available"),
		capabilities: parsePiWebCapabilities(record["capabilities"]),
...
function parsePiWebComponentStatus(value: unknown): PiWebComponentStatus {
	...
	return {
		...
		...optionalField(
			"installedVersion",
			optionalString(record, "installedVersion"),
		),
		stale: requireBoolean(record, "stale"),
		available: requireBoolean(record, "available"),
		...optionalField(
			"installation",
			optionalPiWebInstallationInfo(record["installation"]),
		),
		...optionalField("error", optionalString(record, "error")),
	};
}

function optionalPiWebInstallationInfo(
	value: unknown,
): PiWebInstallationInfo | undefined {
	if (value === undefined) return undefined;
	const record = requireRecord(value);
	const kind = requireString(record, "kind");
	if (
		...
		throw new Error("Invalid PI WEB installation kind");
	const scope = record["scope"];
	if (scope !== undefined && scope !== "user" && scope !== "project")
		throw new Error("Invalid PI WEB installation scope");
	...
	if (
		...
		throw new Error("Invalid PI WEB Docker mode");
	return {
		kind,
		...optionalField("path", optionalString(record, "path")),
		...optionalField("source", optionalString(record, "source")),
...
function parsePiWebCommands(value: unknown): PiWebStatusResponse["commands"] {
	...
	return {
		...
		...optionalField(
			"restartSessiond",
			optionalString(record, "restartSessiond"),
		),
...
function parsePiWebServiceComponent(value: unknown): PiWebServiceComponent {
	if (value !== "web" && value !== "sessiond")
		throw new Error("Invalid PI WEB service component");
	return value;
}
...
function parsePiWebCapabilities(value: unknown): PiWebCapability[] {
	...
	if (capabilities === undefined)
		throw new Error("Invalid PI WEB capabilities");
	return capabilities;
}

function parsePiWebStatusSeverity(value: unknown): PiWebStatusSeverity {
	if (value !== "info" && value !== "warning" && value !== "error")
		throw new Error("Invalid PI WEB status severity");
	return value;
}
...
export function parseCommandResult(value: unknown): CommandResult {
	...
	const type = requireString(record, "type");
	if (type === "unsupported")
		return { type, message: requireString(record, "message") };
	if (type === "select")
		return {
			type,
			requestId: requireString(record, "requestId"),
	...
	if (type === "tree")
		return { type, tree: parseSessionTreeSnapshot(record["tree"]) };
	if (type === "done")
		return {
			type,
			...optionalField("message", optionalString(record, "message")),
			...optionalSession(record["session"]),
...
export function parseSessionTreeSnapshot(value: unknown): SessionTreeSnapshot {
	...
	if (nodeIds.size !== nodes.length)
		throw new Error("Duplicate session tree node id");
	const activeLeafId = requireNullableString(record, "activeLeafId");
	if (activeLeafId !== null && !nodeIds.has(activeLeafId))
		throw new Error("Invalid session tree activeLeafId");
	return {
		nodes,
		activeLeafId,
		activePathIds: arrayOfNonBlankString(
			record["activePathIds"],
			"activePathIds",
		),
	};
}
...
export function parseSessionTreeNavigateResult(
	value: unknown,
): SessionTreeNavigateResult {
	const record = requireRecord(value);
	const cancelled = requireBoolean(record, "cancelled");
	if (Object.hasOwn(record, "summaryEntry"))
		throw new Error(
			"Invalid session tree navigation result field: summaryEntry",
		);
	if (cancelled) {
		rejectResponseField(
			record,
			"editorText",
			"session tree cancellation result",
		);
		const aborted = record["aborted"];
		if (aborted !== undefined && typeof aborted !== "boolean")
			throw new Error("Expected optional boolean field: aborted");
		return { cancelled, ...(aborted === undefined ? {} : { aborted }) };
	}
	rejectResponseField(record, "aborted", "session tree navigation result");
	return {
		cancelled,
		...optionalField("editorText", optionalString(record, "editorText")),
	};
}

function rejectResponseField(
	record: Record<string, unknown>,
	field: string,
	label: string,
): void {
...
function requireNullableString(
	...
	if (typeof value === "string" && value.trim() === "")
		throw new Error(`Expected non-blank string or null field: ${key}`);
	return value;
}
...
function requireNonBlankString(
	...
	const value = requireString(record, key);
	if (value.trim() === "")
		throw new Error(`Expected non-blank string field: ${key}`);
	return value;
}
...
function arrayOfNonBlankString(value: unknown, key: string): string[] {
	...
	if (strings.some((item) => item.trim() === ""))
		throw new Error(`Expected non-blank string array field: ${key}`);
	return strings;
}
...
function optionalSession(
	value: unknown,
): Pick<Extract<CommandResult, { type: "done" }>, "session"> | object {
...
export function parseArchived(value: unknown): ArchiveSessionsResponse {
	const record = requireRecord(value);
	if (record["archived"] !== true)
		throw new Error("Expected archived response");
	const sessionIds =
		record["sessionIds"] === undefined
			? undefined
			: arrayOfString(record["sessionIds"], "sessionIds");
	const archivedCount = optionalNumber(record, "archivedCount");
	const skippedAlreadyArchivedCount = optionalNumber(
		record,
	...
	return {
		...
		...(skippedAlreadyArchivedCount === undefined
			? {}
			: { skippedAlreadyArchivedCount }),
	};
}
...
export function parseRestored(value: unknown): { restored: true } {
	...
	if (record["restored"] !== true)
		throw new Error("Expected restored response");
...
export function parseDetached(value: unknown): { detached: true } {
	...
	if (record["detached"] !== true)
		throw new Error("Expected detached response");
	return { detached: true };
}

export function parseReloaded(value: unknown): { reloaded: true } {
	const record = requireRecord(value);
	if (record["reloaded"] !== true)
		throw new Error("Expected reloaded response");
...
function optionalBoolean(
	...
	if (value === undefined) return undefined;
	if (typeof value !== "boolean")
		throw new Error(`Invalid PI WEB ${key} field`);
	return value;
}
...
function numberOrNull(
	...
	if (typeof value !== "number")
		throw new Error(`Expected number|null field: ${key}`);
	return value;
}
