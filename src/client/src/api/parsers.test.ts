...
import {
	...
	parseSessionInfo,
	parseSessionNotificationInboxEvent,
...
describe("API parsers", () => {
	it("preserves additive interactive API-key flow hints and defaults legacy options", () => {
		...
		expect(
			parseAuthProvidersResponse({
				providers: [{ ...base, loginFlow: "interactive" }, base],
			}).providers,
		).toEqual([{ ...base, loginFlow: "interactive" }, base]);
	});
		...
		expect(
			parseOAuthFlowState({
				...
				select: {
					...
					message: "Choose",
					options: [
						{ value: "work", label: "Work", description: "Company account" },
					],
				},
				...
				info: [
					{
						...
						links: [{ url: "https://example.test/docs", label: "Guide" }],
					},
				],
			}),
		).toMatchObject({
	...
	it("defaults semantic prompt types from legacy OAuth wire kinds", () => {
		...
		expect(
			parseOAuthFlowState({
				...flow,
				prompt: { requestId: "text", message: "Value", kind: "prompt" },
			}).prompt,
		).toMatchObject({
			kind: "prompt",
			promptType: "text",
		...
		expect(
			parseOAuthFlowState({
				...flow,
				prompt: { requestId: "manual", message: "Code", kind: "manual" },
			}).prompt,
		).toMatchObject({
			kind: "manual",
		...
		expect(
			parsePiWebConfigResponse({
				...
				config: {
					...
					plugins: { info: { enabled: false, settings: { compact: true } } },
				...
				effectiveConfig: {
					...
					agent: { command: "agent-lab", dir: "/Users/dev/agent-profiles/lab" },
				},
				envOverrides: {
					host: true,
					port: false,
	...
	it("parses PI WEB runtime responses including the daemon-owned active profile", () => {
		expect(
			...
			components: {
				sessiond: {
					activeAgentProfile: { command: "agent-lab", dir: "/srv/agent-lab" },
				},
			},
		});
	});
	...
	it("retains portable active profiles in machine runtime snapshots and rejects invalid ownership", () => {
		...
		expect(parsed.components?.sessiond.activeAgentProfile).toMatchObject({
			command: profile.command,
			dir: profile.dir,
		});
		expect(
			Object.isFrozen(parsed.components?.sessiond.activeAgentProfile),
		).toBe(true);
		expect(() =>
			parseMachineRuntime({
				machineId: "remote-a",
				...
				components: {
					...components,
					web: { ...components.web, activeAgentProfile: profile },
				},
				capabilities: [],
			}),
		...
		expect(() =>
			parseMachineRuntime({
				...
				components: {
					...
					sessiond: {
						...components.sessiond,
						activeAgentProfile: { ...profile, token: "secret" },
					},
				},
				capabilities: [],
			}),
		...
		const packages = [
			{
				...
				scope: "user",
				filtered: false,
				installedPath: "/home/test/.pi/packages/tools",
			},
	...
	it("rejects malformed Pi package responses", () => {
		expect(() =>
			parsePiPackagesResponse({
				packages: [
					{ source: "npm:@acme/tools", scope: "global", filtered: false },
				],
			}),
		).toThrow("Invalid Pi package scope");
		expect(() =>
			parsePiPackageMutationResponse({ action: "sync", packages: [] }),
		).toThrow("Invalid Pi package mutation action");
		expect(() =>
			parsePiPackagesResponse({
				packages: [
					{ source: "npm:@acme/tools", scope: "user", filtered: "no" },
				],
			}),
		).toThrow("Expected boolean field: filtered");
	});
	...
	it("parses Docker PI WEB installation metadata", () => {
		...
		expect(parsed.components.web.installation).toEqual({
			kind: "docker",
			path: "/srv/pi-web-docker",
			dockerMode: "runtime",
		});
		expect(parsed.components.sessiond.installation).toEqual({
			kind: "docker",
			dockerMode: "dev",
		});
		...
		expect(() =>
			parsePiWebStatusResponse({
				...
				components: {
					...response.components,
					web: {
						...response.components.web,
						installation: { kind: "docker", dockerMode: "hidden" },
					},
				},
			}),
	...
	it("accepts legacy array message pages and paged message responses", () => {
		expect(parseMessagePage(["a", "b"])).toEqual({
			messages: ["a", "b"],
			start: 0,
			total: 2,
		});
		expect(parseMessagePage({ messages: ["c"], start: 3, total: 9 })).toEqual({
			messages: ["c"],
			start: 3,
			total: 9,
		});
	...
	it("parses a session stream snapshot, defaulting a missing partial to null", () => {
		...
		expect(parseSessionStreamSnapshot({ seq: 0, partial: null })).toEqual({
			seq: 0,
			partial: null,
		});
		expect(parseSessionStreamSnapshot({ seq: 3 })).toEqual({
			seq: 3,
			partial: null,
		});
	});
	...
	it("rejects a session stream snapshot without a numeric seq", () => {
		expect(() => parseSessionStreamSnapshot({ partial: null })).toThrow(
			"Expected number field: seq",
		);
	});
	...
	it("rejects malformed, duplicate, unsorted, and mismatched unread payloads", () => {
		...
		).toThrow("Duplicate session unread identity");
		expect(() =>
			parseSessionUnreadCatalogSnapshot({
				catalogId: "catalog-a",
				catalogRevision: 2,
				sessions: [
					summary,
					{ ...summary, sessionId: "session-2", completionOrder: 2 },
				],
			}),
		...
		expect(() =>
			parseSessionUnreadCatalogSnapshot({
				catalogId: "catalog-a",
				catalogRevision: 1,
				sessions: [{ ...summary, completedAt: "never" }],
			}),
		).toThrow("Invalid canonical session unread completion time");
		expect(() =>
			parseSessionUnreadCatalogSnapshot({
				catalogId: "catalog-a",
				catalogRevision: 1,
				sessions: [{ ...summary, completedAt: "2026-07-20" }],
			}),
		).toThrow("Invalid canonical session unread completion time");
		expect(() =>
			parseSessionUnreadCatalogSnapshot({
	...
	it("rejects malformed session cleanup responses", () => {
		expect(() =>
			parseSessionCleanupPreviewResponse({
				generatedAt: "now",
				thresholds: {},
				projects: [{ cwd: "/repo", archiveCount: "2", deleteCount: 0 }],
				totals: { archiveCount: 2, deleteCount: 0 },
			}),
		).toThrow("Expected number field: archiveCount");
		expect(() =>
			parseSessionCleanupExecuteResponse({
				generatedAt: "now",
				thresholds: {},
		...
		).toThrow("Expected string array field: deletedSessionIds");
	});
	...
	it("rejects malformed bulk session mutation responses", () => {
		expect(() =>
			parseSessionBulkArchiveResponse({
				archived: true,
				archivedSessionIds: ["s1"],
				failures: [{ sessionId: "s2" }],
				generatedAt: "now",
			}),
		).toThrow("Expected string field: error");
		expect(() =>
			parseSessionBulkDeleteArchivedResponse({
				deleted: true,
				deletedSessionIds: [1],
	...
	it("parses an agent runtime catalog", () => {
		expect(
			parseAgentRuntimesResponse({
				defaultRuntimeId: "omp",
				runtimes: [
					{
						id: "pi",
						kind: "pi-embedded",
					...
					},
					{
						...
						available: true,
						command: "omp",
						profileDir: "/home/test/.omp/agent",
						protocolVersion: 2,
						capabilities: ["prompt", "subagents"],
					},
				],
			}),
		).toMatchObject({
			defaultRuntimeId: "omp",
			runtimes: [{ id: "pi" }, { id: "omp", protocolVersion: 2 }],
		});
		expect(() =>
			parseAgentRuntimesResponse({
				defaultRuntimeId: "omp",
	...
	it("parses session info including optional persistence signals", () => {
		...
		).toThrow("Expected optional boolean field: persisted");
	});
		...
		expect(
			parseSessionStatus({
				...
				queuedMessages: [
					...
					{ kind: "followUp", text: "then do that" },
				],
				messageCount: 7,
	...
	it("rejects invalid enum-like fields", () => {
		expect(() => parseSlashCommand({ name: "bad", source: "remote" })).toThrow(
			"Invalid command source",
		);
		expect(() => parseFileSuggestion({ path: "a", kind: "deleted" })).toThrow(
			"Invalid file kind",
		);
		expect(() =>
			parseGitStatusResponse({
				isGitRepo: true,
				hash: "h",
				files: [{ path: "a", index: "weird", workingTree: "modified" }],
			}),
			...
			files: [
				{
					...
					submoduleFromCommit: "1111111",
					submoduleToCommit: "2222222",
				},
	...
	it("validates file content responses", () => {
		...
		expect(parseFileContentResponse(textFile)).toMatchObject({
			path: "README.md",
			language: "markdown",
			content: "text",
		});
		...
		expect(() => parseFileContentResponse({ encoding: "base64" })).toThrow(
			"Invalid file encoding",
		);
		expect(() =>
			parseFileContentResponse({ ...textFile, mediaType: "video" }),
		).toThrow("Invalid file media type");
	});
	...
	it("parses command result variants", () => {
		...
		expect(
			parseCommandResult({ type: "unsupported", message: "nope" }),
		).toEqual({ type: "unsupported", message: "nope" });
		expect(
			parseCommandResult({
				type: "select",
				requestId: "r1",
				title: "Pick",
				options: [{ value: "v", label: "Label", description: "desc" }],
			}),
		).toEqual({
			type: "select",
			requestId: "r1",
		...
		expect(parseCommandResult({ type: "tree", tree })).toEqual({
			type: "tree",
			tree,
		});
		expect(
			parseCommandResult({
				type: "done",
				message: "ok",
				promptDraft: "resend me",
			}),
		).toEqual({ type: "done", message: "ok", promptDraft: "resend me" });
		expect(() => parseCommandResult({ type: "later" })).toThrow(
	...
	it("strictly parses session tree snapshots and navigation results", () => {
		...
		).toEqual({ cancelled: false, editorText: "edit this" });
		expect(parseSessionTreeNavigateResult({ cancelled: false })).toEqual({
			cancelled: false,
		});
		expect(
			parseSessionTreeNavigateResult({ cancelled: true, aborted: true }),
		).toEqual({ cancelled: true, aborted: true });
		expect(parseSessionTreeNavigateResult({ cancelled: true })).toEqual({
			cancelled: true,
		});
		expect(
			parseSessionTreeNavigateResult({
		...
		expect(() =>
			parseSessionTreeSnapshot({ ...tree, activeLeafId: undefined }),
		).toThrow("activeLeafId");
		expect(() =>
			parseSessionTreeSnapshot({ ...tree, activeLeafId: "missing" }),
		).toThrow("activeLeafId");
		expect(() =>
			parseSessionTreeSnapshot({ ...tree, activeLeafId: "   " }),
		).toThrow("activeLeafId");
		expect(() =>
			parseSessionTreeSnapshot({ ...tree, activePathIds: ["root", 2] }),
		).toThrow("activePathIds");
		expect(() =>
			parseSessionTreeSnapshot({ ...tree, activePathIds: ["   "] }),
		).toThrow("activePathIds");
		expect(() =>
			parseSessionTreeSnapshot({
				...tree,
				nodes: [{ ...tree.nodes[0], id: "   " }],
			}),
		).toThrow("id");
		expect(() =>
			parseSessionTreeSnapshot({
				...tree,
				nodes: [{ ...tree.nodes[0], parentId: undefined }],
			}),
		).toThrow("parentId");
		expect(() =>
			parseSessionTreeSnapshot({
				...tree,
				nodes: [{ ...tree.nodes[0], parentId: "   " }],
			}),
		).toThrow("parentId");
		expect(() =>
			parseSessionTreeSnapshot({
				...tree,
				nodes: [tree.nodes[0], tree.nodes[0]],
			}),
		).toThrow("Duplicate session tree node id");
		expect(() =>
			parseSessionTreeSnapshot({
				...tree,
				nodes: [{ ...tree.nodes[0], kind: "future-kind" }],
			}),
		).toThrow("Invalid session tree node kind");
		expect(() =>
			parseSessionTreeNavigateResult({
				cancelled: true,
				editorText: "wrong branch",
			}),
		).toThrow("editorText");
		expect(() =>
			parseSessionTreeNavigateResult({ cancelled: false, aborted: true }),
		).toThrow("aborted");
		expect(() =>
			parseSessionTreeNavigateResult({ cancelled: false, editorText: 42 }),
		).toThrow("editorText");
		expect(() =>
			parseSessionTreeNavigateResult({ cancelled: true, aborted: "yes" }),
		).toThrow("aborted");
		expect(() =>
			parseSessionTreeNavigateResult({
				cancelled: false,
				summaryEntry: { raw: true },
			}),
		).toThrow("summaryEntry");
		expect(() =>
			parseSessionTreeNavigateResult({
				cancelled: true,
				summaryEntry: { raw: true },
		...
		expect(() =>
			parseSessionTreeNavigateResult({ editorText: "missing discriminator" }),
		).toThrow("cancelled");
	});
	...
	it("strictly parses selected notification snapshots and realtime events", () => {
		...
		expect(
			...
			type: "notifications.inbox",
			delta: { kind: "added", notification: { severity: "warning" } },
		});
	});
	...
	it("rejects malformed, unsafe, over-cap, and oversized notification payloads", () => {
		...
		expect(() =>
			parseSessionNotificationInboxSnapshot({
				...
				summary: {
					...inbox.summary,
					retainedCount: SESSION_NOTIFICATION_LIMIT,
				},
				notifications: Array.from(
					{ length: SESSION_NOTIFICATION_LIMIT + 1 },
					(_, index) =>
						notificationWire(SESSION_NOTIFICATION_LIMIT + 1 - index),
				),
			}),
		...
		expect(() =>
			parseSessionNotificationInboxSnapshot({
				...
				notifications: [
					{
						...notificationWire(1),
						message: "x".repeat(SESSION_NOTIFICATION_MESSAGE_BYTES + 1),
					},
				],
			}),
...
function sessionTreeWire() {
	...
	const nodes = kinds.map((kind, index) => ({
		...
		...(index === 0
			? { timestamp: "2026-07-20T00:00:00.000Z", label: "root label" }
			: {}),
	}));
	return {
		nodes,
