...
describe("machine-scoped runtime API", () => {
	it("reads machine PI WEB status through the gateway route", async () => {
		...
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/pi-web/status",
		);
	});
	...
	it("requests an uncached update check through the local status route", async () => {
		...
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/pi-web/status?refresh=1",
		);
	...
	it("requests an uncached update check through the selected machine route", async () => {
		...
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/pi-web/status?refresh=1",
		);
		expect(fetchCall(fetchMock, 0)[1]?.cache).toBe("no-store");
	});
	...
	it("reads machine runtime through the gateway route", async () => {
		...
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/runtime",
		);
		expect(fetchCall(fetchMock, 1)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/runtime?refresh=1",
		);
...
describe("settings config and plugin APIs", () => {
	it("preserves gateway config and plugin routes by default", async () => {
		...
		await expect(configApi.config()).resolves.toMatchObject({
			config: { host: "127.0.0.1" },
		});
		await expect(
			configApi.saveConfig({ spawnSessions: true }),
		).resolves.toMatchObject({ config: { spawnSessions: true } });
	...
	it("uses machine-scoped config and plugin routes when a machine id is provided", async () => {
		...
		await expect(configApi.config("remote a")).resolves.toMatchObject({
			config: { spawnSessions: false },
		});
		await expect(
			configApi.saveConfig({ spawnSessions: true }, "remote a"),
		).resolves.toMatchObject({ config: { spawnSessions: true } });
		await expect(pluginsApi.plugins("remote a")).resolves.toEqual(
...
describe("Pi package API", () => {
	it("preserves the legacy local Pi package-management routes by default", async () => {
		...
		expect(JSON.parse(requestBody(fetchCall(fetchMock, 3)[1]))).toEqual({
			source: "npm:@acme/tools",
		});
	...
	it("uses machine-scoped Pi package-management routes when a machine id is provided", async () => {
		...
		await expect(piPackagesApi.packages("local")).resolves.toEqual({
			packages,
		});
		await expect(piPackagesApi.packages("remote a")).resolves.toEqual({
			packages,
		});
...
describe("session API compatibility", () => {
	it("reads and acknowledges daemon-owned unread state through encoded machine routes", async () => {
		...
		await expect(sessionsApi.unreadCatalog("remote a")).resolves.toEqual(
			unread,
		);
		await expect(
			sessionsApi.acknowledgeUnread(
				{ id: "s /?", cwd: "/repo" },
				"catalog-a",
				1,
				"remote a",
		...
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/unread",
		);
		expect(fetchCall(fetchMock, 0)[1]?.cache).toBe("no-store");
		expect(fetchCall(fetchMock, 1)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/s%20%2F%3F/unread/acknowledge",
		);
		expect(fetchCall(fetchMock, 1)[1]?.method).toBe("POST");
		expect(JSON.parse(requestBody(fetchCall(fetchMock, 1)[1]))).toEqual({
			cwd: "/repo",
	...
	it("posts session cleanup preview and execute requests through the selected machine", async () => {
		...
		await expect(
			sessionsApi.cleanupPreview(
				{ archiveIdleDays: 7, deleteArchivedDays: null },
				"remote a",
			),
		).resolves.toEqual(preview);
		await expect(
			sessionsApi.cleanup(
				{ archiveIdleDays: 7, projectCwds: ["/repo"] },
				"remote a",
			),
		...
		expect(fetchMock).toHaveBeenCalledTimes(2);
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/cleanup/preview",
		);
		...
		expect(fetchCall(fetchMock, 1)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/cleanup",
		);
		expect(fetchCall(fetchMock, 1)[1]?.method).toBe("POST");
		expect(JSON.parse(requestBody(fetchCall(fetchMock, 1)[1]))).toEqual({
			archiveIdleDays: 7,
	...
	it("posts bulk session mutation requests through the selected machine", async () => {
		const archived = {
			...
			failures: [{ sessionId: "s 2", error: "busy" }],
			generatedAt: "now",
		};
		const deleted = {
			deleted: true,
		...
		await expect(
			sessionsApi.archiveMany([{ id: "s 1", cwd: "/repo" }, "s 2"], "remote a"),
		).resolves.toEqual(archived);
		await expect(
			sessionsApi.deleteArchivedMany([{ id: "s 1", cwd: "/repo" }], "remote a"),
		).resolves.toEqual(deleted);

		expect(fetchMock).toHaveBeenCalledTimes(2);
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/bulk/archive",
		);
		expect(fetchCall(fetchMock, 0)[1]?.method).toBe("POST");
		expect(JSON.parse(requestBody(fetchCall(fetchMock, 0)[1]))).toEqual({
			sessions: [{ id: "s 1", cwd: "/repo" }, { id: "s 2" }],
		});
		expect(fetchCall(fetchMock, 1)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/bulk/delete-archived",
		);
		expect(fetchCall(fetchMock, 1)[1]?.method).toBe("POST");
		expect(JSON.parse(requestBody(fetchCall(fetchMock, 1)[1]))).toEqual({
			sessions: [{ id: "s 1", cwd: "/repo" }],
		});
	});
	...
	it("keeps legacy session-id calls free of cwd context", async () => {
		...
		expect(url).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/s%201/prompt",
		);
	...
	it("clears a session queue through an encoded machine route and parses the returned status", async () => {
		...
		await expect(
			sessionsApi.clearQueue(
				{ id: "s /?", cwd: "/repo with spaces" },
				"remote /?",
			),
		).resolves.toEqual({
			sessionId: "s /?",
		...
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/remote%20%2F%3F/sessions/s%20%2F%3F/queue/clear",
		);
	...
	it("posts session tree navigation through an encoded cwd-scoped machine route", async () => {
		...
		const navigation = {
			...
			summary: { mode: "custom" as const, instructions: "focus on tests" },
		};

		await expect(
			sessionsApi.navigateTree(
				{ id: "s /?", cwd: "/repo with spaces" },
				navigation,
				"remote /?",
			),
		).resolves.toEqual({ cancelled: false, editorText: "edit this" });

		expect(fetchMock).toHaveBeenCalledOnce();
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/remote%20%2F%3F/sessions/s%20%2F%3F/tree/navigate",
		);
		expect(init?.method).toBe("POST");
		expect(JSON.parse(requestBody(init))).toEqual({
	...
	it("keeps session tree navigation under a canonical nested deployment base", async () => {
		...
		vi.stubGlobal("document", {
			baseURI: "https://pi.example.test/nested/pi-web/",
		});
		...
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/nested/pi-web/api/machines/remote%20%2F%3F/sessions/session%20%2F%3F/tree/navigate",
		);
	});
	...
	it("reads a session stream snapshot through an encoded machine route with cwd context", async () => {
		...
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/remote%20%2F%3F/sessions/s%20%2F%3F/stream-snapshot?cwd=%2Frepo+with+spaces",
		);
	...
	it("reads a session stream snapshot for a legacy session-id ref without cwd context", async () => {
		...
		await expect(
			sessionsApi.streamSnapshot("s 1", "remote a"),
		).resolves.toEqual({ seq: 0, partial: null });

		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/sessions/s%201/stream-snapshot",
		);
	});
	...
	it("uses encoded selected-session notification routes, cwd queries, and authoritative mutation cutoffs", async () => {
		...
		await sessionsApi.dismissNotification(
			ref,
			"daemon-a",
			"opaque/id?",
			"remote /?",
		);
		await sessionsApi.dismissAllNotifications(
			ref,
			"daemon-a",
			{ order: 1, overflowWatermark: 7 },
			"remote /?",
		);
...
describe("machine-scoped file suggestion API", () => {
	it("uses the workspace-scoped route when the caller has enabled workspace-scoped suggestions", async () => {
		...
		await filesApi.files("/repo", "README", {
			projectId: "p 1",
			workspaceId: "w/1",
			scope: "tracked",
			machineId: "remote a",
		...
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/projects/p%201/workspaces/w%2F1/files?q=README&scope=tracked",
		);
	});
	...
	it("falls back to the legacy cwd route when workspace-scoped suggestions are not enabled", async () => {
		...
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/files?q=README&scope=tracked&cwd=%2Frepo",
		);
	});
});
...
describe("machine-scoped workspace API", () => {
	it("keeps project ids in one encoded route segment when listing workspaces", async () => {
		...
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote%20a/projects/..%2Fp%20%2F%3F/workspaces",
		);
	});
});
...
describe("machine-scoped terminal command-run API", () => {
	it("deletes workspaces through the selected machine scope", async () => {
		...
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/remote%20a/projects/p%201/workspaces/w%2F1",
		);
	...
	it("creates command runs through the selected machine scope", async () => {
		...
		await terminalsApi.runTerminalCommand(
			"core",
			{ workspace, title: "Build", command: "npm test", open: true },
			"remote a",
		);
		...
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/remote%20a/projects/p%201/workspaces/w%2F1/terminal-command-runs",
		);
		expect(init?.method).toBe("POST");
		expect(JSON.parse(requestBody(init))).toEqual({
			origin: "core",
	...
	it("closes all workspace terminals through the selected machine scope", async () => {
		...
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/remote%20a/projects/p%201/workspaces/w%2F1/terminals",
		);
		expect(init?.method).toBe("DELETE");
	});
	...
	it("lists, reads, and cancels command runs through the selected machine scope", async () => {
		...
		await terminalsApi.listCommandRuns(
			{
				projectId: "p 1",
				workspaceId: "w/1",
				statuses: ["running"],
	...
	it("returns undefined for missing command runs in the selected machine scope", async () => {
		...
		await expect(
			terminalsApi.getCommandRun("missing", "remote-a"),
		).resolves.toBeUndefined();

		expect(fetchCall(fetchMock, 0)[0]).toBe(
			"https://pi.example.test/api/machines/remote-a/terminal-command-runs/missing",
		);
	});
});
...
describe("workspace file write API", () => {
	it("sends text content with Content-Type text/plain", async () => {
		...
		await workspacesApi.writeWorkspaceFile(
			"p 1",
			"w/1",
			"hello.txt",
			"hello world",
		...
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/local/projects/p%201/workspaces/w%2F1/file?path=hello.txt",
		);
	...
	it("sends binary content with Content-Type application/octet-stream", async () => {
		...
		const [url, init] = fetchCall(fetchMock, 0);
		expect(url).toBe(
			"https://pi.example.test/api/machines/local/projects/p%201/workspaces/w%2F1/file?path=image.png",
		);
		expect(init?.method).toBe("PUT");
	...
	it("sends createDirs and overwrite query parameters", async () => {
		...
		await workspacesApi.writeWorkspaceFile(
			...
			"config/new.json",
			'{"a":1}',
			{ createDirs: false, overwrite: false },
		);
...
function piWebPluginsResponse() {
	return {
		plugins: [
			{
				id: "info",
				module: "/pi-web-plugins/info/plugin.js",
				source: "test",
				scope: "local",
