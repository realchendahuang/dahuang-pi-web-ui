...
export type ToolCardKind = "read" | "bash" | "edit" | "write" | "search" | "generic";
...
const SEARCH_TOOLS = new Set(["grep", "find", "ls", "search", "glob", "ast_grep_search"]);

export function toolCardViewModel(execution: ToolExecutionPart): ToolCardViewModel {
  ...
  const query = getString(execution.args, "pattern") ?? getString(execution.args, "query");
  const diff = toolCardDiff(execution);
  const error = status === "failed" ? firstNonEmpty(execution.resultText, execution.preview?.error) : undefined;
  ...
  return {
    ...
    ...(execution.resultText === undefined ? {} : { resultText: execution.resultText }),
...
export function toolCardStatus(status: ToolExecutionPart["status"]): ToolCardStatus {
...
function toolCardVerb(kind: ToolCardKind, status: ToolCardStatus): string {
  ...
  switch (kind) {
    ...
    case "generic": return done ? "Used" : "Using";
  }
}
...
function toolCardTitle(
  ...
  if (kind === "bash" && command !== undefined && command !== "") return `${verb} ${firstLine(command)}`;
  if ((kind === "read" || kind === "edit" || kind === "write") && filePath !== undefined && filePath !== "") return `${verb} ${filePath}`;
  if (kind === "search" && query !== undefined && query !== "") return `${verb} for “${query}”`;
  if (execution.summary !== "") return `${verb} ${execution.summary}`;
  return `${verb} ${execution.toolName}`;
}

export function toolCardDiff(execution: ToolExecutionPart): ToolCardDiff | undefined {
  ...
  return { additions: added, deletions: removed, content, isPreview: applied === undefined };
}
...
function rawDetailsOf(execution: ToolExecutionPart): { rawDetails: string } | undefined {
  const source = execution.details ?? execution.content;
  if (source === undefined || source === null) return undefined;
  if (typeof source === "string") return source === "" ? undefined : { rawDetails: source };
  try {
