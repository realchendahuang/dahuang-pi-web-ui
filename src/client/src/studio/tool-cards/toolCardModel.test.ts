...
describe("toolCardViewModel", () => {
	it("builds a read card with the file path in the title", () => {
		const model = toolCardViewModel(
			execution({ args: { path: "src/app.ts" } }),
		);
	...
	it("builds a search card quoting the pattern", () => {
		const model = toolCardViewModel(
			execution({ toolName: "grep", args: { pattern: "AgentSession" } }),
		);
	...
	it("falls back to the wire summary for generic tools", () => {
		const model = toolCardViewModel(
			execution({ toolName: "todo", summary: "3 tasks updated" }),
		);
	...
	it("uses present-tense verbs while running", () => {
		expect(
			toolCardViewModel(
				execution({ status: "running", args: { path: "a.ts" } }),
			).verb,
		).toBe("Reading");
		expect(
			toolCardViewModel(
