...
describe("cached new sessions", () => {
	...
	it("merges cached sessions for the selected cwd without duplicating server sessions", () => {
		...
		const mergedWithServerSession = mergeCachedNewSessions(
			"/repo",
			[baseSession],
			"local",
			storage,
		...
		expect(mergedWithServerSession.map((session) => session.id)).toEqual([
			"session-1",
		]);
		expect(isCachedNewSessionInfo(mergedWithServerSession[0])).toBe(false);
		expect(loadCachedNewSessions(storage).map((session) => session.id)).toEqual(
			["other"],
		);
	});
	...
	it("keeps browser-cached sessions scoped by machine", () => {
		...
		expect(
			mergeCachedNewSessions("/repo", [], "local", storage).map(
				(session) => session.id,
			),
		).toEqual(["session-1"]);
		expect(
			mergeCachedNewSessions("/repo", [], "remote", storage).map(
				(session) => session.id,
			),
		).toEqual(["session-2"]);
	});
