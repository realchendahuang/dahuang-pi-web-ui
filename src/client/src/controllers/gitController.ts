...
export class GitController {
	...
	async refreshGit(): Promise<void> {
		...
		try {
			...
			if (selectedDiffPath !== undefined) {
				if (status.files.some((file) => file.path === selectedDiffPath))
	...
	async selectDiff(path: string): Promise<void> {
		...
		const tool =
			currentTool === "core:workspace.git" ||
			currentTool === "core:workspace.changes"
				? currentTool
				: "core:workspace.git";
		this.setState({
			...
			selectedStagedDiff: undefined,
			workspaceTool: tool,
			mainView: this.getState().mainView === "chat" ? "chat" : tool,
		});
	...
	updatePolling(): void {
		...
		if (
			state.workspaceTool === "core:workspace.git" ||
			state.mainView === "core:workspace.git"
		) {
