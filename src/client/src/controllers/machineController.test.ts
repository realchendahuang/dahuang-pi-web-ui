...
describe("MachineController", () => {
	...
	it("selects a newly added machine and clears stale workspace state", async () => {
		...
		let state: AppState = {
			...
			gitStatus: {
				...
				branch: "main",
				files: [
					{ path: "src/index.ts", index: "modified", workingTree: "modified" },
				],
				submodules: [],
			},
		...
		const addMachine = vi
			.spyOn(api, "addMachine")
			.mockResolvedValue(addedMachine);
		const health = vi.spyOn(api, "health").mockResolvedValue({
			machineId: addedMachine.id,
			ok: true,
			checkedAt: "2026-05-27T00:00:01.000Z",
			status: "online",
		});
		const runtime = vi.spyOn(api, "runtime").mockResolvedValue({
			machineId: addedMachine.id,
			ok: true,
			checkedAt: "2026-05-27T00:00:02.000Z",
		});
	...
	it("records offline health without falling back when the routed remote health request rejects", async () => {
		...
		expect(state.selectedMachine).toEqual(remoteMachine);
		expect(state.machineStatuses[remoteMachine.id]).toMatchObject({
			machineId: remoteMachine.id,
			ok: false,
			status: "offline",
	...
	it("does not let an older runtime response overwrite a newer capability negotiation", async () => {
		...
		let resolveOlder:
			| ((runtime: Awaited<ReturnType<typeof api.runtime>>) => void)
			| undefined;
		const older = new Promise<Awaited<ReturnType<typeof api.runtime>>>(
			(resolve) => {
		...
		const controller = new MachineController(() => state, setState, vi.fn(), {
			loadProjects: vi.fn(),
		});
		...
		resolveOlder?.({
			...
			ok: true,
			checkedAt: "old",
			capabilities: ["sessions.unread"],
		});
		await first;

		expect(state.machineRuntimes["local"]).toMatchObject({
			checkedAt: "new",
			capabilities: [],
		});
	});

	it("loads the agent runtime catalog and only selects available runtimes", async () => {
		let state: AppState = {
			...initialAppState(),
			machines: [localMachine],
			selectedMachine: localMachine,
		...
		vi.spyOn(api, "runtimes").mockResolvedValue({
		...
		await controller.refreshAgentRuntimes("local");
		expect(
			state.agentRuntimeCatalogs["local"]?.runtimes.map(
				(runtime) => runtime.id,
			),
		).toEqual(["pi", "omp"]);
		expect(state.selectedAgentRuntimeIds["local"]).toBe("omp");

		controller.selectAgentRuntime("pi", "local");
		expect(state.selectedAgentRuntimeIds["local"]).toBe("omp");
	});
