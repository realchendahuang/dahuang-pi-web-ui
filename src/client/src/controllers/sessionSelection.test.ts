...
import {
	...
	SessionStorageSessionSelectionMemory,
...
describe("selectPreferredSession", () => {
	it("prefers an explicit target session by id", () => {
		...
		expect(
			selectPreferredSession(sessions, {
				targetSessionId: "s2",
				latestSessionId: "s1",
			})?.id,
		).toBe("s2");
	...
	it("matches explicit target sessions by id prefix", () => {
		...
		expect(selectPreferredSession([session], { targetSessionId: "abc" })).toBe(
			session,
		);
	});
	...
	it("remembers the latest selected session when no explicit target is provided", () => {
		...
		expect(
			selectPreferredSession(sessions, { latestSessionId: "s2" })?.id,
		).toBe("s2");
	});
	...
	it("returns a remembered archived session before falling back to active sessions", () => {
		const sessions = [
			{ ...testSession("s1"), archived: true },
			testSession("s2"),
		];

		expect(
			selectPreferredSession(sessions, { latestSessionId: "s1" })?.id,
		).toBe("s1");
	});
	...
	it("falls back to the first active session when the remembered session no longer exists", () => {
		...
		expect(
			selectPreferredSession(sessions, { latestSessionId: "old" })?.id,
		).toBe("s2");
	});
...
describe("SessionStorageSessionSelectionMemory", () => {
	it("persists the latest selected session per workspace cwd", () => {
		...
		expect(
			new SessionStorageSessionSelectionMemory(storage).latestSessionId(
				"local:/tmp/one",
			),
		).toBeUndefined();
		expect(
...
describe("markSessionArchived", () => {
	it("marks the matching session archived without mutating the original", () => {
		...
		expect(next).toEqual([
			{ ...sessions[0], archived: true, archivedAt: "later" },
			sessions[1],
		]);
	...
	it("marks multiple matching sessions archived", () => {
		...
		expect(next).toEqual([
			{ ...sessions[0], archived: true, archivedAt: "later" },
			sessions[1],
			{ ...sessions[2], archived: true, archivedAt: "later" },
		]);
	});
});
...
describe("shouldDeselectAfterArchivedCollapse", () => {
	it("deselects archived selections only when no active sessions remain", () => {
		...
		expect(shouldDeselectAfterArchivedCollapse([archived], archived)).toBe(
			true,
		);
		expect(
			shouldDeselectAfterArchivedCollapse([archived, active], archived),
		).toBe(false);
		expect(shouldDeselectAfterArchivedCollapse([archived], undefined)).toBe(
			false,
		);
...
describe("selectionAfterArchivingSession", () => {
	...
	it("selects the first active session when archiving the selected session", () => {
		...
		expect(
			selectionAfterArchivingSession([testSession("s1"), s2], "s1", "s1"),
		).toEqual({ type: "select", session: s2 });
	});
	...
	it("skips archived sessions when choosing the next selected session", () => {
		...
		expect(
			selectionAfterArchivingSession(
				[testSession("s1"), { ...testSession("s2"), archived: true }, s3],
				"s1",
				"s1",
			),
		).toEqual({ type: "select", session: s3 });
	});

	it("clears selection when no active session remains", () => {
		expect(
			selectionAfterArchivingSession([testSession("s1")], "s1", "s1"),
		).toEqual({ type: "clear" });
	});
