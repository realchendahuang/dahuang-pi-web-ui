...
describe("InMemoryMachineNavigationMemory", () => {
	it("remembers independent navigation snapshots per machine", () => {
		...
		memory.remember({
			...
			surface: { selectedFilePath: "README.md" },
		});
		memory.remember({
			machineId: "remote",
	...
	it("returns cloned snapshots so callers cannot mutate memory", () => {
		...
		if (snapshot !== undefined)
			snapshot.surface.selectedFilePath = "changed.ts";
...
describe("SessionStorageMachineNavigationMemory", () => {
	it("persists independent navigation snapshots in per-tab storage", () => {
		...
		expect(
			new SessionStorageMachineNavigationMemory(storage).latest("local"),
		).toBeUndefined();
		expect(
			new SessionStorageMachineNavigationMemory(storage).latest("remote")
				?.projectId,
		).toBe("remote-project");
	});
	...
	it("ignores malformed stored snapshots", () => {
		const storage = memoryStorage({
			"pi-web:machine-navigation:v1": JSON.stringify({
				version: 1,
				entries: [
					[
						"local",
					...
					["remote", { projectId: "missing-machine", surface: {} }],
				],
			}),
		});
...
describe("routeFromMachineNavigationSnapshot", () => {
	...
	it("creates an empty machine-only snapshot", () => {
		expect(emptyMachineNavigationSnapshot("remote")).toEqual({
			machineId: "remote",
			surface: {},
		});
	});
