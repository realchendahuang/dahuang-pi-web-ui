...
beforeEach(() => {
	framesById.clear();
	nextFrameId = 1;
	// The chat history cache (and a few selection memories) back onto the
	// vitest-provided `sessionStorage` polyfill, which is shared across tests in
	// the process. Clear it so one test's written transcript does not leak into
	// another's fresh controller.
	try {
