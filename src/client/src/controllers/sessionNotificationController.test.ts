...
import {
	...
	type SessionNotificationApi,
} from "./sessionNotificationController";
...
function inboxSnapshot(
	...
	const highestSeverity = notifications.some(
		(notification) => notification.severity === "error",
	)
		? "error"
		: notifications.some((notification) => notification.severity === "warning")
			? "warning"
	...
	return {
		...
		dismissThrough: {
			...
			overflowWatermark: options.discardedCount ?? 0,
		},
	};
}
...
function createHarness(
	...
	const api: SessionNotificationApi = {
		...
		dismissNotification: vi.fn(() =>
			Promise.resolve(
				inboxSnapshot([], { inboxRevision: 2, catalogRevision: 2 }),
			),
		),
...
describe("SessionNotificationController selected inbox ownership", () => {
	it("makes no notification request and preserves marked legacy output without effective capability support", async () => {
		...
		expect(
			harness.controller.shouldFilterLegacyNotification(
				"local",
				"notification-1",
			),
		).toBe(false);
	...
	it("treats a validated selected-inbox event as support during a rolling capability transition", async () => {
		...
		expect(
			selectedNotificationView(harness.state.selectedNotificationInbox)
				?.notifications,
		).toHaveLength(1);
		expect(
			harness.controller.shouldFilterLegacyNotification("local", "daemon-a:1"),
		).toBe(true);
	});
	...
	it("joins selected live events that arrive while the selected snapshot is loading", async () => {
		...
		const harness = createHarness(capableState(), {
			notificationInbox: vi.fn(() => pendingInbox.promise),
		});

		harness.controller.prepareSelectedSession(session, "local");
		const refresh = harness.controller.refreshSelectedSession(session, "local");
		harness.controller.applyInboxEvent(
			"local",
			addedEvent(entry(2, "warning"), 2, 2),
		);
		pendingInbox.resolve(
			inboxSnapshot([entry(1)], { inboxRevision: 1, catalogRevision: 1 }),
		);
		await refresh;

		expect(
			selectedNotificationView(
				harness.state.selectedNotificationInbox,
			)?.notifications.map((notification) => notification.id),
		).toEqual(["daemon-a:2", "daemon-a:1"]);
		expect(
			selectedNotificationView(harness.state.selectedNotificationInbox)
				?.announcements,
		).toMatchObject([{ severity: "warning", message: "notice 2" }]);
	});
	...
	it("ignores notification events for an unselected chat", async () => {
		...
		harness.controller.applyInboxEvent("local", {
			...
			summary: {
				...addedEvent(entry(2), 2, 1).summary,
				sessionId: "session-2",
			},
		});
	...
	it("recovers a selected inbox revision gap from its bounded snapshot", async () => {
		const first = inboxSnapshot([entry(1)], {
			inboxRevision: 1,
			catalogRevision: 1,
		});
		const recovered = inboxSnapshot([entry(3), entry(1)], {
			inboxRevision: 3,
			catalogRevision: 3,
		});
		const notificationInbox = vi
			.fn()
	...
	it("ignores an old selected-inbox response after selection changes", async () => {
		...
		const harness = createHarness(capableState(), {
			notificationInbox: vi.fn(() => oldInbox.promise),
		});
		const otherSession = {
			...session,
			id: "session-2",
		...
		expect(harness.state.selectedNotificationInbox).toMatchObject({
			sessionId: "session-2",
			status: "loading",
			notifications: [],
		});
	...
	it("hides a selected remote inbox when the machine becomes unreachable and ignores an in-flight snapshot", async () => {
		...
		if (runtime === undefined)
			throw new Error("expected capable runtime fixture");
		const initial = {
			...capableState(),
			machines: [remoteMachine],
			selectedMachine: remoteMachine,
			machineRuntimes: {
				[remoteMachine.id]: { ...runtime, machineId: remoteMachine.id },
			},
		};
		const pendingInbox = deferred<SessionNotificationInboxSnapshot>();
		const notificationInbox = vi
		...
		const refresh = harness.controller.refreshSelectedSession(
			session,
			remoteMachine.id,
		);
...
describe("SessionNotificationController optimistic mutations", () => {
	it("does not let a delayed refresh snapshot roll back a newer dismissal response", async () => {
		const initial = inboxSnapshot([entry(1)], {
			inboxRevision: 1,
			catalogRevision: 1,
		});
		...
		expect(
			harness.state.selectedNotificationInbox?.summary?.inboxRevision,
		).toBe(2);
		expect(
			selectedNotificationView(harness.state.selectedNotificationInbox)
				?.notifications,
	...
	it("optimistically dismisses one card, reconciles the response, and rolls back/refetches on failure", async () => {
		...
		dismiss.resolve(
			inboxSnapshot([entry(1)], { inboxRevision: 2, catalogRevision: 2 }),
		);
		...
		const failedDismissal =
			harness.controller.dismissNotification("daemon-a:1");
		await vi.waitFor(() => {
			expect(harness.state.error).toContain("offline");
		});
		refreshAfterFailure.resolve(
			inboxSnapshot([entry(1)], { inboxRevision: 2, catalogRevision: 2 }),
		);
	...
	it("uses the server cutoff for dismiss-all and leaves a concurrently arriving newer card visible", async () => {
		...
		const harness = createHarness(capableState(), {
			notificationInbox: vi.fn(() =>
				Promise.resolve(inboxSnapshot([entry(2), entry(1)])),
			),
		...
		expect(dismissAllNotifications).toHaveBeenCalledWith(
			{ id: session.id, cwd: session.cwd },
			"daemon-a",
			{ order: 2, overflowWatermark: 0 },
			"local",
		);
		...
		dismissAll.resolve(
			inboxSnapshot([entry(3)], { inboxRevision: 3, catalogRevision: 3 }),
		);
		await dismissal;
		expect(
			selectedNotificationView(
