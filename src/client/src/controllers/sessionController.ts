...
import {
	forgetCachedNewSession,
	isCachedNewSessionInfo,
	markCachedNewSessionInfo,
	mergeCachedNewSessions,
	rememberCachedNewSession,
	stripCachedNewSessionMarker,
} from "../cachedNewSessions";
...
import {
	...
	isTransientNewSessionInfo,
	sessionPersistenceOptionsForRuntime,
...
import {
	InMemorySessionSelectionMemory,
	markSessionArchived,
	markSessionsArchived,
	selectPreferredSession,
	selectionAfterArchivingSession,
	selectionAfterArchivingSessions,
	shouldDeselectAfterArchivedCollapse,
...
export interface SessionNotificationSessionBridge {
	...
	shouldFilterLegacyNotification(
		machineId: string,
...
export interface SessionControllerDependencies {
	...
	replacePromptEditorText?: (
		replacement: PromptEditorTextReplacement,
	) => void | Promise<void>;
...
type ClientPendingStartSessionInfo = SessionInfo & {
...
export class SessionController {
	...
	private readonly pendingSessionStarts = new Map<
		...
		PendingSessionStart
	>();
	private readonly suppressedCreatedSessions = new Map<
		string,
		SuppressedCreatedSession
	>();
	private readonly selectedSessionRefreshes =
		new TrailingRefreshCoordinator<string>();
	...
	applyGlobalEvent(event: GlobalSessionEvent): void {
		if (event.type === "status.update") this.queueStatusUpdate(event.status);
			...
			this.applyCreatedSession(event.session);
	...
	deselectSession(options?: {
		forgetRememberedSelection?: boolean | undefined;
		updateUrl?: boolean | undefined;
	}) {
		const state = this.getState();
		const cwd = state.selectedSession?.cwd ?? state.selectedWorkspace?.path;
		if (options?.forgetRememberedSelection === true && cwd !== undefined)
			this.sessionSelection.forgetWorkspace(this.workspaceSelectionKey(cwd));
	...
	async startSession(runtimeId: AgentRuntimeId = AGENT_RUNTIME_IDS.pi) {
		const workspace = this.getState().selectedWorkspace;
		if (!workspace) return;
		const machineId = selectedMachineId(this.getState());
		const pending = this.createPendingSessionStart(
			workspace,
			machineId,
			runtimeId,
		);
	...
	preferredSession(
		...
		return selectPreferredSession(sessions, {
			targetSessionId,
			latestSessionId: this.sessionSelection.latestSessionId(
				this.workspaceSelectionKey(cwd),
			),
		});
	}
	...
	async selectSession(
		...
		this.sessionSelection.rememberSession({
			...session,
			cwd: this.workspaceSelectionKey(session.cwd),
		});
		...
		const transcriptKey = this.sessionCacheKey(session);
		const cached = this.transcripts.cachedView(transcriptKey);
		this.setState({
			selectedSession: session,
			...cached,
			isLoadingEarlierMessages: false,
			...(options?.preserveTreeDialog === true
				? {}
				: { treeDialog: undefined }),
			status:
				session.archived === true
					? undefined
					: this.getState().sessionStatuses[session.id],
			activity:
				session.archived === true
					? undefined
					: this.getState().sessionActivities[session.id],
			availableThinkingLevels: [],
		});
		...
		try {
			...
			if (
				...
				if (
					options?.propagateRefreshError === true &&
					this.isSelectedSessionIdentity(session.id, machineId)
				)
					throw error;
				return;
			}
	...
	async loadEarlierMessages() {
		...
		if (
			!session ||
			state.isLoadingEarlierMessages ||
		...
		try {
			const page = await this.api.messages(
				session,
				{ before: state.messagePageStart, limit: MESSAGE_PAGE_SIZE },
				selectedMachineId(this.getState()),
			);
			if (this.getState().selectedSession?.id !== session.id) return;
			const history = this.transcripts.mergeHistory(
				this.sessionCacheKey(session),
				page,
			);
	...
	async send(
		...
		if (isClientPendingStartSessionInfo(session)) {
			...
			else if (!hasAttachments && isShellInput(text))
				this.enqueuePendingSessionSend(session, { type: "shell", text });
			else
				this.enqueuePendingSessionSend(session, {
					type: "prompt",
					text,
					streamingBehavior,
					attachments,
		...
		await this.deliverPromptToSession(
			session,
			text,
			streamingBehavior,
			attachments,
	...
	private markSendingPrompt(sessionId: string, sending: boolean): void {
		...
		if (sending) {
			if (current[sessionId] !== true)
				this.setState({ sendingPrompts: { ...current, [sessionId]: true } });
	...
	async runShell(text: string) {
		...
		await this.deliverShellToSession(
			session,
			text,
			selectedMachineId(this.getState()),
			{ optimisticLine: true },
	...
	async runCommand(text: string) {
		...
		await this.deliverCommandToSession(
			session,
			text,
			selectedMachineId(this.getState()),
			{ applyResult: true },
	...
	private enqueuePendingSessionSend(
		...
		if (pending === undefined || pending.discarded) {
			this.setState({
				error:
					"The backend session is not ready for queued sends. Copy your message before discarding this failed start.",
			});
			return;
		}
		...
		const activity = creatingPendingSessionActivity(
			session.id,
			pending.queuedSends.length,
		);
		this.setState({
			clientQueuedSessionMessages: {
				...state.clientQueuedSessionMessages,
				[session.id]: [...current, queuedSessionMessagePreview(queued)],
			},
			sessionActivities: { ...state.sessionActivities, [session.id]: activity },
			activity:
				state.selectedSession?.id === session.id ? activity : state.activity,
			error: "",
		});
	}

	private async flushQueuedPendingSends(
		session: SessionInfo,
		machineId: string,
		queuedSends: readonly QueuedPendingSessionSend[],
	): Promise<void> {
		for (const queued of queuedSends) {
			const delivered = await this.deliverQueuedPendingSend(
				session,
				machineId,
				queued,
			);
	...
	private async deliverQueuedPendingSend(
		session: SessionInfo,
		machineId: string,
		queued: QueuedPendingSessionSend,
	): Promise<boolean> {
		if (queued.type === "prompt")
			return this.deliverPromptToSession(
				session,
				queued.text,
		...
		if (queued.type === "shell")
			return this.deliverShellToSession(session, queued.text, machineId, {
				optimisticLine: true,
			});
		return this.deliverCommandToSession(session, queued.text, machineId, {
			applyResult: true,
		});
	}
	...
	private async deliverPromptToSession(
		...
		try {
			if (hasAttachments && delivery === "folder") {
				const saved = await this.api.saveAttachments(
					session,
					attachments,
					machineId,
				);
				const references = saved
					.map((file) => fileCompletionInsertText(file.path, false))
					.join(" ");
	...
	private async deliverShellToSession(
		session: SessionInfo,
		text: string,
		machineId: string,
		options: { optimisticLine: boolean },
	): Promise<boolean> {
		if (
			options.optimisticLine &&
			this.getState().selectedSession?.id === session.id
		) {
			this.setState({
				messages: [...this.getState().messages, textMessage("user", text)],
			});
		}
		try {
			...
			if (this.getState().selectedSession?.id === session.id)
				this.setState({
					messages: [
						...
						textMessage("system", String(error)),
					],
				});
			this.setState({ error: String(error) });
			return false;
	...
	private async deliverCommandToSession(
		session: SessionInfo,
		text: string,
		machineId: string,
		...
		try {
			...
			else if (result.type === "select" || result.type === "tree")
				this.setState({
					error: `Queued command “${text}” needs input; open the session and run it again.`,
				});
			this.markCachedNewSessionPersisted(session);
			return true;
		} catch (error) {
	...
	private dropNextQueuedSessionMessage(sessionId: string): void {
		...
		this.setState({
			clientQueuedSessionMessages:
				remaining.length === 0
					? omitKey(state.clientQueuedSessionMessages, sessionId)
					: { ...state.clientQueuedSessionMessages, [sessionId]: remaining },
		});
	}
	...
	async respondToCommand(requestId: string, value: string) {
		...
		try {
			this.applyCommandResult(
				await this.api.respondToCommand(
					session,
					requestId,
					value,
					selectedMachineId(this.getState()),
	...
	async navigateTree(
		...
		if (
			...
			tree === undefined ||
			session.archived === true ||
			isClientPendingStartSessionInfo(session)
		) {
		...
		try {
			result = await this.api.navigateTree(
				session,
				{ targetId, expectedLeafId: tree.activeLeafId, summary },
				machineId,
			);
		} catch (error) {
		...
		try {
			await this.replacePromptEditorText?.({
				machineId,
		...
		if (authoritativeRefreshFailure !== undefined) {
			if (this.isSelectedSessionIdentity(session.id, machineId))
				this.setState({ error: String(authoritativeRefreshFailure.error) });
			throw authoritativeRefreshFailure.error;
		}
		if (
			this.isSelectedSessionIdentity(session.id, machineId) &&
			this.getState().treeDialog === tree
		)
	...
	async archiveSession(session = this.getState().selectedSession) {
		...
		try {
			...
			const state = this.getState();
			const sessions = markSessionArchived(
				state.sessions,
				session.id,
				new Date().toISOString(),
			);
			const selectionChange = selectionAfterArchivingSession(
				sessions,
			...
			if (selectionChange.type === "select")
				await this.selectSession(selectionChange.session);
	...
	async archiveSessionWithDescendants(
		session = this.getState().selectedSession,
	) {
		if (
			session === undefined ||
			!isArchivableSessionInfo(
				session,
				this.statusForSession(session),
				this.sessionPersistenceOptions(),
			)
		)
			return;
		try {
			const response = await this.api.archiveWithDescendants(
				session,
				selectedMachineId(this.getState()),
			);
			const archivedIds =
				response.sessionIds !== undefined && response.sessionIds.length > 0
					? response.sessionIds
					: [session.id];
			const state = this.getState();
			const sessions = markSessionsArchived(
				state.sessions,
				archivedIds,
				new Date().toISOString(),
			);
			const selectionChange = selectionAfterArchivingSessions(
				sessions,
				state.selectedSession?.id,
				archivedIds,
			);
	...
	async archiveSessions(sessions: readonly SessionInfo[]): Promise<void> {
		const persistenceOptions = this.sessionPersistenceOptions();
		const candidates = uniqueSessionsById(sessions).filter((session) =>
			isArchivableSessionInfo(
				session,
				this.statusForSession(session),
				persistenceOptions,
		...
		try {
			const machineId = selectedMachineId(this.getState());
			const {
				succeededIds: archivedIds,
				failures,
				generatedAt,
			} = await this.archiveSessionBatch(candidates, machineId);
			if (archivedIds.length > 0) {
				const state = this.getState();
				const nextSessions = markSessionsArchived(
					state.sessions,
					archivedIds,
	...
	async deleteArchivedSessions(
		...
		const candidates = uniqueSessionsById(sessions).filter(
		...
		if (
			...
			this.setState({
				error:
					"Deleting archived sessions requires an updated Pi-Web runtime on this machine.",
			});
			return;
		}
		try {
			const { succeededIds: deletedIds, failures } =
				await this.deleteArchivedSessionBatch(candidates, machineId);
			if (deletedIds.length > 0) {
				...
				if (
					...
					deletedIdSet.has(state.selectedSession.id)
				) {
					const next =
						nextSessions.find((session) => session.archived !== true) ??
						nextSessions[0];
	...
	private async archiveSessionBatch(
		sessions: readonly SessionInfo[],
		machineId: string,
		...
		if (
			...
			return {
				succeededIds: response.archivedSessionIds,
				failures: bulkFailureMessages(response.failures),
		...
		return {
			succeededIds: fulfilledValues(results),
			failures: settledSessionFailureMessages(sessions, results),
		};
	}

	private async deleteArchivedSessionBatch(
		sessions: readonly SessionInfo[],
		machineId: string,
	): Promise<BulkSessionMutationResult> {
		const runtime = this.getState().machineRuntimes[machineId];
		if (
			...
			return {
				succeededIds: response.deletedSessionIds,
				failures: bulkFailureMessages(response.failures),
			};
		}
	...
	async applySessionCleanupResult(
		result: SessionCleanupExecuteResponse,
		...
		if (archivedIds.length > 0 || deletedIds.length > 0) {
			...
			const nextSessions = markSessionsArchived(
				...
				archivedIds,
				result.generatedAt,
			).filter((session) => !deletedIdSet.has(session.id));
			const selectedAffected =
				state.selectedSession !== undefined &&
				affectedIds.includes(state.selectedSession.id);
			this.setState({
	...
	async refreshCurrentWorkspaceSessions(
		...
		try {
			const listedSessions = mergeCachedNewSessions(
				workspace.path,
				await this.api.sessions(workspace.path, machineId),
				machineId,
			).filter(
				(session) => !this.isSuppressedCreatedSession(session, machineId),
			);
			if (
				...
				return;
			const sessions = this.mergePendingStartSessions(
				workspace.path,
				listedSessions,
				machineId,
			);
			...
			const refreshedSelected = sessions.find(
				(session) => session.id === selectedSession.id,
			);
			if (refreshedSelected !== undefined) {
				if (refreshedSelected !== selectedSession)
					this.setState({ selectedSession: refreshedSelected });
				return;
			}
			const next =
				sessions.find((session) => session.archived !== true) ?? sessions[0];
			if (next !== undefined) await this.selectSession(next);
			else this.deselectSession({ forgetRememberedSelection: true });
		} catch (error) {
			if (
	...
	async deleteCachedNewSession(session = this.getState().selectedSession) {
		if (
			...
			return;
		const pendingStart = isClientPendingStartSessionInfo(session)
			? this.pendingSessionStarts.get(session.id)
			: undefined;
		if (pendingStart !== undefined) {
			...
			void this.api
				.stop(session, selectedMachineId(this.getState()))
				.catch(() => {
		...
		clearDraft(this.sessionCacheKey(session));
		const state = this.getState();
		...
		if (this.getState().selectedSession?.id !== session.id) return;
		const next =
			sessions.find((candidate) => candidate.archived !== true) ?? sessions[0];
		if (next !== undefined) await this.selectSession(next);
		else {
			this.clearActiveSession();
			this.updateUrl();
	...
	async restoreSession(session = this.getState().selectedSession) {
		...
		try {
			...
			if (this.getState().selectedSession?.id === restored.id)
	...
	async reloadSession(session = this.getState().selectedSession) {
		...
		if (
			...
			this.setState({
				error:
					"Reloading sessions from disk requires an updated Pi-Web runtime on this machine.",
			});
			return;
		}
		try {
			await this.api.reloadSession(session.id, machineId);
			this.transcripts.discard(this.sessionCacheKey(session));
			if (this.getState().selectedSession?.id === session.id) {
	...
	async listModels() {
		...
		try {
			return (
				await this.api.models(session, selectedMachineId(this.getState()))
			).models;
		} catch (error) {
			this.setState({ error: String(error) });
			return [];
	...
	async refreshAvailableThinkingLevels() {
		...
		if (!session || session.archived === true) {
			if (this.getState().availableThinkingLevels.length > 0)
				this.setState({ availableThinkingLevels: [] });
			return;
		}
	...
	async setThinkingLevel(level: string) {
		...
		try {
			this.applyStatus(
				await this.api.setThinkingLevel(
					session,
					level,
					selectedMachineId(this.getState()),
				),
	...
	async cycleThinkingLevel() {
		...
		try {
			this.applyStatus(
				await this.api.cycleThinkingLevel(
					session,
					selectedMachineId(this.getState()),
				),
			);
	...
	async dismissWarning(dismissId: string) {
		const state = this.getState();
		const session = state.selectedSession;
		if (session === undefined || isClientPendingStartSessionInfo(session))
			return;
		const machineId = selectedMachineId(state);
		const selectionSeq = this.selectionSeq;
		try {
			const status = await this.api.dismissWarning(
				session,
				dismissId,
				machineId,
			);
	...
	refreshSelectedSession(
		sessionId = this.getState().selectedSession?.id,
	): Promise<void> {
		const session = this.getState().selectedSession;
		if (
			sessionId === undefined ||
		...
		return this.requestSelectedSessionRefresh(target).catch(
			(error: unknown) => {
	...
	private requestSelectedSessionRefresh(
		target: SelectedSessionRefreshTarget,
	): Promise<void> {
		const key = machineRuntimeSessionKey(
			target.machineId,
		...
		return this.selectedSessionRefreshes.request(key, async () => {
			...
			const [page, status, streamSnapshot] = await Promise.all([
				...
				this.api
					.streamSnapshot(target.session, target.machineId)
					.catch((): SessionStreamSnapshot => ({ seq: 0, partial: null })),
				this.notifications?.refreshSelectedSession(
					target.session,
					target.machineId,
				) ?? Promise.resolve(),
			]);
			...
			const messages = this.transcripts.seedStreamingPartial(
				history.messages,
				streamSnapshot.partial,
			);
			this.streamWatermark = {
				sessionId: target.session.id,
				seq: streamSnapshot.seq,
			};
			this.setState({
				...history,
				messages,
	...
	private isCurrentRefreshTarget(
		target: SelectedSessionRefreshTarget,
	): boolean {
		return this.isCurrentSessionSelection(
			target.session.id,
			target.machineId,
			target.selectionSeq,
		);
	...
	private isCurrentSessionSelection(
		sessionId: string,
		machineId: string,
		selectionSeq: number,
	): boolean {
	...
	private isSelectedSessionIdentity(
		sessionId: string,
		machineId: string,
	): boolean {
	...
	private applyBulkSessionFailures(
		action: string,
		failures: readonly string[],
	): void {
		if (failures.length === 0) return;
		this.setState({
			error: `${action} failed for ${String(failures.length)} session${failures.length === 1 ? "" : "s"}: ${failures.join("; ")}`,
		});
	}

	private sessionCacheKey(
		session: Pick<SessionInfo, "id" | "runtimeId">,
	): string {
		return machineRuntimeSessionKey(
			selectedMachineId(this.getState()),
			session.runtimeId,
			session.id,
		);
	...
	private requireRuntimeCapability(
		...
		const state = this.getState();
		const catalog = state.agentRuntimeCatalogs[selectedMachineId(state)];
		if (catalog === undefined) return true;
		if (supportsAgentRuntimeCapability(catalog, session.runtimeId, capability))
			return true;
		this.setState({
			error: `${label} are not supported by the ${session.runtimeId.toUpperCase()} runtime.`,
		});
		return false;
	}

	private statusForSession(
		session: SessionInfo | undefined,
	): SessionStatus | undefined {
		if (session === undefined) return undefined;
		const state = this.getState();
		if (
			state.status?.sessionId === session.id &&
	...
	private sessionPersistenceOptions() {
		const state = this.getState();
		return sessionPersistenceOptionsForRuntime(
			state.machineRuntimes[selectedMachineId(state)],
		);
	}
	...
	private replaceSession(session: SessionInfo) {
		...
		this.setState({
			sessions: this.getState().sessions.map((candidate) =>
	...
	private insertAndSelectPendingSession(
		session: ClientPendingStartSessionInfo,
	): void {
		const state = this.getState();
	...
	private selectClientPendingStartSession(
		session: ClientPendingStartSessionInfo,
		options?: {
			updateUrl?: boolean | undefined;
		...
		const activity =
			options?.activity ??
			state.sessionActivities[session.id] ??
			(pendingStart !== undefined
				? creatingPendingSessionActivity(
						session.id,
						pendingStart.queuedSends.length,
					)
				: undefined);
		this.setState({
	...
	private async resolvePendingSessionStart(
		tempId: string,
		session: SessionInfo,
	): Promise<void> {
		...
		const releasedCreatedSessions = this.takeSuppressedCreatedSessionsFor(
			pending.cwd,
			pending.machineId,
			session.id,
		);
		if (pending.discarded) {
			clearDraft(
				machineRuntimeSessionKey(
					pending.machineId,
					pending.session.runtimeId,
					tempId,
				),
			);
			this.setState({
				clientQueuedSessionMessages: omitKey(
					this.getState().clientQueuedSessionMessages,
					tempId,
				),
			});
			this.applyReleasedCreatedSessions(
				releasedCreatedSessions,
				pending.machineId,
			);
		...
		if (!this.isCurrentPendingStart(pending)) {
			...
			await this.flushQueuedPendingSends(
				cachedSession,
				pending.machineId,
				queuedSends,
			);
		...
		this.setState({
			sessions: replacePendingSessionInList(
				state.sessions,
				tempId,
				cachedSession,
			),
			sessionActivities: omitSessionActivity(state.sessionActivities, tempId),
			sendingPrompts: moveRecordKey(
				state.sendingPrompts,
				tempId,
				cachedSession.id,
			),
			clientQueuedSessionMessages: moveRecordKey(
				state.clientQueuedSessionMessages,
				tempId,
				cachedSession.id,
			),
			...(wasSelected
				? {
						...
						status: state.sessionStatuses[cachedSession.id],
						activity: state.sessionActivities[cachedSession.id],
					}
				: {}),
			error: "",
		});
	...
	private failPendingSessionStart(tempId: string, error: unknown): void {
		...
		const activity = failedPendingSessionActivity(
			tempId,
			message,
			pending.queuedSends.length,
		);
		const hasPendingRow = state.sessions.some(
			(session) => session.id === tempId,
		);
		this.setState({
			sessions: hasPendingRow
				? state.sessions
				: [pending.session, ...state.sessions],
			sessionActivities: { ...state.sessionActivities, [tempId]: activity },
			activity:
				state.selectedSession?.id === tempId ? activity : state.activity,
	...
	private isCurrentPendingStart(pending: PendingSessionStart): boolean {
		const state = this.getState();
		return (
			selectedMachineId(state) === pending.machineId &&
			state.selectedWorkspace?.id === pending.workspaceId
		);
	}

	private hasPendingStartFor(cwd: string, machineId: string): boolean {
		return Array.from(this.pendingSessionStarts.values()).some(
			(pending) => pending.cwd === cwd && pending.machineId === machineId,
		);
	}

	private isSuppressedCreatedSession(
		session: SessionInfo,
		machineId: string,
	): boolean {
		const suppressed = this.suppressedCreatedSessions.get(session.id);
		return (
			suppressed?.session.cwd === session.cwd &&
			suppressed.machineId === machineId
		);
	}

	private takeSuppressedCreatedSessionsFor(
		cwd: string,
		machineId: string,
		resolvedSessionId?: string,
	): SessionInfo[] {
		if (resolvedSessionId !== undefined)
			this.suppressedCreatedSessions.delete(resolvedSessionId);
		...
		for (const [sessionId, suppressed] of this.suppressedCreatedSessions) {
			if (suppressed.session.cwd !== cwd || suppressed.machineId !== machineId)
				continue;
	...
	private applyReleasedCreatedSessions(
		sessions: readonly SessionInfo[],
		machineId: string,
	): void {
		if (
		...
		const released = sessions.filter(
			(session) =>
				session.cwd === state.selectedWorkspace?.path &&
	...
	private mergePendingStartSessions(
		cwd: string,
		sessions: SessionInfo[],
		machineId: string,
	): SessionInfo[] {
		const pending = this.getState().sessions.filter(
			(session): session is ClientPendingStartSessionInfo =>
				isClientPendingStartSessionInfo(session) &&
				session.cwd === cwd &&
		...
		return [
			...pending,
			...sessions.filter((session) => !pendingIds.has(session.id)),
		];
	}
	...
	private async recreateCachedNewSession(
		...
		try {
			const machineId = selectedMachineId(this.getState());
			const replacement = await this.api.startSession(
				session.cwd,
				machineId,
				session.runtimeId,
			);
			...
			moveDraft(
				this.sessionCacheKey(session),
				this.sessionCacheKey(replacement),
			);
			forgetCachedNewSession(session.id, machineId);
			const cachedReplacement = markCachedNewSessionInfo(
				replacement,
				machineId,
			);
			this.setState({
			...
			this.updateUrl(
				options?.updateUrl === false ? { replace: true } : undefined,
			);
		} catch (error) {
			this.setState({ error: String(error) });
		}
	...
	private applyCommandResult(result: CommandResult) {
		...
		const message =
			result.type === "unsupported" ? result.message : result.message;
		if (message !== undefined && message !== "")
			this.setState({
				messages: [
					...this.getState().messages,
					textMessage(
						result.type === "unsupported" ? "system" : "tool",
						message,
					),
				],
			});
		if (result.type === "done" && result.session) {
			if (result.promptDraft !== undefined)
				saveDraft(this.sessionCacheKey(result.session), result.promptDraft);
			const current = this.getState().selectedSession;
			const sessions = [
				result.session,
				...this.getState().sessions.filter(
			...
			if (current?.id !== result.session.id)
	...
	private applyActivity(activity: SessionActivity) {
		this.setState({
			sessionActivities: {
				...this.getState().sessionActivities,
				[activity.sessionId]: activity,
			},
			activity:
				this.getState().selectedSession?.id === activity.sessionId
					? activity
					: this.getState().activity,
		});
	}

	private applyStatus(status: SessionStatus) {
		const state = this.getState();
		const clearsStaleActivity =
			state.sessionActivities[status.sessionId]?.phase === "active" &&
		...
		this.setState({
			...
			...(clearsStaleActivity
				...
				: {}),
			status:
				state.selectedSession?.id === status.sessionId ? status : state.status,
			activity:
				state.selectedSession?.id === status.sessionId && clearsStaleActivity
					? undefined
					: state.activity,
		});
	}
	...
	private applySessionName(sessionId: string, name: string | undefined) {
		...
		this.setState({
			...
			selectedSession:
				selectedSession === undefined ? undefined : rename(selectedSession),
		});
	}
	...
	private applyEvent(event: SessionUiEvent) {
		...
		if (event.type === "notifications.inbox") {
			this.notifications?.applyInboxEvent(
				selectedMachineId(this.getState()),
				event,
			);
			return;
		...
		if (
			...
			this.notifications?.shouldFilterLegacyNotification(
				selectedMachineId(this.getState()),
		...
		this.flushPendingUpdates();
		const transcript = this.transcripts.applyLiveEvent(
			this.getState().messages,
			event,
		);
		if (transcript) {
	...
	private isStreamEventBelowWatermark(event: SessionUiEvent): boolean {
		...
		if (
			watermark === undefined ||
			watermark.sessionId !== this.getState().selectedSession?.id
		)
			return false;
...
function replacePendingSessionInList(
	sessions: readonly SessionInfo[],
	pendingSessionId: string,
	resolvedSession: SessionInfo,
): SessionInfo[] {
...
function isClientPendingStartSessionInfo(
	session: SessionInfo | undefined,
): session is ClientPendingStartSessionInfo {
	return (
		session !== undefined &&
		"clientPendingStart" in session &&
		session.clientPendingStart === true
	);
}

function creatingPendingSessionActivity(
	sessionId: string,
	queuedCount = 0,
): SessionActivity {
	return {
		...
		detail:
			queuedCount > 0
				? `${String(queuedCount)} queued ${queuedCount === 1 ? "message" : "messages"} will send when the backend session is ready`
				: "Waiting for the backend session to be ready",
		at: new Date().toISOString(),
	};
}

function failedPendingSessionActivity(
	sessionId: string,
	message: string,
	queuedCount = 0,
): SessionActivity {
	const queuedDetail =
		queuedCount > 0
			? ` · ${String(queuedCount)} queued ${queuedCount === 1 ? "message" : "messages"} kept below`
			: "";
	return {
		sessionId,
		phase: "error",
...
function queuedSessionMessagePreview(
	queued: QueuedPendingSessionSend,
): QueuedSessionMessage {
	if (queued.type === "prompt") {
		return {
			kind: queued.streamingBehavior === "steer" ? "steer" : "followUp",
			text: queuedPromptPreviewText(queued.text, queued.attachments),
		};
	}
	return { kind: "followUp", text: queued.text };
}

function queuedPromptPreviewText(
	text: string,
	attachments: PromptAttachment[] | undefined,
): string {
...
function queuedAttachmentSummary(
	attachments: PromptAttachment[] | undefined,
): string | undefined {
...
function bulkFailureMessages(
	failures: readonly SessionBulkFailure[],
): string[] {
	return failures.map((failure) => `${failure.sessionId}: ${failure.error}`);
}

function settledSessionFailureMessages(
	sessions: readonly SessionInfo[],
...
function sessionMessageCountPatch(
	state: AppState,
	sessionId: string,
	messageCount: number | undefined,
): Pick<Partial<AppState>, "sessions" | "selectedSession"> {
	if (messageCount === undefined) return {};

	const sessionsChanged = state.sessions.some(
		(session) =>
			session.id === sessionId && session.messageCount !== messageCount,
	);
	const sessions = sessionsChanged
		? state.sessions.map((session) =>
				session.id === sessionId ? { ...session, messageCount } : session,
			)
		: undefined;
	const selectedSession =
		state.selectedSession?.id === sessionId &&
		state.selectedSession.messageCount !== messageCount
