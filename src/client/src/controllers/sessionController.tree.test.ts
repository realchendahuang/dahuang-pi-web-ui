...
describe("SessionController session tree navigation", () => {
	it("opens tree command results and keeps older-server unsupported results inert", async () => {
		...
		const results: CommandResult[] = [
			...
			{
				type: "unsupported",
				message: "Session tree navigation is unavailable on this server",
			},
		];
		const api: typeof defaultApi = {
			...
			runCommand: () =>
				Promise.resolve(
					results.shift() ?? { type: "unsupported", message: "missing result" },
				),
			navigateTree,
		};
		...
		expect(state.messages).toEqual([
			{
				...
				parts: [
					{
						type: "text",
						text: "Session tree navigation is unavailable on this server",
					},
				],
			},
		]);
	...
	it("discards stale history and pending live updates, runs a trailing authoritative refresh, and replaces the user draft", async () => {
		...
		const api: typeof defaultApi = {
			...
			navigateTree: (session, request, machineId) => {
				navigationCalls.push({
					sessionId: sessionLookupId(session),
					request,
					machineId,
				});
				return Promise.resolve({
					cancelled: false,
					editorText: "edit original prompt",
				});
			},
			messages: () => {
				messageCalls += 1;
			...
			status: () => {
				...
				return Promise.resolve({
					...
					messageCount: statusCalls === 1 ? 1 : 2,
				});
			},
		...
		socket.emit({
			type: "message.append",
			message: { role: "assistant", content: "stale live event" },
			seq: 1,
		});
		...
		const navigation = controller.navigateTree("root", {
			mode: "custom",
			instructions: "focus on the prompt",
		});
		await Promise.resolve();
		...
		expect(navigationCalls).toEqual([
			{
				...
				request: {
					...
					summary: { mode: "custom", instructions: "focus on the prompt" },
				},
				machineId: "local",
			},
		]);
		...
		expect(state.messages).toEqual([
			{ role: "assistant", parts: [{ type: "text", text: "fresh branch" }] },
		]);
		expect(state.treeDialog).toBeUndefined();
	...
	it("keeps the busy tree mounted until authoritative history and editor replacement finish", async () => {
		...
		const messages = vi.fn<typeof defaultApi.messages>(
			() => authoritativePage.promise,
		);
		const api: typeof defaultApi = {
			...defaultApi,
			navigateTree: () =>
				Promise.resolve({ cancelled: false, editorText: "edit after refresh" }),
			messages,
	...
	it("retains the navigator when the authoritative post-navigation refresh fails", async () => {
		...
		const api: typeof defaultApi = {
			...defaultApi,
			navigateTree: () =>
				Promise.resolve({ cancelled: false, editorText: "recovered draft" }),
			messages: () =>
				Promise.reject(new Error("authoritative history refresh failed")),
			status: () => Promise.resolve(status(oldSession.id)),
			streamSnapshot: () => Promise.resolve({ seq: 0, partial: null }),
			thinkingLevels: () => Promise.resolve({ levels: [] }),
		};
		...
		await expect(
			controller.navigateTree("root", { mode: "none" }),
		).rejects.toThrow("authoritative history refresh failed");
	...
	it("retains the navigator when live prompt-editor replacement fails", async () => {
		...
		const replacePromptEditorText = vi.fn(() =>
			Promise.reject(new Error("prompt editor replacement failed")),
		);
		const api: typeof defaultApi = {
			...defaultApi,
			navigateTree: () =>
		...
		await expect(
			controller.navigateTree("root", { mode: "none" }),
		).rejects.toThrow("prompt editor replacement failed");

		expect(state.messages).toEqual([
			{
				role: "assistant",
				parts: [{ type: "text", text: "authoritative branch" }],
			},
		]);
		expect(state.treeDialog).toBe(tree);
	...
	it("retains the tree on cancellation and errors and exposes abort and close lifecycle methods", async () => {
		...
		navigateTree
			...
			.mockRejectedValueOnce(new Error("The session changed; reopen /tree"));
		const abort = vi.fn<typeof defaultApi.abort>(() =>
		...
		await expect(
			controller.navigateTree("root", { mode: "default" }),
		).resolves.toEqual({ cancelled: true, aborted: true });
		expect(state.treeDialog).toBe(tree);

		await expect(
			controller.navigateTree("root", { mode: "none" }),
		).rejects.toThrow("reopen /tree");
		expect(state.treeDialog).toBe(tree);
	...
	it("keeps live socket events flowing when a selected-session join refresh fails", async () => {
		...
		socket.emit({
			type: "message.append",
			message: { role: "assistant", content: "live after failed refresh" },
			seq: 1,
		});
		...
		expect(state.messages).toEqual([
			{
				role: "assistant",
				parts: [{ type: "text", text: "live after failed refresh" }],
			},
		]);
	});

	it("does not reopen a disposed controller when navigation settles late", async () => {
		const navigationResult = deferred<{
			cancelled: false;
			editorText: string;
		}>();
		const messages = vi.fn<typeof defaultApi.messages>(() =>
			Promise.resolve(page("must not load", 1)),
		);
		const replacePromptEditorText = vi.fn();
		let state: AppState = {
			...initialAppState(),
	...
	it("replaces live editor text when the same session is reselected during the authoritative refresh", async () => {
		...
		const api: typeof defaultApi = {
			...
			messages: () => {
				...
				return messageCalls === 1
					...
					: Promise.resolve(page("reselected authoritative branch", 1));
			},
			status: () => Promise.resolve(status(oldSession.id)),
			streamSnapshot: () => Promise.resolve({ seq: 0, partial: null }),
			thinkingLevels: () => Promise.resolve({ levels: [] }),
		...
		const reselection = controller.selectSession(oldSession, {
			updateUrl: false,
		});
		...
		expect(state.messages).toEqual([
			{
				role: "assistant",
				parts: [{ type: "text", text: "reselected authoritative branch" }],
			},
		]);
		expect(replacePromptEditorText).toHaveBeenCalledWith({
			machineId: "local",
	...
	it("discards only the originating cache and does not refresh or replace another session after a selection race", async () => {
		...
		expect(state.messages).toEqual([
			{
				role: "assistant",
				parts: [{ type: "text", text: "replacement authoritative" }],
			},
		]);
		expect(requestedMessages).toEqual([replacementSession.id]);
		expect(removedKeys).toEqual([oldCacheKey]);
		expect(cachedPages.get(replacementCacheKey)).toEqual(
			page("replacement authoritative", 1),
		);
	...
	it("does not open a delayed tree snapshot for the same session id on a different machine", async () => {
		...
		expect(state.error).toContain(
			"needs input; open the session and run it again",
		);
	});
