...
describe("parseAuthSlashCommand", () => {
	...
	it("parses provider arguments", () => {
		expect(parseAuthSlashCommand("/login openai")).toEqual({
			command: "login",
			providerId: "openai",
		});
		expect(parseAuthSlashCommand("/logout openai-codex ")).toEqual({
			command: "logout",
...
describe("AuthController", () => {
	it("uses auth type to disambiguate provider options with the same id", async () => {
		const providers = [
			...
			authProvider("anthropic", "api_key"),
		];
		...
		expect(getState().authDialog).toMatchObject({
			step: "apiKey",
			provider: { id: "anthropic", authType: "api_key" },
		});
	});
	...
	it("starts provider-driven API-key interactions instead of opening the legacy one-secret form", async () => {
		...
		const provider: AuthProviderOption = {
			...authProvider("amazon-bedrock", "api_key"),
		...
		try {
			...
			expect(calls).toEqual([
				{ providerId: "amazon-bedrock", machineId: "local" },
			]);
			expect(getState().authDialog).toMatchObject({
				step: "oauth",
				flow: {
					providerId: "amazon-bedrock",
					select: { requestId: "request-1" },
				},
			});
		} finally {
			controller.dispose();
					...
					Promise.resolve(
						oauthFlow({
							select: {
								...
								options: [{ value: "acct-1", label: "Account 1" }],
							},
	...
	it("closes the OAuth dialog and refreshes selected session status when the flow completes", async () => {
		...
		const statusCalls: {
			session: Parameters<typeof defaultApi.status>[0];
			machineId: string | undefined;
		}[] = [];
	...
	it("does not apply an auth status refresh after the selected session changes", async () => {
		...
		const { controller, setState } = createController(
			...
			{
				respondOAuthFlow: () =>
					Promise.resolve(oauthFlow({ status: "complete" })),
		...
		expect(statusCalls).toEqual([
			{ session: originalSession, machineId: "remote-1" },
		]);
		expect(appliedStatuses).toEqual([]);
	});
	...
	it("best-effort cancels a running flow whose start response arrives after the dialog closes", async () => {
		...
		const pendingStart = controller.selectLoginProvider(
			provider.id,
			provider.authType,
		);
		...
		expect(cancelCalls).toEqual([
			{ flowId: "stale-flow", machineId: "remote-1" },
		]);
		expect(getState().authDialog).toBeUndefined();
	...
	it("keeps prompt responses and cancellation bound to the flow's originating machine", async () => {
		...
		expect(respondCalls).toEqual([
			{ value: "secret-value", machineId: "remote-1" },
		]);
	...
	it("keeps polling bound to the machine where an interactive flow started", async () => {
		...
		try {
			...
			if (pollCallback === undefined)
				throw new Error("Expected auth polling to start");
			pollCallback();
			await flushMicrotasks();
	...
	it("does not let a stale OAuth response overwrite a newer flow", async () => {
		...
		try {
			...
			response.resolve(
				oauthFlow({ prompt: oldPrompt, progress: ["Stale response"] }),
			);
			await responsePending;

			expect(dialogAfterNewFlow).toMatchObject({
				step: "oauth",
				flow: { flowId: "flow-2" },
			});
			expect(getState().authDialog).toMatchObject({
	...
	it("does not let an older poll restore a running flow after a newer poll stops polling", async () => {
		...
		const { controller, getState } = createController(
			{},
			{
				...
				oauthFlow: () => {
					...
					return pollCalls === 1
						? stalePoll.promise
						: Promise.resolve(oauthFlow({ status: "cancelled", prompt }));
				},
			},
		);
		...
		try {
			...
			stalePoll.resolve(
				oauthFlow({ prompt, progress: ["Stale running poll"] }),
			);
			await flushMicrotasks();

			expect(pollCalls).toBe(2);
			expect(dialogAfterPollingStopped).toMatchObject({
				step: "oauth",
	...
	it("saves a trimmed API key on the selected machine and refreshes selected session status", async () => {
		...
		expect(saveCalls).toEqual([
			{ providerId: "openai", key: "sk-live", machineId: "remote-1" },
		]);
		expect(getState().authDialog).toBeUndefined();
...
function createController(
	...
	const controller = new AuthController(() => state, setState, applyStatus, {
		api,
	});
