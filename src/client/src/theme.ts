...
export function resolveThemePreference(
	...
	const fallbackTheme = findFallbackTheme(
		options.themes,
		options.fallbackThemeId ?? CLASSIC_THEME_ID,
	);
	const selectedTheme =
		options.themes.find(
			(candidate) => candidate.id === options.preference.themeId,
		) ?? fallbackTheme;
	if (selectedTheme === undefined) {
		return {
			selectedTheme: undefined,
			activeTheme: undefined,
			selectedThemePair: undefined,
			fallbackTheme,
		};
	}
	const selectedThemePair = findThemePairForTheme(
		options.themePairs,
		selectedTheme.id,
	);
	...
	const activeThemeId = options.prefersLight
		? selectedThemePair.light
		: selectedThemePair.dark;
	const activeTheme =
		options.themes.find((candidate) => candidate.id === activeThemeId) ??
		selectedTheme;
	return { selectedTheme, activeTheme, selectedThemePair, fallbackTheme };
}

export function findFallbackTheme(
	themes: readonly QualifiedThemeContribution[],
	fallbackThemeId: QualifiedContributionId = CLASSIC_THEME_ID,
): QualifiedThemeContribution | undefined {
	return (
		themes.find((candidate) => candidate.id === fallbackThemeId) ?? themes[0]
	);
}

export function findThemePairForTheme(
	themePairs: readonly QualifiedThemePairContribution[],
	themeId: QualifiedContributionId,
): QualifiedThemePairContribution | undefined {
	return themePairs.find(
		(pair) => pair.light === themeId || pair.dark === themeId,
	);
}
...
function isQualifiedContributionId(
	value: unknown,
): value is QualifiedContributionId {
	return (
		typeof value === "string" && qualifiedContributionIdPattern.test(value)
	);
}
