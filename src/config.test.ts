...
import {
	...
	hasAgentSessionDirEnvOverride,
	loadPiWebConfig,
	maxUploadBytes,
	savePiWebConfig,
	spawnSessionsEnabled,
	subsessionsEnabled,
} from "./config.js";
...
describe("PI WEB config persistence", () => {
	it("writes and reads the configured PI WEB config path", () => {
		const requestedConfig = {
			...
			plugins: {
				"workspace-tasks": {
					enabled: false,
					settings: { configPath: ".pi-web/tasks.json" },
				},
			},
	...
	it("preserves unrelated config keys while replacing managed keys", async () => {
		await writeFile(
			configPath,
			`${JSON.stringify({ host: "old", port: 31415, allowedHosts: true, plugins: { info: { enabled: false } }, pathAccess: { allowedPaths: ["/old"] }, uploads: { defaultFolder: "old" }, future: { enabled: true } }, null, 2)}\n`,
			"utf8",
		);
		...
		savePiWebConfig(
			{
				port: 9000,
				allowedHosts: [],
				pathAccess: { allowedPaths: ["/new"] },
				uploads: { defaultFolder: "new" },
			},
			testOptions(),
		);
	...
	it("rejects invalid plugin config", async () => {
		await writeFile(
			configPath,
			`${JSON.stringify({ plugins: { info: { enabled: "no" } } }, null, 2)}\n`,
			"utf8",
		);

		expect(() => loadPiWebConfig(testOptions())).toThrow(
			"PI WEB config plugin enabled values must be booleans",
		);
	});
	...
	it("rejects invalid path access config", async () => {
		await writeFile(
			configPath,
			`${JSON.stringify({ pathAccess: { allowedPaths: [""] } }, null, 2)}\n`,
			"utf8",
		);

		expect(() => loadPiWebConfig(testOptions())).toThrow(
			"PI WEB config pathAccess.allowedPaths must be an array of non-empty strings",
		);
	});
	...
	it("persists and resolves isolated OMP runtime settings", () => {
		...
		savePiWebConfig(
			{
				...
				agentRuntimes: {
					default: "omp",
					omp: { command: "omp-custom", dir: ompDir },
				},
			},
			testOptions(),
		);

		const loaded = loadPiWebConfig(testOptions());
		expect(loaded.config.agent).toEqual({
			command: "acme-agent",
			dir: "/opt/acme-agent/state",
		});
		expect(loaded.config.agentRuntimes).toEqual({
			default: "omp",
			omp: { command: "omp-custom", dir: ompDir },
		});
		expect(defaultAgentRuntimeId({}, loaded.config)).toBe("omp");
		expect(effectiveOmpRuntimeConfig({}, loaded.config)).toEqual({
			command: "omp-custom",
			dir: ompDir,
		});
	});

	it("reports invalid JSON with the config path instead of leaking a raw parser error", async () => {
		await writeFile(configPath, "{ invalid", "utf8");
		expect(() => loadPiWebConfig(testOptions())).toThrow(
			`PI WEB config contains invalid JSON: ${configPath}`,
		);
		expect(() => savePiWebConfig({ port: 9000 }, testOptions())).toThrow(
			`PI WEB config contains invalid JSON: ${configPath}`,
		);
	});
	...
	it("requires explicit state for alternate names and absolute Pi launchers", () => {
		...
		for (const command of ["acme-agent", absolutePiCommand]) {
			expect(() => effectiveAgentConfig({}, { agent: { command } })).toThrow(
				`PI WEB config agent.dir or PI_WEB_AGENT_DIR is required when agent.command is ${JSON.stringify(command)}`,
			);
			expect(() =>
				savePiWebConfig({ agent: { command } }, testOptions()),
			).toThrow(
				`PI WEB config agent.dir or PI_WEB_AGENT_DIR is required when agent.command is ${JSON.stringify(command)}`,
			);
		}
	...
	it("accepts safe bare executable names and host-absolute executable paths", () => {
		...
		expect(
			effectiveAgentConfig(
				{},
				{ agent: { command: "acme-agent", dir: agentDir } },
			),
		).toMatchObject({ command: "acme-agent", dir: agentDir });
		expect(
			effectiveAgentConfig(
				{},
				{ agent: { command: absoluteCommand, dir: agentDir } },
			),
		).toMatchObject({ command: absoluteCommand, dir: agentDir });
	});
	...
	it.each([
		...
		"acme-agent;other",
		"-acme-agent",
	])("rejects unsafe or workspace-relative agent command %j", (command) => {
		expect(() =>
			savePiWebConfig(
				{ agent: { command, dir: join(tempDir, "agent") } },
				testOptions(),
			),
		).toThrow("safe bare executable name or host-absolute executable path");
	});

	it.skipIf(process.platform === "win32")(
		"rejects foreign-platform absolute agent command and state paths",
		() => {
			expect(() =>
				effectiveAgentConfig(
					{},
					{
						agent: {
							command: "C:\\tools\\acme-agent.exe",
							dir: join(tempDir, "agent"),
						},
					},
				),
			).toThrow("safe bare executable name or host-absolute executable path");
			expect(() =>
				effectiveAgentConfig(
					{},
					{ agent: { command: "acme-agent", dir: "C:\\profiles\\acme" } },
				),
			).toThrow("agent.dir must be a host-absolute path");
		},
	);

	it("rejects home expansion that would create a workspace-relative agent directory", () => {
		expect(() => effectiveAgentConfig({ HOME: "relative-home" })).toThrow(
			"agent.dir must be a host-absolute path",
		);
	});
	...
	it("resolves explicit alternate agent command and state directory settings", () => {
		expect(
			effectiveAgentConfig(
				{ HOME: join(tempDir, ".home") },
				{ agent: { command: "acme-agent", dir: "~/agent-profiles/acme" } },
			),
		).toMatchObject({
			command: "acme-agent",
	...
	it("uses explicit PI WEB agent directory env precedence", () => {
		...
		expect(
			effectiveAgentConfig(env, {
				agent: { command: "pi", dir: join(tempDir, "config-agent") },
			}),
		).toMatchObject({
			command: "acme-agent",
	...
	it("keeps legacy Pi env directory overrides scoped to the canonical Pi command", () => {
		...
		expect(
			effectiveAgentConfig(env, {
				agent: { dir: join(tempDir, "config-agent") },
			}),
		).toMatchObject({ dir: legacyDir });
		expect(
			effectiveAgentConfig(env, {
				agent: { command: "acme-agent", dir: alternateDir },
			}),
		).toMatchObject({ command: "acme-agent", dir: alternateDir });
		...
		for (const command of ["acme-agent", join(tempDir, "bin", "pi")]) {
			expect(() => effectiveAgentConfig(env, { agent: { command } })).toThrow(
				`PI WEB config agent.dir or PI_WEB_AGENT_DIR is required when agent.command is ${JSON.stringify(command)}`,
			);
		}
	});
	...
	it("uses only explicit session directory env keys", () => {
		...
		expect(
			effectiveAgentConfig({
				HOME: join(tempDir, ".home"),
				PI_WEB_AGENT_COMMAND: "acme-agent",
				PI_WEB_AGENT_DIR: join(tempDir, "agent"),
			}).sessionDirEnvKeys,
		).toEqual(["PI_WEB_AGENT_SESSION_DIR"]);
		expect(agentSessionDirEnvKeys(join(tempDir, "bin", "pi"))).toEqual([
			"PI_WEB_AGENT_SESSION_DIR",
		]);
	});
	...
	it("rejects unknown nested agent keys instead of erasing them", async () => {
		...
		expect(() => loadPiWebConfig(testOptions())).toThrow(
			'PI WEB config agent contains unknown key "futureSetting"',
		);
		expect(() => savePiWebConfig({ port: 9000 }, testOptions())).toThrow(
			'PI WEB config agent contains unknown key "futureSetting"',
		);
	...
	it("exposes the default upload folder in the effective config", () => {
		expect(effectivePiWebConfig(testOptions()).config.uploads).toEqual({
			defaultFolder: DEFAULT_UPLOADS_FOLDER,
		});
	});
	...
	it("rejects upload defaults that are not workspace-relative", async () => {
		await writeFile(
			configPath,
			`${JSON.stringify({ uploads: { defaultFolder: "../outside" } }, null, 2)}\n`,
			"utf8",
		);

		expect(() => loadPiWebConfig(testOptions())).toThrow(
			"PI WEB config uploads.defaultFolder must not contain path traversal",
		);
	});
});
...
describe("maxUploadBytes", () => {
	...
	it("falls back to config when env is unset or invalid", () => {
		expect(
			maxUploadBytes(
				{ PI_WEB_MAX_UPLOAD_BYTES: "not-a-number" },
				{ maxUploadBytes: 555 },
			),
		).toBe(555);
	});
...
describe("subsessionsEnabled", () => {
	...
	it("lets the env var override the config in both directions", () => {
		expect(
			subsessionsEnabled({ PI_WEB_SUBSESSIONS: "1" }, { subsessions: false }),
		).toBe(true);
		expect(
			subsessionsEnabled({ PI_WEB_SUBSESSIONS: "0" }, { subsessions: true }),
		).toBe(false);
	});
});
