...
import type {
	PiWebAgentDirEnvSource,
	PiWebConfigValues,
} from "./shared/apiTypes.js";
import {
	isPiCompanionCommand,
	usesPiCodingAgentStateCompatibility,
} from "./shared/activeAgentProfile.js";
...
export interface EffectivePiWebConfig
	extends Omit<
		PiWebConfig,
		"uploads" | "spawnSessions" | "subsessions" | "agent"
	> {
...
export interface LoadedEffectivePiWebConfig
	extends Omit<LoadedPiWebConfig, "config"> {
...
export function defaultPiWebConfigPath(
...
export const PI_WEB_DEFAULT_RUNTIME_ENV = "PI_WEB_DEFAULT_RUNTIME";
export const PI_WEB_OMP_COMMAND_ENV = "PI_WEB_OMP_COMMAND";
export const PI_WEB_OMP_AGENT_DIR_ENV = "PI_WEB_OMP_AGENT_DIR";
...
export interface EffectiveOmpRuntimeConfig {
	command: string;
	dir: string;
}
...
export function effectiveAgentConfig(
	env: NodeJS.ProcessEnv = process.env,
	config: Pick<PiWebConfig, "agent"> = {},
): EffectivePiWebAgentConfig {
	const command = parseAgentCommand(
		envValue(env, PI_WEB_AGENT_COMMAND_ENV) ??
	...
	const configuredDir =
		envValue(env, PI_WEB_AGENT_DIR_ENV) ??
		(usesPiCodingAgentStateCompatibility(command)
			? envValue(env, PI_CODING_AGENT_DIR_ENV)
			: undefined) ??
		config.agent?.dir ??
		defaultAgentDirForCommand(command, env);
	return {
		command,
...
export function effectiveOmpRuntimeConfig(
	env: NodeJS.ProcessEnv = process.env,
	config: Pick<PiWebConfig, "agentRuntimes"> = {},
): EffectiveOmpRuntimeConfig {
	const command = parseAgentCommand(
		envValue(env, PI_WEB_OMP_COMMAND_ENV) ??
			config.agentRuntimes?.omp?.command ??
			"omp",
		"agentRuntimes.omp.command",
		"environment",
		"current",
	);
	const configuredDir =
		envValue(env, PI_WEB_OMP_AGENT_DIR_ENV) ??
		config.agentRuntimes?.omp?.dir ??
		join(homedir(), ".omp", "agent");
	return {
		command,
		dir: resolveAgentDirPath(
			configuredDir,
			env,
			"agentRuntimes.omp.dir",
			"environment",
		),
	};
}

export function defaultAgentRuntimeId(
	env: NodeJS.ProcessEnv = process.env,
	config: Pick<PiWebConfig, "agentRuntimes"> = {},
): AgentRuntimeId {
	const value =
		envValue(env, PI_WEB_DEFAULT_RUNTIME_ENV) ??
		config.agentRuntimes?.default ??
		AGENT_RUNTIME_IDS.pi;
	if (!isAgentRuntimeId(value))
		throw new Error(
			`PI_WEB_DEFAULT_RUNTIME must be ${AGENT_RUNTIME_IDS.pi} or ${AGENT_RUNTIME_IDS.omp}`,
		);
	return value;
}

export function agentSessionDirEnvKeys(
...
export function effectiveUploadsConfig(
	config: Pick<PiWebConfig, "uploads"> = {},
): NonNullable<PiWebConfig["uploads"]> {
	return {
		defaultFolder: config.uploads?.defaultFolder ?? DEFAULT_UPLOADS_FOLDER,
	};
}
...
export function loadPiWebConfig(options: LoadOptions = {}): LoadedPiWebConfig {
	...
	const parsed = parseConfigJson(path);
	if (!isRecord(parsed))
...
export function effectivePiWebConfig(
	options: LoadOptions = {},
): LoadedEffectivePiWebConfig {
	return resolveEffectivePiWebConfig(loadPiWebConfig(options), options);
}

export function resolveEffectivePiWebConfig(
	loaded: LoadedPiWebConfig,
	options: LoadOptions = {},
): LoadedEffectivePiWebConfig {
	const env = options.env ?? process.env;
	...
	return {
		...
		config: {
			...
			...(port !== undefined && port !== ""
				? { port: parsePort(port, "PI_WEB_PORT") }
				: {}),
			...(allowedHosts !== undefined && allowedHosts !== ""
				? { allowedHosts: parseAllowedHostsEnv(allowedHosts) }
				: {}),
			...(maxUpload !== undefined && maxUpload !== ""
				? {
...
export function savePiWebConfig(
	config: PiWebConfig,
	options: LoadOptions = {},
): LoadedPiWebConfig {
	const env = options.env ?? process.env;
	...
	effectiveOmpRuntimeConfig(env, normalized);
	defaultAgentRuntimeId(env, normalized);
	...
	if (normalized.agentRuntimes !== undefined) delete existing["agentRuntimes"];
...
function parseConfigJson(path: string): unknown {
	try {
		return JSON.parse(readFileSync(path, "utf8"));
	} catch (error) {
		throw new Error(`PI WEB config contains invalid JSON: ${path}`, {
			cause: error,
		});
	}
}
...
function piWebConfigRecord(config: PiWebConfig): Record<string, unknown> {
	return {
		...
		...(config.pathAccess !== undefined
			? { pathAccess: config.pathAccess }
			: {}),
		...(config.uploads !== undefined ? { uploads: config.uploads } : {}),
		...(config.maxUploadBytes !== undefined
			? { maxUploadBytes: config.maxUploadBytes }
			: {}),
		...(config.spawnSessions !== undefined
			? { spawnSessions: config.spawnSessions }
			: {}),
		...(config.subsessions !== undefined
			? { subsessions: config.subsessions }
			: {}),
		...(config.agent !== undefined ? { agent: config.agent } : {}),
		...(config.agentRuntimes !== undefined
			? { agentRuntimes: config.agentRuntimes }
			: {}),
	};
}

function parsePiWebConfig(
	value: Record<string, unknown>,
	path: string,
): PiWebConfig {
	return {
		...(value["host"] !== undefined
			? { host: parseString(value["host"], "host", path) }
			: {}),
		...(value["port"] !== undefined
			? { port: parsePort(value["port"], "port", path) }
			: {}),
		...(value["allowedHosts"] !== undefined
			? { allowedHosts: parseAllowedHosts(value["allowedHosts"], path) }
			: {}),
		...(value["shortcuts"] !== undefined
			? { shortcuts: parseShortcuts(value["shortcuts"], path) }
			: {}),
		...(value["plugins"] !== undefined
			? { plugins: parsePlugins(value["plugins"], path) }
			: {}),
		...(value["pathAccess"] !== undefined
			? { pathAccess: parsePathAccessConfig(value["pathAccess"], path) }
			: {}),
		...(value["uploads"] !== undefined
			? { uploads: parseUploadsConfig(value["uploads"], path) }
			: {}),
		...(value["maxUploadBytes"] !== undefined
			...
			: {}),
		...(value["spawnSessions"] !== undefined
			? { spawnSessions: parseSpawnSessions(value["spawnSessions"], path) }
			: {}),
		...(value["subsessions"] !== undefined
			? { subsessions: parseSubsessions(value["subsessions"], path) }
			: {}),
		...(value["agent"] !== undefined
			? { agent: parseAgentConfig(value["agent"], path) }
			: {}),
		...(value["agentRuntimes"] !== undefined
			? {
					agentRuntimes: parseAgentRuntimesConfig(value["agentRuntimes"], path),
				}
			: {}),
	};
}
 ...
 */
export function spawnSessionsEnabled(
	env: NodeJS.ProcessEnv = process.env,
	config: PiWebConfig = {},
): boolean {
 ...
 */
export function subsessionsEnabled(
	env: NodeJS.ProcessEnv = process.env,
	config: PiWebConfig = {},
): boolean {
...
const AGENT_RUNTIMES_CONFIG_KEYS = new Set(["default", "omp"]);
const SAFE_BARE_AGENT_COMMAND_PATTERN = /^[A-Za-z0-9_][A-Za-z0-9._+-]*$/u;

export type AgentPathHost = "current" | "portable";

export function parseAgentRuntimesConfig(
	value: unknown,
	path: string,
	pathHost: AgentPathHost = "current",
): NonNullable<PiWebConfig["agentRuntimes"]> {
	if (!isRecord(value))
		throw new Error(`PI WEB config agentRuntimes must be an object: ${path}`);
	const unknownKey = Object.keys(value).find(
		(key) => !AGENT_RUNTIMES_CONFIG_KEYS.has(key),
	);
	if (unknownKey !== undefined)
		throw new Error(
			`PI WEB config agentRuntimes contains unknown key ${JSON.stringify(unknownKey)}: ${path}`,
		);
	const defaultRuntime = value["default"];
	if (defaultRuntime !== undefined && !isAgentRuntimeId(defaultRuntime))
		throw new Error(
			`PI WEB config agentRuntimes.default must be pi or omp: ${path}`,
		);
	return {
		...(defaultRuntime === undefined ? {} : { default: defaultRuntime }),
		...(value["omp"] === undefined
			? {}
			: { omp: parseAgentConfig(value["omp"], path, pathHost) }),
	};
}
...
export function parseAgentConfig(
	...
	path: string,
	pathHost: AgentPathHost = "current",
): NonNullable<PiWebConfig["agent"]> {
	if (!isRecord(value))
		throw new Error(`PI WEB config agent must be an object: ${path}`);
	const unknownKey = Object.keys(value).find(
		(key) => !AGENT_CONFIG_KEYS.has(key),
	);
	if (unknownKey !== undefined)
		throw new Error(
			`PI WEB config agent contains unknown key ${JSON.stringify(unknownKey)}: ${path}`,
		);
	const command = value["command"];
	const dir = value["dir"];
	return {
		...(command !== undefined
			? { command: parseAgentCommand(command, "agent.command", path, pathHost) }
			: {}),
		...(dir !== undefined
			? { dir: parseAgentDir(dir, "agent.dir", path, pathHost) }
			: {}),
	};
}
...
function parseAgentCommand(
	...
	if (!isSafeAgentCommand(command, pathHost)) {
		...
		throw new Error(
			`PI WEB config ${key} must be a safe bare executable name or ${absoluteLabel} executable path: ${path}`,
		);
	}
	return command;
}

function parseAgentDir(
	value: unknown,
	key: string,
	path: string,
	pathHost: AgentPathHost,
	...
	const isAbsoluteDir =
		pathHost === "current"
			? isHostAbsoluteAgentDir(dir)
			: isPortableAbsoluteAgentPath(dir);
	if (!isAbsoluteDir && !isHomePath(dir, pathHost)) {
		const absoluteLabel =
			pathHost === "current" ? "a host-absolute" : "an absolute";
		throw new Error(
			`PI WEB config ${key} must be ${absoluteLabel} path or start with ~: ${path}`,
		);
	}
	return dir;
}

function resolveAgentDirPath(
	value: string,
	env: NodeJS.ProcessEnv,
	key: string,
	path: string,
	...
	if (!isHostAbsoluteAgentDir(expanded)) {
		throw new Error(
			`PI WEB config ${key} must resolve to a host-absolute path: ${path}`,
		);
	}
...
function isSafeAgentCommand(value: string, pathHost: AgentPathHost): boolean {
	...
	if (pathHost === "current")
		return isAbsolute(value) && basename(value) !== "";
...
function parsePort(value: unknown, key: string, path = "environment"): number {
	...
	if (!Number.isInteger(port) || port < 1 || port > 65535)
		throw new Error(
			`PI WEB config ${key} must be an integer from 1 to 65535: ${path}`,
		);
	return port;
}
...
function parseAllowedHosts(value: unknown, path: string): string[] | true {
	...
	if (!isNonEmptyStringArray(value)) {
		throw new Error(
			`PI WEB config allowedHosts must be true or an array of non-empty strings: ${path}`,
		);
	}
	return value;
}
...
export function parsePathAccessConfig(
	value: unknown,
	path: string,
): NonNullable<PiWebConfigValues["pathAccess"]> {
	if (!isRecord(value))
		throw new Error(`PI WEB config pathAccess must be an object: ${path}`);
	const allowedPaths = value["allowedPaths"];
	return {
		...(allowedPaths !== undefined
			? { allowedPaths: parseAllowedPaths(allowedPaths, path) }
			: {}),
	};
}
...
function parseAllowedPaths(value: unknown, path: string): string[] {
	if (!isNonEmptyStringArray(value))
		throw new Error(
			`PI WEB config pathAccess.allowedPaths must be an array of non-empty strings: ${path}`,
		);
	return value;
}

export function parseUploadsConfig(
	value: unknown,
	path: string,
): NonNullable<PiWebConfigValues["uploads"]> {
	if (!isRecord(value))
	...
	return {
		...(defaultFolder !== undefined
			? {
					defaultFolder: parseWorkspaceRelativeFolder(
						defaultFolder,
						"uploads.defaultFolder",
						path,
					),
				}
			: {}),
...
function parseWorkspaceRelativeFolder(
	value: unknown,
	key: string,
	path: string,
): string {
	if (typeof value !== "string" || value.trim() === "")
		throw new Error(
			`PI WEB config ${key} must be a non-empty workspace-relative path: ${path}`,
		);
	...
	if (parts.some((part) => part === ".."))
		throw new Error(
			`PI WEB config ${key} must not contain path traversal: ${path}`,
		);
	return parts.join("/");
}
...
function isHomePath(value: string, pathHost: AgentPathHost): boolean {
	return (
		value === "~" ||
		value.startsWith("~/") ||
		((pathHost === "portable" || process.platform === "win32") &&
			value.startsWith("~\\"))
	);
}
...
function defaultAgentDirForCommand(
	command: string,
	env: NodeJS.ProcessEnv,
): string {
	if (usesPiCodingAgentStateCompatibility(command))
		return expandHomePath("~/.pi/agent", env);
	throw new Error(
		`PI WEB config agent.dir or ${PI_WEB_AGENT_DIR_ENV} is required when agent.command is ${JSON.stringify(command)}`,
	);
}
	...
	return Object.fromEntries(
		Object.entries(value).map(([actionId, shortcut]) => {
			if (
				...
				throw new Error(
					`PI WEB config shortcut values must be non-empty strings or null: ${path}`,
				);
			}
...
function parsePlugins(
	value: unknown,
	path: string,
): NonNullable<PiWebConfigValues["plugins"]> {
	...
	return Object.fromEntries(
		Object.entries(value).map(([pluginId, config]) => {
			if (!isPiWebPluginId(pluginId))
				throw new Error(
					`PI WEB config plugin ids must match ${piWebPluginIdPattern.source}: ${path}`,
				);
			if (!isRecord(config) || Array.isArray(config))
				throw new Error(
					`PI WEB config plugin entries must be objects: ${path}`,
				);
			const enabled = config["enabled"];
			if (enabled !== undefined && typeof enabled !== "boolean")
				throw new Error(
					`PI WEB config plugin enabled values must be booleans: ${path}`,
				);
			...
			if (
				...
				throw new Error(
					`PI WEB config plugin settings must be objects: ${path}`,
				);
