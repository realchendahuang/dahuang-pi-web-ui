...
function validateDevCheckout(root: string): void {
	...
	if (!existsSync(packageJsonPath)) {
		throw new Error(
			`Development mode must be installed from a PI WEB checkout. Missing package.json: ${packageJsonPath}`,
		);
	}
...
async function writeInitialConfig(
	...
	if (!existsSync(configPath)) {
		await writeFile(
			configPath,
			examplePiWebConfig({ host: options.host, port: Number(options.port) }),
		);
	}
}
...
function printServiceStatus(status: ServiceRuntimeStatus): void {
	...
	console.log(
		`${icon} ${serviceDisplayName(status.ref)}: ${status.detail} (${status.target}${pid})`,
	);
...
function printServiceStatusReport(backend: ServiceBackend): boolean {
	...
	console.log(
		`PI WEB services: ${serviceInstallMode(backend)} (${backend.label})`,
	);
	if (refs.length === 0) {
...
async function install(args: string[]): Promise<void> {
	...
	if (!printNodePtyNativeModuleCheck()) {
		throw new Error(
			"Install preflight checks failed without changing config or services. Fix the failure above, then run `pi-web doctor` for more detail.",
		);
	}
...
async function uninstall(): Promise<void> {
	...
	console.log(
		`PI WEB ${backend.label} removed. Production and development service files were removed; config and data were left in place.`,
	);
}
...
function printOptionalDoctorChecks(): void {
	...
	if (missingOptionalTool) {
		...
		console.log(
			"  PI WEB falls back to a bounded filesystem scan when rg is unavailable.",
		);
	}
}
...
function printPathSetupAdvice(
	...
	if (shell.name === "bash") {
		...
		console.log(
			"  Avoid relying on prompt hooks; PI WEB services run non-interactive login shells.",
		);
	} else {
		console.log(
...
async function doctor(): Promise<void> {
	...
	if (supportsSystemdUserServices()) {
		...
		if (linger === true) {
			...
			console.log(
				`  Recommended on servers: sudo loginctl enable-linger ${userInfo().username}`,
			);
		} else {
...
export function openBrowser(
	...
	if (invocation === undefined) {
		return {
			ok: false,
			detail: `No browser opener for platform ${platform}. Open ${url} manually.`,
		};
	}
	...
	if (result.status !== 0)
		return { ok: false, detail: `exit code ${String(result.status ?? 1)}` };
 ...
 * Install is `pi install` (package). This command only brings the UI up:
 * if user services are missing it sets them up automatically, starts them,
...
async function up(args: string[]): Promise<void> {
	...
	if (!servicesInstalled(backend)) {
		if (noInstall) {
			console.log("PI WEB user services are not set up yet.");
			console.log("Run: pi-web up   (sets up services automatically)");
			process.exitCode = 1;
			return;
		}
		console.log("Setting up PI WEB user services...");
		await install([]);
