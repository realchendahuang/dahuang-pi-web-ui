...
export function registerSessionProxyRoutes(
	...
	const proxy = async (
		...
		try {
			const upstream = await daemon.request(
				request.method,
				stripPrefix(request.url, prefix),
				request.body,
			);
	...
	app.get(`${prefix}/sessiond/health`, (_request, reply) =>
		proxy({ method: "GET", url: `${prefix}/health` }, reply),
	);
	app.get(`${prefix}/sessiond/runtime`, (_request, reply) =>
		proxy({ method: "GET", url: `${prefix}/runtime` }, reply),
	);
	...
	app.get<{ Params: { sessionId: string } }>(
		...
		(socket, request) => {
			bridgeSockets(
				socket,
				daemon.connectWebSocket(stripPrefix(request.url, prefix)),
			);
		},
	);
	...
	app.all(`${prefix}/runtimes`, (request, reply) => proxy(request, reply));
...
function stripPrefix(url: string, prefix: string): string {
	...
	const stripped = path.startsWith(prefix)
		? `${path.slice(prefix.length)}${query}`
		: url;
...
function parseJson(text: string): unknown {
	try {
		...
		throw new Error("Session daemon returned invalid JSON", { cause: error });
	}
}
