...
import {
	effectivePiWebCapabilities,
	WEB_RUNTIME_CAPABILITIES,
} from "../shared/capabilities.js";
import { piWebDockerCommand } from "../docker/piWebDockerCommandPlan.js";
import {
	parsePiWebComponentStatus,
	parsePiWebRuntimeComponent,
} from "../shared/piWebStatusParsing.js";
import { SessionDaemonClient } from "../sessiond/sessionDaemonClient.js";
import {
	isHostAbsoluteAgentDir,
	isPiCompanionCommand,
	isSafeAgentCommandForHost,
	PI_CODING_AGENT_DIR_ENV,
} from "../config.js";
import {
	createPiWebReleaseLookupCache,
	type PiWebReleaseLookup,
} from "./piWebReleaseLookupCache.js";
...
const latestReleaseLookupCache = createPiWebReleaseLookupCache(
	fetchLatestNpmVersion,
);
const runtimePackageInfo = readPackageInfoSync();

export function getPiWebRuntimeComponent(
...
export async function getPiWebComponentStatus(
	component: PiWebServiceComponent,
	options: PiWebStatusOptions = {},
): Promise<PiWebComponentStatus> {
...
export async function getPiWebVersionStatus(
	daemon: PiWebStatusDaemon = new SessionDaemonClient(),
	options: PiWebStatusOptions = {},
...
export async function getPiWebStatus(
	daemon: PiWebStatusDaemon = new SessionDaemonClient(),
	options: PiWebStatusOptions = {},
): Promise<PiWebStatusResponse> {
	const versionStatus = await getPiWebVersionStatus(daemon, options);
	const { web, sessiond } = versionStatus.components;
	const release = await getLatestReleaseStatus(
		web.installedVersion ?? web.runtimeVersion ?? DEFAULT_VERSION,
		options.forceReleaseCheck === true,
	);
...
async function detectPiWebInstallation(
	agentDir?: string,
): Promise<PiWebInstallationInfo> {
	...
	if (agentDir !== undefined) {
		const piPackage = await detectPiPackageInstallation(
			realRoot,
			root,
			agentDir,
		);
...
function detectDockerInstallation(): PiWebInstallationInfo | undefined {
	...
	const dockerMode =
		dockerModeFromEnv(process.env["PI_WEB_DOCKER_MODE"]) ??
		inferredDockerModeFromRoots();
...
function dockerRootPathFromEnv(
	mode: PiWebInstallationInfo["dockerMode"] | undefined,
): string | undefined {
	if (mode === "dev")
...
async function detectPiPackageInstallation(
	...
	try {
		...
		for (const configuredPackage of packageManager.listConfiguredPackages()) {
			...
			if (
				isSameOrWithin(realInstalledPath, realRoot) ||
				isSameOrWithin(realRoot, realInstalledPath)
			) {
				return {
					kind: "pi-package",
					path: displayPath,
...
async function getSessiondRuntimeComponent(
	daemon: PiWebStatusDaemon,
): Promise<PiWebRuntimeComponent> {
	try {
		const upstream = await daemon.request("GET", "/runtime");
		if (upstream.statusCode < 200 || upstream.statusCode >= 300) {
			return (
				(await legacySessiondRuntimeComponent(daemon)) ??
				unavailableSessiondRuntime(
					`runtime check returned HTTP ${String(upstream.statusCode)}`,
				)
			);
		}
		const parsed: unknown =
			upstream.body === "" ? undefined : JSON.parse(upstream.body);
		const runtime = parsePiWebRuntimeComponent(parsed);
		if (runtime !== undefined) return runtime;
		const legacyVersion = isRecord(parsed)
			? parsePiWebComponentStatus(parsed["version"])
			: undefined;
		if (legacyVersion !== undefined)
			return runtimeComponentFromStatus(legacyVersion);
		return (
			(await legacySessiondRuntimeComponent(daemon)) ??
			unavailableSessiondRuntime(
				"runtime response did not include valid runtime information",
			)
		);
	} catch (error) {
		return unavailableSessiondRuntime(
...
async function getSessiondComponentStatus(
	...
	try {
		...
		if (upstream.statusCode < 200 || upstream.statusCode >= 300) {
			return (
				(await legacySessiondComponentStatus(daemon)) ??
				unavailableSessiond(
					`runtime check returned HTTP ${String(upstream.statusCode)}`,
				)
			);
		...
		return unavailableSessiond(
			error instanceof Error ? error.message : String(error),
		);
	}
}

async function legacySessiondRuntimeComponent(
	daemon: PiWebStatusDaemon,
): Promise<PiWebRuntimeComponent | undefined> {
...
async function legacySessiondComponentStatus(
	daemon: PiWebStatusDaemon,
): Promise<PiWebComponentStatus | undefined> {
	try {
...
function runtimeComponentFromStatus(
	status: PiWebComponentStatus,
): PiWebRuntimeComponent {
	return {
		component: status.component,
		label: status.label,
		...(status.runtimeVersion === undefined
			? {}
...
async function getLatestReleaseStatus(
	currentVersion: string,
	force: boolean,
): Promise<PiWebReleaseStatus> {
	...
	return releaseStatusFromCache(
		await latestReleaseLookupCache.get(currentVersion, { force }),
		currentVersion,
	);
}

function releaseStatusFromCache(
	cache: PiWebReleaseLookup,
	currentVersion: string,
): PiWebReleaseStatus {
	return {
		packageName: PI_WEB_PACKAGE_NAME,
		...
		updateAvailable:
			cache.latestVersion === undefined
				? false
				: isNewerPackageVersion(cache.latestVersion, currentVersion),
...
async function fetchLatestNpmVersion(currentVersion: string): Promise<string> {
	const response = await fetch(
		`https://registry.npmjs.org/${encodeURIComponent(PI_WEB_PACKAGE_NAME)}/latest`,
		{
			headers: {
...
async function commandsFor(
	...
	const restartSessiond =
		serviceCommands.restartSessiond ?? cliCommands.restart;
...
function dockerCommands(
	...
	return {
		...
		restartSessiond: piWebDockerCommand(
...
async function piWebCliCommands(
	installation: PiWebInstallationInfo | undefined,
): Promise<NativeServiceCommands> {
...
export async function updateCommandFor(
	...
	if (installation?.kind === "pi-package") {
		...
		if (
			profile === undefined ||
			!isSafeAgentCommandForHost(profile.command) ||
			!isHostAbsoluteAgentDir(profile.dir) ||
			!isPiCompanionCommand(profile.command)
		)
			return undefined;
	...
	if (installation?.kind === "local" && installation.path !== undefined) {
		if (
			...
			!(await isGitCheckoutWithUpstream(installation.path))
		)
			return undefined;
...
async function nativeServiceCommands(): Promise<NativeServiceCommands> {
	...
	return {
		...(restartable.length === 0
			? {}
			: {
					restart: restartNativeServicesCommand(
						backend,
						restartable,
						"pi-web-restart",
					),
		...
		...(web.length === 0
			? {}
			: {
					restartWeb: restartNativeServicesCommand(
						backend,
						web,
						"pi-web-restart-web",
					),
		...
		...(sessiond.length === 0
			? {}
			: {
					restartSessiond: restartNativeServicesCommand(
						backend,
						sessiond,
						"pi-web-restart-sessiond",
					),
				}),
		...(status.length === 0
			? {}
			: { status: statusNativeServicesCommand(backend, status) }),
	};
}

async function nativeServiceBackend(): Promise<
	NativeServiceBackendKind | undefined
> {
...
function installedServiceIds(
	backend: NativeServiceBackendKind,
): Set<ServiceId> {
	return new Set(
		startServiceOrder.filter((id) =>
			existsSync(serviceFilePath(backend, serviceRefs[id])),
		),
	);
}
...
function installedServiceRefs(
	...
	order: ServiceId[] = startServiceOrder,
): NativeServiceRef[] {
	return order
		.filter((id) => candidates.includes(id) && installed.has(id))
		.map((id) => serviceRefs[id]);
}

function serviceFilePath(
	backend: NativeServiceBackendKind,
	ref: NativeServiceRef,
): string {
...
function restartNativeServicesCommand(
	backend: NativeServiceBackendKind,
	refs: NativeServiceRef[],
	systemdUnit: string,
): string {
	...
	return refs
		.map((ref) => `launchctl kickstart -k gui/$(id -u)/${ref.launchdLabel}`)
		.join(" && ");
}

function statusNativeServicesCommand(
	backend: NativeServiceBackendKind,
	refs: NativeServiceRef[],
): string {
	if (backend === "systemd")
	...
	return refs
		.map((ref) => `launchctl print gui/$(id -u)/${ref.launchdLabel}`)
		.join(" && ");
}
...
function hasCommand(command: string): Promise<boolean> {
	return commandSucceeds("/usr/bin/env", [
		"sh",
		"-c",
...
function buildMessages(
	...
	const installedVersion =
		components.web.installedVersion ?? components.web.runtimeVersion;
...
function optionalMessageCommand(
	command: string | undefined,
): Pick<PiWebStatusMessage, "command"> | object {
...
function isInstalledVersionNewer(
...
function isNewerPackageVersion(
