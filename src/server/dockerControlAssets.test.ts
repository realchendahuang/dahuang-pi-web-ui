...
describe("Docker command assets", () => {
	...
	it("keeps shell entrypoints syntactically valid", async () => {
		await Promise.all([
			...
			execUtf8(
				"sh",
				["-n", join(repoRoot, "docker", "install.sh")],
				process.env,
			),
			execUtf8(
				"sh",
				["-n", join(repoRoot, "docker", "internal", "dev", "compose")],
				process.env,
			),
			execUtf8(
				"bash",
				[
					"-n",
					join(repoRoot, "docker", "internal", "dev", "sync-node-modules"),
				],
				process.env,
			),
			execUtf8(
				"sh",
				["-n", join(repoRoot, "docker", "internal", "host-profile.sh")],
				process.env,
			),
		]);
	});
	...
	it("packages the canonical Docker command and internal support assets", async () => {
		...
		const customImageHooksIndex = devDockerfile.indexOf(
			"for script in /tmp/pi-web-custom-image.d/*.sh",
		);
		const dependencyGenerationIndex = devDockerfile.indexOf(
		...
		expect(dockerfile).toContain(
			'"@jmfederico/pi-web@${PI_WEB_VERSION}" --allow-scripts=node-pty',
		);
		expect(dockerfile).toContain(
		...
		expect(devDockerfile).toContain(
			"/opt/pi-web-dev-dependencies/node_modules",
		);
	...
	dockerCommandIt(
		"fetches remote installer assets without clobbering the write target",
		async () => {
			...
			expect(await readFile(join(installDir, "Dockerfile"), "utf8")).toContain(
				"COPY pi-web-docker /usr/local/bin/pi-web-docker",
			);
			expect(
				await readFile(join(installDir, "pi-web-docker"), "utf8"),
			).toContain("Usage: pi-web-docker");
	...
	dockerCommandIt(
		"runs status through Docker Compose in the foreground",
		async () => {
			...
			const result = await runDockerCommand(
				["status"],
				runtimeEnv(fakeDocker, installDir),
			);

			expect(result.stdout).toContain(
				"fake docker compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml ps",
			);
			const log = await readFile(fakeDocker.logPath, "utf8");
			expect(log).toContain("compose version");
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml ps",
			);
	...
	dockerCommandIt(
		"runs production host lifecycle commands through the generated runtime env",
		async () => {
			const installDir = await createRuntimeInstall();
			const fakeDocker = await installFakeDocker();
			...
			const log = await readFile(fakeDocker.logPath, "utf8");
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml up -d",
			);
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml down",
			);
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml restart sessiond",
			);
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml logs -f web",
			);
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml exec sessiond bash",
			);
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml exec web pi-web config show",
			);
			expect(log).not.toContain("run -d");
		},
	);
	...
	dockerCommandIt(
		"ignores ambient Compose project names for runtime lifecycle commands",
		async () => {
			const installDir = await createRuntimeInstall();
			const fakeDocker = await installFakeDocker();
	...
	dockerCommandIt(
		"runs development commands through generated env while preserving host user ids",
		async () => {
			...
			await withUnixSocket(socketPath, async () => {
				await runDockerCommand(
					["--dev", "status"],
					devHostEnv(fakeDocker, devRoot, home, {
						PI_WEB_DOCKER_RUNTIME_ENV_FILE: runtimeEnvFile,
					}),
				);
			});

			const generatedEnvPath = join(
				devRoot,
				".pi-web",
				"docker-compose-dev.generated.env",
			);
			...
			expect(generatedEnv).not.toContain(
				"COMPOSE_PROJECT_NAME=runtime-project",
			);
			...
			expect(regeneratedEnv).toContain(
				`PI_WEB_DOCKER_DATA_DIR=${runtimeDataDir}\n`,
			);
			expect(regeneratedEnv).not.toContain(
				"COMPOSE_PROJECT_NAME=ambient-dev-project",
			);

			const localConfig = await readFile(
				join(devRoot, ".pi-web", "docker-compose-dev.local.env"),
				"utf8",
			);
			expect(localConfig).toContain(
				"docker/pi-web-docker --dev creates this file once",
			);
			expect(localConfig).toContain(
				"PI_WEB_UID and PI_WEB_GID default to the current host user",
			);
			const override = await readFile(
				join(devRoot, ".pi-web", "docker-compose-dev.host.generated.yml"),
				"utf8",
			);
			...
			const log = await readFile(fakeDocker.logPath, "utf8");
			expect(log).toContain(
				`compose --project-name pi-web-dev --env-file ${generatedEnvPath} -f ${devRoot}/docker/compose.dev.yml -f ${devRoot}/.pi-web/docker-compose-dev.host.generated.yml ps`,
			);
		},
	);

	dockerCommandIt(
		"rejects development commands as root unless explicitly allowed",
		async () => {
			const fakeDocker = await installFakeDocker();
			...
			expect(result.stderr).toContain(
				"refusing to run Docker development mode as root",
			);
		},
	);

	dockerCommandIt(
		"passes the root override through to the development compose helper",
		async () => {
			const fakeDocker = await installFakeDocker();
			await installFakeId(fakeDocker.binDir, 0, 0);
	...
	dockerCommandIt(
		"refuses development updates when the checkout has uncommitted files",
		async () => {
			const helperLog = join(tempDir, "dev-helper.log");
			...
			expect(result.exitCode).not.toBe(0);
			expect(result.stderr).toContain(
				"refusing to update the Docker development stack because the checkout has uncommitted changes",
			);
	...
	dockerCommandIt(
		"refuses dirty development updates before scheduling a detached helper",
		async () => {
			const helperLog = join(tempDir, "dev-helper.log");
			const devRoot = await createCleanDevGitRepoWithFakeHelper(helperLog);
			const fakeDocker = await installFakeDocker();
			...
			const result = await runDockerCommandAllowFailure(
				["--dev", "update"],
				devRuntimeEnv(fakeDocker, devRoot),
			);

			expect(result.exitCode).not.toBe(0);
			expect(result.stderr).toContain("checkout has uncommitted changes");
			expect(result.stdout).not.toContain(
				"Started detached PI WEB Docker helper",
			);
	...
	dockerCommandIt(
		"refuses development updates while a Git operation is in progress",
		async () => {
			const helperLog = join(tempDir, "dev-helper.log");
			const devRoot = await createCleanDevGitRepoWithFakeHelper(helperLog);
			const fakeDocker = await installFakeDocker();
	...
	dockerCommandIt(
		"allows clean development updates and dirty development starts",
		async () => {
			const helperLog = join(tempDir, "dev-helper.log");
			const devRoot = await createCleanDevGitRepoWithFakeHelper(helperLog);
			const fakeDocker = await installFakeDocker();
			...
			await writeFile(
				join(devRoot, "in-progress-work.txt"),
				"dirty by design\n",
				"utf8",
			);
	...
	dockerCommandIt(
		"starts development detached helpers as the generated dev user",
		async () => {
			const devRoot = await createDevGeneratedEnv({
				uid: 1234,
				gid: 2345,
				dockerGid: 3456,
			});
			...
			await runDockerCommand(
				["--dev", "restart-sessiond"],
				devRuntimeEnv(fakeDocker, devRoot),
			);

			const log = await readFile(fakeDocker.logPath, "utf8");
			expect(log).toContain(
				`--env-file ${devRoot}/.pi-web/docker-compose-dev.generated.env`,
			);
			...
			expect(log).toContain(
				"pi-web:test pi-web-docker --dev __run-detached restart-sessiond",
			);
	...
	dockerCommandIt(
		"rejects inside-container commands whose explicit mode does not match the container mode",
		async () => {
			...
			expect(result.exitCode).not.toBe(0);
			expect(result.stderr).toContain(
				"this PI WEB Docker container is in dev mode",
			);
		},
	);

	dockerCommandIt(
		"routes production install to the bootstrap installer",
		async () => {
			const result = await runDockerCommand(
				["install", "--help"],
				cleanProcessEnv(),
	...
	dockerCommandIt(
		"explains source checkout runtime-mode mistakes",
		async () => {
			const fakeDocker = await installFakeDocker();
			...
			expect(result.stderr).toContain(
				"running this checkout's Docker command in runtime mode",
			);
			expect(result.stderr).toContain("./docker/pi-web-docker --dev start");
			expect(result.stderr).toContain(
				"/home/pi-web-test/.local/share/pi-web-docker/pi-web-docker start",
			);
	...
	dockerCommandIt(
		"starts restart-sessiond in a detached Docker helper",
		async () => {
			const installDir = await createRuntimeInstall();
			const fakeDocker = await installFakeDocker();
			...
			expect(result.stdout).toContain(
				"Streaming detached PI WEB Docker helper logs inline.",
			);
			expect(result.stdout).toContain(
				"Reconnect with: docker logs -f pi-web-docker-restart-sessiond-",
			);
			...
			expect(log).toContain(
				"pi-web:test pi-web-docker __run-detached restart-sessiond",
			);
			expect(log).not.toContain("--user 0:0");
			expect(log).not.toContain(
				"compose -f compose.yml -f compose.override.yml restart sessiond",
			);
		},
	);

	dockerCommandIt(
		"executes the detached restart-sessiond action through Compose",
		async () => {
			const installDir = await createRuntimeInstall();
			const fakeDocker = await installFakeDocker();

			await runDockerCommand(
				["__run-detached", "restart-sessiond"],
				runtimeEnv(fakeDocker, installDir),
			);
	...
	dockerCommandIt(
		"executes the detached runtime update action through Compose without nesting helpers",
		async () => {
			const installDir = await createRuntimeInstall();
			const fakeDocker = await installFakeDocker();

			await runDockerCommand(
				["__run-detached", "update"],
				runtimeEnv(fakeDocker, installDir),
			);

			const log = await readFile(fakeDocker.logPath, "utf8");
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml build --pull --no-cache",
			);
			expect(log).toContain(
				"compose --project-name pi-web-test --env-file .env -f compose.yml -f compose.override.yml up -d --force-recreate --remove-orphans",
			);
			expect(log).not.toContain("run -d");
		},
	);
...
function runDockerCommandAllowFailure(
	args: string[],
	env: NodeJS.ProcessEnv,
...
async function createRuntimeInstall(): Promise<string> {
	...
	await writeFile(
		join(installDir, "compose.yml"),
		"name: pi-web\nservices: {}\n",
		"utf8",
	);
	await writeFile(
		join(installDir, "compose.override.yml"),
		"services: {}\n",
		"utf8",
	);
	return installDir;
...
async function createDevRepoFixture(): Promise<string> {
	...
	await copyFile(
		join(repoRoot, "docker", "internal", "dev", "compose"),
		join(devRoot, "docker", "internal", "dev", "compose"),
	);
	await chmod(join(devRoot, "docker", "internal", "dev", "compose"), 0o755);
	await copyFile(
		join(repoRoot, "docker", "internal", "host-profile.sh"),
		join(devRoot, "docker", "internal", "host-profile.sh"),
	);
	await writeFile(
		join(devRoot, "docker", "compose.dev.yml"),
		"name: pi-web-dev\nservices: {}\n",
		"utf8",
	);
	return devRoot;
...
async function createDevRepoFixtureWithFakeHelper(
	logPath: string,
): Promise<string> {
...
async function createCleanDevGitRepoWithFakeHelper(
	logPath: string,
): Promise<string> {
	...
	await execUtf8(
		...
		[
			...
			devRoot,
			"-c",
			"user.name=PI WEB Test",
			"-c",
			"user.email=pi-web-test@example.invalid",
			"-c",
...
async function createDevGeneratedEnv(ids: {
	uid: number;
	gid: number;
	dockerGid: number;
}): Promise<string> {
	...
	await writeFile(
		join(devRoot, ".pi-web", "docker-compose-dev.generated.env"),
		[
