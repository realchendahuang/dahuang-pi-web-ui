...
export interface CommandRuntime<
	TSession extends CommandSession = CommandSession,
> {
	cwd: string;
	session: TSession;
	fork: (
...
export type GetCommandActiveSession<
	TSession extends CommandSession = CommandSession,
> = (sessionId: string) => Promise<CommandActiveSession<TSession>>;

export interface CommandEventPublisher {
	publish(sessionId: string, event: SessionUiEvent): void;
	publishGlobal?(
		event: Extract<SessionUiEvent, { type: "session.name" }>,
	): void;
}

export interface SessionCommandLifecycle<
	TSession extends CommandSession = CommandSession,
> {
...
> {
	...
	async run(sessionId: string, text: string): Promise<ClientCommandResult> {
		...
		return {
			type: "unsupported",
			message: `/${name} is not implemented in the web UI yet`,
		};
	}
	...
	async forkFromEntry(
		...
		return {
			...
			session: clientSessionFromRuntime(active.runtime),
	...
	private async reload(session: TSession): Promise<ClientCommandResult> {
		if (this.hasActiveWork(session))
			return {
				type: "unsupported",
				message:
					"Cannot reload while the session is active. Stop current activity before reloading.",
			};
		if (this.lifecycle.reloadSession === undefined)
			return {
				type: "unsupported",
				message: "/reload is not available for this session runtime.",
			};
		...
		return {
			type: "done",
			message:
				"Session runtime resources reloaded. Extensions, skills, prompt templates, themes, and context/system prompt files are refreshed for this session. Reload the browser page separately for PI WEB browser plugin changes.",
		};
	}
	...
	private async clone(
		...
		if (initialLeafId === null || initialLeafId === "")
			return {
				type: "unsupported",
				message: "Cannot clone: no current session entry",
			};
		...
		const result = await this.runSessionReplacement(
			active.runtime,
			async () => {
				const cloneResult = await active.runtime.fork(leafId, {
					position: "at",
				});
	...
	private tree(session: TSession): ClientCommandResult {
		if (this.hasActiveWork(session)) {
			return {
				type: "unsupported",
				message:
					"Cannot open the session tree while the session is active. Stop current activity and try /tree again.",
			};
		}
		...
		try {
			...
			return {
				type: "unsupported",
				message: `Unable to open the session tree: ${message}`,
			};
		}
	}
	...
	private hasActiveWork(session: TSession): boolean {
		return (
			...
			this.lifecycle.hasActiveWork?.(session) === true
		);
	}

	private runSessionReplacement<T>(
		runtime: CommandRuntime<TSession>,
		operation: () => Promise<T>,
	): Promise<T> {
		const runReplacement = this.lifecycle.runSessionReplacement;
		return runReplacement === undefined
			? operation()
			: runReplacement(runtime.session, operation);
	}

	private async nextRelatedSessionName(
		active: CommandActiveSession<TSession>,
		kind: RelatedSessionKind,
	): Promise<string> {
		...
		try {
			existingNames =
				(await this.naming.listSessionNames?.(active.runtime.cwd)) ?? [];
		} catch {
			existingNames = [];
		}
		return uniqueRelatedSessionName(
			sourceTitle,
			kind,
			sourceName === undefined ? existingNames : [...existingNames, sourceName],
		);
	}
	...
	private tryNameRelatedSession(session: TSession, name: string): void {
		try {
			...
			this.events.publish(session.sessionId, {
				type: "command.output",
				level: "error",
				message: `Session created, but naming failed: ${message}`,
			});
		}
	}
	...
	private isRuntimeCommand(session: TSession, name: string): boolean {
		return (
			...
			session.resourceLoader
				.getSkills()
				.skills.some((skill) => `skill:${skill.name}` === name)
		);
	}
}
...
function clientSessionFromRuntime(runtime: CommandRuntime): ClientSession {
	...
	const parentSessionPath =
		typeof session.sessionManager.getHeader === "function"
...
function uniqueRelatedSessionName(
	sourceTitle: string,
	kind: RelatedSessionKind,
	...
	const usedNames = new Set(
		existingNames.map(normalizedName).filter(isDefined),
	);
...
function forkActiveUnsupported(command: "fork" | "clone"): ClientCommandResult {
	return {
		type: "unsupported",
		message: `Cannot ${command} while the session is active. Stop current activity before ${command === "fork" ? "forking" : "cloning"}.`,
	};
}
...
function treeNavigationActiveUnsupported(): ClientCommandResult {
	return {
		type: "unsupported",
		message:
			"Cannot run commands while session tree navigation is active. Stop or finish the navigation first.",
	};
}
...
function promptDraft(
	...
	Pick<Extract<ClientCommandResult, { type: "done" }>, "promptDraft">
> {
