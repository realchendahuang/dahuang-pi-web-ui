...
if (process.platform === "win32") {
	throw new Error(
		"The installed-package PTY smoke test requires a POSIX shell",
	);
}
...
if (npmExecPath === undefined || npmExecPath === "") {
	throw new Error(
		"npm_execpath is required; run this check through `npm run smoke:package-install`",
	);
}
...
try {
	...
	console.log(
		`Installed-package PTY smoke test passed with npm ${NPM_VERSION}.`,
	);
} finally {
...
async function smokeInstalledTerminalService(packageRoot) {
	...
	const nodePtyPackageJsonPath = requireFromPackage.resolve(
		"node-pty/package.json",
	);
	const nodePtyPackage = parseJson(
		await readFile(nodePtyPackageJsonPath, "utf8"),
		nodePtyPackageJsonPath,
	);
	if (
		typeof nodePtyPackage.version !== "string" ||
		nodePtyPackage.version.includes("-")
	) {
		throw new Error(
			`Installed package resolved a non-stable node-pty version: ${String(nodePtyPackage.version)}`,
		);
	}

	const terminalModuleUrl = pathToFileURL(
		join(packageRoot, "dist", "server", "terminals", "terminalService.js"),
	).href;
	...
	try {
		...
		const exitCode = await new Promise((resolvePromise, reject) => {
			const timeout = setTimeout(
				() =>
					reject(
						new Error(
							`Timed out waiting for installed node-pty output: ${JSON.stringify(output)}`,
						),
					),
				10_000,
			);
		...
		detach();
		if (exitCode !== 0)
			throw new Error(`Installed PTY command exited with ${String(exitCode)}`);
		if (!output.includes(MARKER))
			throw new Error(
				`Installed PTY output did not contain ${MARKER}: ${JSON.stringify(output)}`,
			);
	} finally {
		service.dispose();
