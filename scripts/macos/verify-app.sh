...
if (manifest.schemaVersion !== 1 || manifest.keychainHelper?.path !== "Contents/Helpers/PiAgentKeychainHelper" || manifest.keychainHelper?.sha256 !== hash(helperPath)) throw new Error("Native Keychain helper manifest did not match helper bytes");
