...
| 优先级 | 交付 | 完成证据 |
| --- | --- | --- |
| P0 | 把 receipt 从 abort 扩展到 prompt、create/fork/import、archive/delete、terminal、Git 与 approval | 同 `commandId` 重试返回同一 receipt；runtime epoch 变化时返回可诊断的拒绝；端到端断线恢复测试。 |
